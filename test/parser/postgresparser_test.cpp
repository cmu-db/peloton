...
TEST_F(PostgresParserTests, InsertTest) {
  ...
  for (auto query : queries) {
    ...
    EXPECT_TRUE(((expression::ConstantValueExpression *)
                 insert_stmt->insert_values->at(0)->at(0).get())->GetValue().IsNull());
    // Test normal value
...
TEST_F(PostgresParserTests, CreateTest) {
  ...
  EXPECT_EQ(std::string(column->name.get()), "id");
  ...
  EXPECT_EQ("c_id", std::string(column->foreign_key_source->at(0).get()));
  EXPECT_EQ("cid", std::string(column->foreign_key_sink->at(0).get()));
  EXPECT_EQ("country", std::string(column->table_info_->table_name.get()));
...
TEST_F(PostgresParserTests, ConstraintTest) {
  ...
  EXPECT_EQ("bb", std::string(column->foreign_key_sink->at(0).get()));
  EXPECT_EQ("table2", std::string(column->table_info_->table_name.get()));
  ...
  EXPECT_TRUE(column->foreign_key_sink != nullptr);
  EXPECT_TRUE(column->foreign_key_sink->size() == 1);
  EXPECT_EQ("cc", std::string(column->foreign_key_sink->at(0).get()));
  EXPECT_EQ("table3", std::string(column->table_info_->table_name.get()));
  ...
  EXPECT_TRUE(column->foreign_key_sink != nullptr);
  EXPECT_TRUE(column->foreign_key_sink->size() == 1);
  EXPECT_EQ("dd", std::string(column->foreign_key_sink->at(0).get()));
  EXPECT_EQ("table4", std::string(column->table_info_->table_name.get()));
