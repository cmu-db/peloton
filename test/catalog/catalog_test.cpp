...
TEST_F(CatalogTests, CreatingDatabase) {
  global_catalog->CreateDatabase("EMP_DB");
  EXPECT_EQ(global_catalog->GetDatabaseWithName("EMP_DB")->GetDBName(), "EMP_DB");
}
...
TEST_F(CatalogTests, CreatingTable) {
  ...
  global_catalog->CreateTable(global_catalog->GetDatabaseWithName("EMP_DB")->GetOid(), "department_table", std::move(table_schema));
