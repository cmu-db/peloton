...
TEST_F(ConstraintsTests, CombinedPrimaryKeyTest) {
...

  {
    ...
    EXPECT_TRUE(ResultType::RESULT_TYPE_SUCCESS == scheduler.schedules[0].txn_result);
    EXPECT_TRUE(ResultType::RESULT_TYPE_ABORTED == scheduler.schedules[1].txn_result);
  }
}
...
TEST_F(ConstraintsTests, MultiTransactionUniqueConstraintsTest) {
...

  {
    ...
    EXPECT_TRUE((ResultType::RESULT_TYPE_SUCCESS == scheduler.schedules[0].txn_result &&
                 ResultType::RESULT_TYPE_ABORTED == scheduler.schedules[1].txn_result) ||
                (ResultType::RESULT_TYPE_SUCCESS == scheduler.schedules[1].txn_result &&
                 ResultType::RESULT_TYPE_ABORTED == scheduler.schedules[0].txn_result));
  }

  {
    ...
    EXPECT_TRUE(ResultType::RESULT_TYPE_ABORTED == scheduler.schedules[0].txn_result);
    EXPECT_TRUE(ResultType::RESULT_TYPE_SUCCESS == scheduler.schedules[1].txn_result);
  }
