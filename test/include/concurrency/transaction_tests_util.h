...
struct TransactionSchedule {
  ...
  TransactionSchedule(int schedule_id_, bool ro = false)
      : txn_result(RESULT_TYPE_FAILURE),
        stored_value(0),
...
class TransactionThread {
 public:
  ...
  void ExecuteNext() {
    ...
    if (schedule->txn_result == RESULT_TYPE_ABORTED) {
      cur_seq++;
      return;
    }
    ...
    switch (op) {
      ...
      case TXN_OP_COMMIT: {
        ...
        LOG_INFO("Txn %d commits: %s", schedule->schedule_id,
                 schedule->txn_result == RESULT_TYPE_SUCCESS ? "Success" : "Fail");
        txn = NULL;
        break;
      }
    ...
    if (txn != NULL && txn->GetResult() == RESULT_TYPE_FAILURE) {
      txn_manager->AbortTransaction(txn);
      txn = NULL;
      LOG_TRACE("ABORT NOW");
      if (execute_result == false) LOG_TRACE("Executor returns false");
      schedule->txn_result = RESULT_TYPE_ABORTED;
    }
  }
