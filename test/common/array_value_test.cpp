...
TEST_F(ArrayValueTests, GetElementTest) {
  ...
  Value array_bool = Value(Type::ARRAY, vec_bool,
    Type::BOOLEAN);
  for (size_t i = 0; i < n; i++) {
  ...
  Value array_tinyint = Value(Type::ARRAY, vec_tinyint,
    Type::TINYINT);
  for (size_t i = 0; i < n; i++) {
  ...
  Value array_smallint = Value(Type::ARRAY, vec_smallint,
    Type::SMALLINT);
  for (size_t i = 0; i < n; i++) {
  ...
  Value array_integer = Value(Type::ARRAY, vec_integer,
    Type::INTEGER);
  for (size_t i = 0; i < n; i++) {
  ...
  Value array_bigint = Value(Type::ARRAY, vec_bigint,
    Type::BIGINT);
  for (size_t i = 0; i < n; i++) {
  ...
  Value array_decimal = Value(Type::ARRAY, vec_decimal,
    Type::DECIMAL);
  for (size_t i = 0; i < n; i++) {
  ...
  Value array_varchar = Value(Type::ARRAY, vec_varchar,
    Type::VARCHAR);
  for (size_t i = 0; i < n; i++) {
...
TEST_F(ArrayValueTests, InListTest) {
  ...
  Value array_varchar = Value(Type::ARRAY, vec_varchar,
    Type::VARCHAR);
  for (size_t i = 0; i < n; i++) {
    Value in_list = array_varchar.InList(ValueFactory::GetVarcharValue(vec_varchar[i]));
    EXPECT_TRUE((in_list).IsTrue());
