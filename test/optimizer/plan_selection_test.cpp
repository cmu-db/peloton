...
class PlanSelectionTest : public PelotonTest {
 protected:
  ...
  std::string CreateThreeWayJoinQuery(const std::string &table_1,
    ...
    return CreateThreeWayJoinQuery(table_1, table_2, table_3, column_1, column_2, column_3, "", "");
  }
  ...
  std::string CreateThreeWayJoinQuery(const std::string &table_1,
    ...
    ss << "SELECT * FROM "
       ...
       << " AND "
       << table_2 << "." << column_2 << " = " << table_3 << "." << column_3;
    if (!order_by_column.empty() and !order_by_table.empty()) {
      ss << " ORDER BY " << order_by_table << "." << order_by_column;
    }
    ss << ";";
...
TEST_F(PlanSelectionTest, ThreeWayJoinTest) {
  ...
  auto plan = PerformTransactionAndGetPlan(CreateThreeWayJoinQuery(
      table_1_name, table_2_name, table_3_name, column_1_name, column_1_name, column_1_name));
...
/* The similarity between this and the SimpleJoinOrderSmallTest2 is to ensure that the optimizer does not pick
