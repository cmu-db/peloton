...
TEST_F(BinderCorrectnessTest, SelectStatementTest) {
  ...
  EXPECT_EQ(tupleExpr->bound_obj_id_,
  ...
  EXPECT_EQ(tupleExpr->bound_obj_id_, make_tuple(db_oid, tableA_oid, 0));  // a1
  ...
  EXPECT_EQ(tupleExpr->bound_obj_id_, make_tuple(db_oid, tableA_oid, 1));  // a1
  ...
  EXPECT_EQ(tupleExpr->bound_obj_id_, make_tuple(db_oid, tableA_oid, 0));
  tupleExpr =
      (expression::TupleValueExpression*)(selectStmt->select_list->at(1));
  EXPECT_EQ(tupleExpr->bound_obj_id_, make_tuple(db_oid, tableB_oid, 1));
...
TEST_F(BinderCorrectnessTest, DeleteStatementTest) {
  ...
  auto tupleExpr =
      (expression::TupleValueExpression*)deleteStmt->expr->GetChild(0)
          ->GetChild(1);
  EXPECT_EQ(tupleExpr->bound_obj_id_, make_tuple(db_oid, tableB_oid, 0));
