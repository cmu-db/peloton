...
TEST_F(ValueFactoryTests, CastTest) {
  ...
  EXPECT_THROW(
      type::ValueFactory::CastAsBigInt(type::Value(type::TypeId::BOOLEAN, 0)),
      peloton::Exception);
  EXPECT_THROW(type::ValueFactory::CastAsSmallInt(
                   type::Value(type::TypeId::INTEGER, type::PELOTON_INT32_MAX)),
               peloton::Exception);
  EXPECT_THROW(type::ValueFactory::CastAsTinyInt(
                   type::Value(type::TypeId::INTEGER, type::PELOTON_INT32_MAX)),
               peloton::Exception);

  type::Value v3(type::ValueFactory::CastAsVarchar(
  ...
  type::Value v5(type::ValueFactory::CastAsVarchar(
      type::Value(type::TypeId::TINYINT, (int8_t)type::PELOTON_INT8_MAX)));
  EXPECT_EQ(v5.ToString(), "127");
  type::Value v6(type::ValueFactory::CastAsVarchar(
      type::Value(type::TypeId::BIGINT, (int64_t)type::PELOTON_INT64_MAX)));
  EXPECT_EQ(v6.ToString(), "9223372036854775807");

  std::string str1("9999-12-31 23:59:59.999999+14");
  type::Value v7(type::ValueFactory::CastAsTimestamp(
      type::Value(type::TypeId::VARCHAR, str1)));
  EXPECT_EQ(v7.ToString(), str1);
  std::string str2("9999-12-31 23:59:59-01");
  type::Value v77(type::ValueFactory::CastAsTimestamp(
      type::Value(type::TypeId::VARCHAR, str2)));
  EXPECT_EQ(v77.ToString(), "9999-12-31 23:59:59.000000-01");
  EXPECT_THROW(type::ValueFactory::CastAsTimestamp(type::Value(
                   type::TypeId::VARCHAR, "1900-02-29 23:59:59.999999+12")),
               peloton::Exception);

  type::Value v8(type::ValueFactory::CastAsBigInt(
      type::Value(type::TypeId::VARCHAR, "9223372036854775807")));
  EXPECT_EQ(v8.GetAs<int64_t>(), 9223372036854775807);
  EXPECT_THROW(type::ValueFactory::CastAsBigInt(
                   type::Value(type::TypeId::VARCHAR, "9223372036854775808")),
               peloton::Exception);
  EXPECT_THROW(type::ValueFactory::CastAsBigInt(
                   type::Value(type::TypeId::VARCHAR, "-9223372036854775808")),
               peloton::Exception);

  type::Value v9(type::ValueFactory::CastAsInteger(
      type::Value(type::TypeId::VARCHAR, "2147483647")));
  EXPECT_EQ(v9.GetAs<int32_t>(), 2147483647);
  EXPECT_THROW(type::ValueFactory::CastAsInteger(
                   type::Value(type::TypeId::VARCHAR, "-2147483648")),
               peloton::Exception);
  EXPECT_THROW(type::ValueFactory::CastAsInteger(
                   type::Value(type::TypeId::VARCHAR, "2147483648")),
               peloton::Exception);

  type::Value v10(type::ValueFactory::CastAsSmallInt(
      type::Value(type::TypeId::VARCHAR, "32767")));
  EXPECT_EQ(v10.GetAs<int16_t>(), 32767);
  EXPECT_THROW(type::ValueFactory::CastAsSmallInt(
                   type::Value(type::TypeId::VARCHAR, "-32768")),
               peloton::Exception);
  EXPECT_THROW(type::ValueFactory::CastAsSmallInt(
                   type::Value(type::TypeId::VARCHAR, "32768")),
               peloton::Exception);

  type::Value v11(type::ValueFactory::CastAsTinyInt(
      type::Value(type::TypeId::VARCHAR, "127")));
  EXPECT_EQ(v11.GetAs<int8_t>(), 127);
  EXPECT_THROW(type::ValueFactory::CastAsTinyInt(
                   type::Value(type::TypeId::VARCHAR, "-128")),
               peloton::Exception);
  EXPECT_THROW(type::ValueFactory::CastAsTinyInt(
                   type::Value(type::TypeId::VARCHAR, "128")),
               peloton::Exception);
...
TEST_F(ValueFactoryTests, SerializationTest) {
  ...
  for (auto col_type : valuefactory_test_types) {
    for (auto expect_min : {true, false}) {
      type::Value v = (type::Value::DeserializeFrom(
          in, type::Type::GetInstance(col_type)->GetTypeId(), nullptr));
      EXPECT_EQ(v.GetTypeId(), col_type);
      type::Value expected = (expect_min ? type::Type::GetMinValue(col_type)
