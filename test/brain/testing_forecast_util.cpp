...
matrix_eig TestingForecastUtil::GetWorkload(WorkloadType w, size_t num_samples, size_t num_feats) {
  ...
  switch(w) {
    case WorkloadType::SimpleSinusoidal:
      ...
      LOG_INFO("Generating a Sinusoidal workload of dims: %ld x %ld", num_samples, num_feats);
      break;
    case WorkloadType::NoisySinusoidal:
      data = GetWorkload(WorkloadType::SimpleSinusoidal, num_samples, num_feats) +
    ...
    case WorkloadType::SimpleLinear:
      // y=m*x
      data = vector_eig::LinSpaced(num_samples, 0, num_samples - 1).replicate(1, num_feats);
      for (size_t i = 0; i < num_feats; i++) {
        data.col(i) *= (3*i);
      }
      LOG_INFO("Generating a Linear workload of dims: %ld x %ld", num_samples, num_feats);
      break;
