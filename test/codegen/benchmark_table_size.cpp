...
class BenchmarkTableSizeTest : public PelotonTest {
 public:
  ~BenchmarkTableSizeTest() { DropDatabase(); }
  ...
  Stats RunCompiledExperiment(TestConfig &config, uint32_t num_runs = 5) {
    ...
    for (uint32_t i = 0; i < num_runs; i++) {
      ...
      std::unique_ptr<planner::SeqScanPlan> simple_scan{
          new planner::SeqScanPlan(&GetTable(), nullptr, {0, 1, 2})};

      // Do binding
      planner::BindingContext context;
      simple_scan->PerformBinding(context);
      ...
      stats.Merge(compile_stats, runtime_stats, buffer.GetOutputTuples().size());
  ...
  Stats RunInterpretedExperiment(TestConfig &config, uint32_t num_runs = 5) {
    ...
    for (uint32_t i = 0; i < num_runs; i++) {
      ...
      executor::SeqScanExecutor executor{simple_scan.get(), &ctx};
      ...
      while (executor.Execute()) {
        ...
        for (oid_t tuple_id : *tile) {
          const expression::ContainerTuple<executor::LogicalTile> tuple{
              tile, tuple_id};
          std::vector<Value> tv;
          for (oid_t col_id : simple_scan->GetColumnIds()) {
...
TEST_F(BenchmarkTableSizeTest, TableSizeTestWithCompilation) {
  uint32_t scale_factor_multiplier = 10;
  uint32_t scale_factor_iters = 10;

  PrintName("TABLE_SIZE: COMPILATION");
  for (uint32_t i = 0; i < scale_factor_iters; ++i) {
    TestConfig config;
    config.layout = LAYOUT_ROW;
    config.scale_factor = (i + 1) * scale_factor_multiplier;
...
TEST_F(BenchmarkTableSizeTest, TableSizeTestWithInterpretation) {
  uint32_t scale_factor_multiplier = 10;
  uint32_t scale_factor_iters = 10;

  PrintName("TABLE_SIZE: INTERPRETATION");
  for (uint32_t i = 0; i < scale_factor_iters; ++i) {
    TestConfig config;
    config.layout = LAYOUT_ROW;
    config.scale_factor = (i + 1) * scale_factor_multiplier;
