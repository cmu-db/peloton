...
TEST_F(AutoTests, BasicTest) {
  UNUSED_ATTRIBUTE auto a = 1 + 2;
  LOG_INFO("type of a: %s", typeid(a).name());
  UNUSED_ATTRIBUTE auto b = add(1, 1.2);
  LOG_INFO("type of b: %s", typeid(b).name());
  UNUSED_ATTRIBUTE auto d = {1, 2};
  LOG_INFO("type of d: %s", typeid(d).name());

  UNUSED_ATTRIBUTE auto my_lambda = [](int x) { return x + 3; };
  LOG_INFO("my_lambda: %d", my_lambda(5));

  UNUSED_ATTRIBUTE auto my_fun = get_fun(2);
