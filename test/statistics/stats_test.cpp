...
TEST_F(StatsTest, PerThreadStatsTest) {
  ...
  for (size_t i = 0; i < tuple_slot_ids.size(); i += 2) {
    txn_manager.PerformRead(txn, tuple_slot_ids[i]);
  }
  txn_manager.CommitTransaction(txn);
  ...
  txn = txn_manager.BeginTransaction();
  ItemPointer new_location = data_table->InsertEmptyVersion();
  EXPECT_TRUE(new_location.block != INVALID_OID);
  EXPECT_TRUE(new_location.offset != INVALID_OID);
  txn_manager.PerformDelete(txn, tuple_slot_ids[5], new_location);
  txn_manager.PerformRead(txn, tuple_slot_ids[1]);
  txn_manager.CommitTransaction(txn);
