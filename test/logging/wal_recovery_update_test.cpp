...
void *RecoveryUpdateTest(int port) {
  try {
    logging::WalRecovery rec(0,"");
    FileHandle f;
    logging::LoggingUtil::OpenFile("logging_samples/logfile_update", "rb", f);
    rec.RecoveryTest(f);
...
TEST_F(RecoveryTests, RecoveryUpdateQueryTest) {
