...
struct TransactionSchedule {
  ...
  void AddAbort(int time) {
    operations.emplace_back(TXN_OP_ABORT, 0, 0);
    times.push_back(time);
  }
...
struct TransactionSchedules {
  std::vector<TransactionSchedule *> schedules;
  std::atomic<int> next_sched = {0};
...
class TransactionTestsUtil {
 public:
  ...
  static int ExecuteRead(concurrency::Transaction *transaction,
                         storage::DataTable *table, int id);
  static bool ExecuteDelete(concurrency::Transaction *transaction,
                            storage::DataTable *table, int id);
