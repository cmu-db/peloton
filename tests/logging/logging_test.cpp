...
TEST_F(LoggingTests, AllCommittedTest) {
  ...
  LoggingScheduler scheduler(2, &log_manager, table.get());
  ...
  scheduler.Logger(2).Prepare();
  scheduler.Logger(2).Begin(3);
  scheduler.Logger(2).Insert(3);
  scheduler.Logger(2).Commit(3);
  scheduler.Logger(0).Collect();
  scheduler.Logger(0).Flush();
  scheduler.Logger(1).Done(1);
  scheduler.Logger(2).Done(1);
  scheduler.Run();
...
TEST_F(LoggingTests, LaggardTest) {
  ...
  scheduler.Logger(1).Prepare();
  scheduler.Logger(1).Begin(4);
  scheduler.Logger(1).Insert(4);
  scheduler.Logger(1).Commit(4);
  scheduler.Logger(2).Prepare();
  scheduler.Logger(0).Collect();
  scheduler.Logger(0).Flush();
  scheduler.Logger(1).Done(1);
...
TEST_F(LoggingTests, FastLoggerTest) {
  ...
  scheduler.Logger(1).Commit(4);
  scheduler.Logger(2).Prepare();
  scheduler.Logger(2).Insert(5);
  scheduler.Logger(2).Commit(5);
  scheduler.Logger(2).Prepare();
  scheduler.Logger(0).Collect();
  scheduler.Logger(0).Flush();
  scheduler.Logger(1).Done(1);
...
TEST_F(LoggingTests, BothPreparingTest) {
  ...
  scheduler.Logger(0).Collect();
  scheduler.Logger(0).Flush();
  scheduler.Logger(2).Begin(6);
  scheduler.Logger(2).Insert(6);
  scheduler.Logger(2).Commit(6);
