...
TEST_F(StatsTest, PerThreadStatsTest) {
  ...
  int64_t txn_commited =
      stats_->GetDatabaseMetric(database_id)->GetTxnCommitted().GetCounter();
  ...
  txn_commited =
      stats_->GetDatabaseMetric(database_id)->GetTxnCommitted().GetCounter();
  int64_t txn_aborted =
      stats_->GetDatabaseMetric(database_id)->GetTxnAborted().GetCounter();
  ...
  reads = stats_->GetTableMetric(database_id, table_id)
              ->GetTableAccess()
              .GetReads();
  int64_t deletes = stats_->GetTableMetric(database_id, table_id)
                        ->GetTableAccess()
                        .GetDeletes();
