...
class TransactionThread {
 public:
  TransactionThread(TransactionSchedule *sched, storage::DataTable *table_,
                    ...
                    std::vector<stats::BackendStatsContext*> *stats_)
      : schedule(sched),
  ...
  void RunLoop() {
    stats::BackendStatsContext *thread_stats = new stats::BackendStatsContext(100);
    stats->push_back(thread_stats);
    while (true) {
      while (!go) {
  ...
  void RunNoWait() {
    stats->push_back(new stats::BackendStatsContext(100));
    while (true) {
      ExecuteNext();
      if (cur_seq == (int)schedule->operations.size()) {
        break;
  ...
  void ExecuteNext() {
    ...
    switch (op) {
      case TXN_OP_INSERT: {
        ...
        execute_result =
            StatsTestsUtil::ExecuteInsert(txn, table, id, value);
        break;
      }
      case TXN_OP_READ: {
        ...
        int result;
        execute_result =
            StatsTestsUtil::ExecuteRead(txn, table, id, result);
        schedule->results.push_back(result);
        break;
      }
      case TXN_OP_DELETE: {
        LOG_INFO("Execute Delete");
        execute_result = StatsTestsUtil::ExecuteDelete(txn, table, id);
        break;
      }
      case TXN_OP_UPDATE: {
        LOG_INFO("Execute Update");
        execute_result =
            StatsTestsUtil::ExecuteUpdate(txn, table, id, value);
        break;
      }
      case TXN_OP_SCAN: {
        LOG_INFO("Execute Scan");
        execute_result = StatsTestsUtil::ExecuteScan(
      ...
      case TXN_OP_UPDATE_BY_VALUE: {
        int old_value = id;
        int new_value = value;
        execute_result = StatsTestsUtil::ExecuteUpdateByValue(
  ...
  std::vector<stats::BackendStatsContext*> *stats;
  int cur_seq;
  bool go;
 ...
 public:
  ...
  void Run() {
    ...
    for (int i = 0; i < (int)schedules.size(); i++) {
      tthreads.emplace_back(&schedules[i], table, txn_manager, &tstats);
    }
    if (!concurrent) {
      for (int i = 0; i < (int)schedules.size(); i++) {
  ...
  std::vector<stats::BackendStatsContext*>& GetThreadStats() {
    return tstats;
  }
  ...
  std::vector<stats::BackendStatsContext*> tstats;
