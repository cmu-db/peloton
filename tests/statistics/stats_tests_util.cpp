...
storage::DataTable *StatsTestsUtil::CreateCombinedPrimaryKeyTable() {
  ...
  auto index_metadata = new index::IndexMetadata(
      "primary_btree_index", 1234, INVALID_OID, INVALID_OID, INDEX_TYPE_BTREE,
...
storage::DataTable *StatsTestsUtil::CreatePrimaryKeyUniqueKeyTable() {
  auto id_column = catalog::Column(VALUE_TYPE_INTEGER,
                                   GetTypeSize(VALUE_TYPE_INTEGER), "id", true);
  id_column.AddConstraint(catalog::Constraint(CONSTRAINT_TYPE_NOTNULL,
                                              "not_null"));
  ...
  auto index_metadata2 = new index::IndexMetadata(
      "unique_btree_index", 1235, INVALID_OID, INVALID_OID, INDEX_TYPE_BTREE,
...
storage::DataTable *StatsTestsUtil::CreateTable(int num_key,
  ...
  auto index_metadata = new index::IndexMetadata(
      "primary_btree_index", index_oid, relation_id, database_id, INDEX_TYPE_BTREE,
...
StatsTestsUtil::MakeProjectInfoFromTuple(const storage::Tuple *tuple) {
...
bool StatsTestsUtil::ExecuteInsert(concurrency::Transaction *transaction,
...
StatsTestsUtil::MakePredicate(int id) {
...
bool StatsTestsUtil::ExecuteRead(concurrency::Transaction *transaction,
                                       storage::DataTable *table, int id,
                                       int &result) {
  std::unique_ptr<executor::ExecutorContext> context(
      new executor::ExecutorContext(transaction));
...
bool StatsTestsUtil::ExecuteDelete(concurrency::Transaction *transaction,
                                         storage::DataTable *table, int id) {
  std::unique_ptr<executor::ExecutorContext> context(
      new executor::ExecutorContext(transaction));
...
bool StatsTestsUtil::ExecuteUpdate(concurrency::Transaction *transaction,
                                         storage::DataTable *table, int id,
                                         int value) {
  std::unique_ptr<executor::ExecutorContext> context(
      new executor::ExecutorContext(transaction));
...
bool StatsTestsUtil::ExecuteUpdateByValue(concurrency::Transaction *txn,
...
bool StatsTestsUtil::ExecuteScan(concurrency::Transaction *transaction,
