...
DatabaseMetricOld *BackendStatsContext::GetDatabaseMetric(oid_t database_id) {
  if (database_metrics_.find(database_id) == database_metrics_.end()) {
    database_metrics_[database_id] = std::unique_ptr<DatabaseMetricOld>(
        new DatabaseMetricOld{MetricType::DATABASE, database_id});
  }
...
void BackendStatsContext::IncreaseTableMemoryAlloc(oid_t database_id,
                                                   oid_t table_id,
                                                   int64_t bytes) {
  auto table_metric = GetTableMetric(database_id, table_id);
  PELOTON_ASSERT(table_metric != nullptr);
  table_metric->GetTableMemory().IncreaseAllocation(bytes);
}

void BackendStatsContext::IncreaseTableMemoryUsage(oid_t database_id,
                                                   oid_t table_id,
                                                   int64_t bytes) {
  auto table_metric = GetTableMetric(database_id, table_id);
  PELOTON_ASSERT(table_metric != nullptr);
  table_metric->GetTableMemory().IncreaseUsage(bytes);
}

void BackendStatsContext::DecreaseTableMemoryAlloc(oid_t database_id,
                                                   oid_t table_id,
                                                   int64_t bytes) {
  auto table_metric = GetTableMetric(database_id, table_id);
  PELOTON_ASSERT(table_metric != nullptr);
  table_metric->GetTableMemory().DecreaseAllocation(bytes);
}

void BackendStatsContext::DecreaseTableMemoryUsage(oid_t database_id,
                                                   oid_t table_id,
                                                   int64_t bytes) {
  auto table_metric = GetTableMetric(database_id, table_id);
  PELOTON_ASSERT(table_metric != nullptr);
