...
bool RunNewOrder(const size_t &thread_id) {
  ...
  for (size_t i = 0; i < i_ids.size(); ++i) {
    ...
    s_quantity_attr.expr = expression::ExpressionUtil::ConstantValueFactory(
    ...
    s_ytd_attr.expr = expression::ExpressionUtil::ConstantValueFactory(
    ...
    s_order_cnt_attr.expr = expression::ExpressionUtil::ConstantValueFactory(
        type::ValueFactory::GetIntegerValue(s_order_cnt));
    s_order_cnt_attr.attribute_info.type =
        s_order_cnt_attr.expr->GetValueType();

    planner::DerivedAttribute s_remote_cnt_attr;
    s_remote_cnt_attr.expr = expression::ExpressionUtil::ConstantValueFactory(
        type::ValueFactory::GetIntegerValue(s_remote_cnt));
    s_remote_cnt_attr.attribute_info.type =
        s_remote_cnt_attr.expr->GetValueType();
