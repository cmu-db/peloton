...
bool RunPayment(const size_t &thread_id){
  ...
  if (customer_credit == customers_bad_credit) {
    ...
    c_new_bal.expr =
        expression::ExpressionUtil::ConstantValueFactory(customer_new_balance_value);
    c_new_ytd.expr =
        expression::ExpressionUtil::ConstantValueFactory(customer_new_ytd_value);
    c_new_paycnt.expr =
        expression::ExpressionUtil::ConstantValueFactory(customer_new_paycnt_value);
    c_new_data.expr =
        expression::ExpressionUtil::ConstantValueFactory(customer_new_data_value);
