...
bool RunDelivery(const size_t &thread_id) {
  ...
  for (int d_id = 0; d_id < state.districts_per_warehouse; ++d_id) {
    ...
    // Get the index
    auto orders_pkey_index =
        orders_table->GetIndexWithOid(orders_table_pkey_index_oid);
    ...
    auto order_line_pkey_index =
        order_line_table->GetIndexWithOid(order_line_table_pkey_index_oid);
    ...
    planner::IndexScanPlan::IndexScanDesc new_order_delete_idex_scan_desc(
        new_order_pkey_index, new_order_key_column_ids,
    ...
    auto plus_operator_expr = expression::ExpressionUtil::OperatorFactory(
        ExpressionType::OPERATOR_PLUS, type::Type::INTEGER, tuple_val_expr,
        constant_val_expr);
