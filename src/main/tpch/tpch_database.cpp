...
void ForEachLine(const std::string fname, std::function<void(char *)> cb) {
  ...
  while ((bytes_read = read(input_fd, buf_pos, bytes_to_read)) != 0) {
    ...
    if (tail_size > 0) {
      std::memcpy(buffer, line_start, tail_size);
...
uint32_t ConvertDate(char *p) {
  std::tm t_shipdate;
  memset(&t_shipdate, 0, sizeof(std::tm));
...
void TPCHDatabase::LoadLineitemTable() {
  ...
  if (TableIsLoaded(TPCHDatabase::TableId::Lineitem)) {
    return;
  }
  ...
  auto &table = GetTable(TPCHDatabase::TableId::Lineitem);
