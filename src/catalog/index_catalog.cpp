...
IndexCatalogObject::IndexCatalogObject(executor::LogicalTile *tile, int tupleId) {
  auto field_location = tile->GetTuplePointer(tupleId, IndexCatalog::ColumnId::INDEX_OID);
  PL_ASSERT(field_location != nullptr);
  index_oid = *(reinterpret_cast<const oid_t *>(field_location));

  field_location = tile->GetTuplePointer(tupleId, IndexCatalog::ColumnId::INDEX_NAME);
  PL_ASSERT(field_location != nullptr);
  ...
  field_location = tile->GetTuplePointer(tupleId, IndexCatalog::ColumnId::TABLE_OID);
  PL_ASSERT(field_location != nullptr);
  table_oid = *(reinterpret_cast<const oid_t *>(field_location));

  field_location = tile->GetTuplePointer(tupleId, IndexCatalog::ColumnId::INDEX_TYPE);
  PL_ASSERT(field_location != nullptr);
  index_type = *(reinterpret_cast<const IndexType *>(field_location));

  field_location = tile->GetTuplePointer(tupleId, IndexCatalog::ColumnId::INDEX_CONSTRAINT);
  PL_ASSERT(field_location != nullptr);
  index_constraint = *(reinterpret_cast<const IndexConstraintType *>(field_location));

  field_location = tile->GetTuplePointer(tupleId, IndexCatalog::ColumnId::UNIQUE_KEYS);
  PL_ASSERT(field_location != nullptr);
  unique_keys = *(reinterpret_cast<const bool *>(field_location));

  field_location = tile->GetTuplePointer(tupleId, IndexCatalog::ColumnId::INDEXED_ATTRIBUTES);
  PL_ASSERT(field_location != nullptr);
