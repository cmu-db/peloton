...
ForeignKey *ForeignKey::DeserializeFrom(SerializeInput &in){
	...
	for (oid_t src_col_idx = 0; src_col_idx < source_column_count; src_col_idx++) {
	...
	return new ForeignKey(source_table_id, sink_table_id, sink_col_ids, source_col_ids,
			update_action, delete_action, fk_name);
}
