...
bool ProcCatalog::InsertProc(const std::string &proname,
  ...
  auto val3 =
      type::ValueFactory::GetVarcharValue(TypeArrayToString(proargtypes));
...
type::TypeId ProcCatalog::GetProRetType(
    const std::string &proname, const std::vector<type::TypeId> &proargtypes,
    concurrency::Transaction *txn) {
