...
//
// Identification: src/optimizer/postgres_cost_calculator.cpp
//
...
void PostgresCostCalculator::Visit(const PhysicalIndexScan *op) {
  auto table_stats = std::dynamic_pointer_cast<TableStats>(
      StatsStorage::GetInstance()->GetTableStats(
          op->table_->GetDatabaseOid(), op->table_->GetTableOid(), txn_));
...
void PostgresCostCalculator::Visit(UNUSED_ATTRIBUTE const PhysicalOrderBy *) { SortCost(); }
 ...
 * manner to postgres.
 */
void PostgresCostCalculator::Visit(const PhysicalInnerHashJoin *op) {
