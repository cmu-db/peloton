...
void StatsStorage::CreateStatsCatalog() {
  ...
  std::unique_ptr<storage::DataTable> table(
      storage::TableFactory::GetDataTable(catalog_db_oid, catalog->GetNextOid(),
        stats_schema.get(), STATS_TABLE_NAME, DEFAULT_TUPLES_PER_TILEGROUP,
...
std::unique_ptr<catalog::Schema> StatsStorage::InitializeStatsSchema() {
  ...
  oid_t array_type_size = type::Type::GetTypeSize(type::Type::ARRAY);
  type::Type::TypeId array_type = type::Type::ARRAY;
  ...
  auto most_common_vals_column = catalog::Column(array_type, array_type_size,
      "most_common_vals_column", true);
  most_common_vals_column.AddConstraint(not_null_constraint);

  auto most_common_freqs_column = catalog::Column(array_type, array_type_size,
      "most_common_freqs_column", true);
  most_common_freqs_column.AddConstraint(not_null_constraint);

  auto histogram_bounds_column = catalog::Column(array_type, array_type_size,
      "histogram_bounds", true);
...
/**
 * Generate a table stats tuple
 */
std::unique_ptr<storage::Tuple> StatsStorage::GetTableStatsCatalogTuple(
...
void StatsStorage::StoreTableStats(TableStats *table_stats) {
  ...
  for(oid_t column_id = 0; column_id < column_count; column_id ++) {
    ...
    auto table_tuple = GetTableStatsCatalogTuple(stats_table->GetSchema(),
...
// TableStats *StatsStorage::GetTableStats(const std::string table_name) {
