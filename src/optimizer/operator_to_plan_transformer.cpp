...
OperatorToPlanTransformer::ConvertOpExpression(
  ...
  (void)plan;
  (void)requirements;
  (void)required_input_props;
  (void)children_plans;
  (void)children_expr_map;
  (void)output_expr_map;
...
void OperatorToPlanTransformer::Visit(const PhysicalProject *) {
  ...
  //   // children_expr_map_, but I think we need to clean up the code here a
  //   bit
...
void OperatorToPlanTransformer::Visit(const PhysicalLimit *) {
  ...
  //     new planner::LimitPlan(limit_prop->GetLimit(),
  //     limit_prop->GetOffset()));
...
void OperatorToPlanTransformer::Visit(const PhysicalAggregate *) {
  ...
  //
  // output_plan_ =
  //     GenerateAggregatePlan(col_prop, AggregateType::PLAIN, nullptr,
  //     nullptr);
}
...
void OperatorToPlanTransformer::Visit(const PhysicalDistinct *) {
  ...
  //
  // auto prop_distinct =
  // requirements_->GetPropertyOfType(PropertyType::DISTINCT)
  //                          ->As<PropertyDistinct>();
  //
...
void OperatorToPlanTransformer::Visit(const PhysicalInnerNLJoin *op) {
  ...
  // output_plan_ = GenerateJoinPlan(join_predicate, JoinType::INNER, false);
}
...
void OperatorToPlanTransformer::Visit(const PhysicalInnerHashJoin *op) {
  ...
  // auto join_predicate = util::CombinePredicates(op->join_predicates);
  //
  // output_plan_ = GenerateJoinPlan(join_predicate, JoinType::INNER, true);
}
...
void OperatorToPlanTransformer::GenerateTableExprMap(
  ...
  //       new
  //       expression::TupleValueExpression(cols[col_id].column_name.c_str(),
  //                                            alias.c_str()));
...
OperatorToPlanTransformer::GenerateAggregatePlan(
  ...
  // unique_ptr<const planner::ProjectInfo> proj_info(
  //     new planner::ProjectInfo(move(tl), move(dml)));
  // unique_ptr<const expression::AbstractExpression>
...
unique_ptr<planner::AbstractPlan> OperatorToPlanTransformer::GenerateJoinPlan(
  ...
  // for (size_t output_offset = 0; output_offset < output_size;
  // output_offset++) {
  ...
  //
  //   unique_ptr<planner::HashPlan> hash_plan(new
  //   planner::HashPlan(hash_keys));
