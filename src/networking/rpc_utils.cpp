...
void SetTupleDescMsg(TupleDesc tuple_desc, TupleDescMsg& tuple_desc_msg) {
    ...
    for (int it = 0; it < tuple_desc->natts; it++) {
        ...
        attrs->set_attalign(
                PL_ASSERT_range_cast_same<int32, int8>(
        ...
        attrs->set_attlen(
                PL_ASSERT_range_cast_same<int32, int16>(
    ...
    if ( tuple_desc->constr != NULL ) {
        ...
        tuple_desc_msg.mutable_constr()->set_num_check(
                PL_ASSERT_range_cast_same<uint32, uint16>(
...
std::unique_ptr<tupleDesc> ParseTupleDescMsg(const TupleDescMsg& tuple_desc_msg) {
    ...
    if (attrs_count > 0) {
        ...
        PL_ASSERT(tuple_desc_msg.attrs_size() == attrs_count);
        ...
        for (int it = 0; it < attrs_count; it++) {
            ...
            attrs[it]->attalign = PL_ASSERT_range_cast_same<int8, int32>(
            ...
            attrs[it]->attlen = PL_ASSERT_range_cast_same<int16, int32>(
            ...
            // It is int16 in postgres, so we convert it
            attrs[it]->attnum = PL_ASSERT_range_cast_same<int16, int32>(
            ...
            attrs[it]->attstorage = PL_ASSERT_range_cast_same<int8, int32>(
    ...
    if (tuple_desc_msg.has_constr()) {
        ...
        // uint16
        tuple_constr->num_check = PL_ASSERT_range_cast_same<uint16, uint32>(
                tuple_desc_msg.constr().num_check());
        // uint16
        tuple_constr->num_defval = PL_ASSERT_range_cast_same<uint16, uint32>(
