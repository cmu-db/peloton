...
const std::unordered_map<std::string, std::string>
    // clang-format off
    NetworkManager::parameter_status_map_ =
...
std::vector<NetworkManager *> NetworkManager::network_managers_;
std::mutex NetworkManager::packet_managers_mutex_;

NetworkManager::NetworkManager()
    : txn_state_(NetworkTransactionStateType::IDLE), pkt_cntr_(0) {
  traffic_cop_.reset(new tcop::TrafficCop());
  {
    std::lock_guard<std::mutex> lock(NetworkManager::packet_managers_mutex_);
    NetworkManager::network_managers_.push_back(this);
    LOG_DEBUG("Registered new NetworkManager [count=%d]",
              (int)NetworkManager::network_managers_.size());
  }
}
...
NetworkManager::~NetworkManager() {
  {
    std::lock_guard<std::mutex> lock(NetworkManager::packet_managers_mutex_);
    NetworkManager::network_managers_.erase(
        std::remove(NetworkManager::network_managers_.begin(),
                    NetworkManager::network_managers_.end(), this),
        NetworkManager::network_managers_.end());
    LOG_DEBUG("Removed NetworkManager [count=%d]",
              (int)NetworkManager::network_managers_.size());
  }
}

void NetworkManager::InvalidatePreparedStatements(oid_t table_id) {
...
void NetworkManager::ReplanPreparedStatement(Statement *statement) {
...
void NetworkManager::MakeHardcodedParameterStatus(
 ...
 */
int NetworkManager::ProcessInitialPacket(InputPacket *pkt) {
...
bool NetworkManager::ProcessStartupPacket(InputPacket* pkt, int32_t proto_version) {
  std::string token, value;
  std::unique_ptr<OutputPacket> response(new OutputPacket());
...
bool NetworkManager::ProcessSSLRequestPacket(InputPacket *pkt) {
  UNUSED(pkt);
  std::unique_ptr<OutputPacket> response(new OutputPacket());
...
void NetworkManager::PutTupleDescriptor(
...
void NetworkManager::SendDataRows(std::vector<StatementResult> &results,
...
void NetworkManager::CompleteCommand(const std::string &query, const QueryType& query_type, int rows) {
  std::unique_ptr<OutputPacket> pkt(new OutputPacket());
 ...
 */
void NetworkManager::SendEmptyQueryResponse() {
  std::unique_ptr<OutputPacket> response(new OutputPacket());
  response->msg_type = NetworkMessageType::EMPTY_QUERY_RESPONSE;
  responses.push_back(std::move(response));
}

bool NetworkManager::HardcodedExecuteFilter(QueryType query_type) {
  switch (query_type) {
    // Skip SET
...
void NetworkManager::ExecQueryMessage(InputPacket *pkt, const size_t thread_id) {
  std::string query;
 ...
 */
void NetworkManager::ExecParseMessage(InputPacket *pkt) {
...
void NetworkManager::ExecBindMessage(InputPacket *pkt) {
...
size_t NetworkManager::ReadParamType(InputPacket *pkt, int num_params,
...
size_t NetworkManager::ReadParamFormat(InputPacket *pkt, int num_params_format,
...
size_t NetworkManager::ReadParamValue(
...
bool NetworkManager::ExecDescribeMessage(InputPacket *pkt) {
  if (skipped_stmt_) {
...
void NetworkManager::ExecExecuteMessage(InputPacket *pkt,
...
void NetworkManager::ExecCloseMessage(InputPacket *pkt) {
  uchar close_type = 0;
  std::string name;
 ...
 */
bool NetworkManager::ProcessPacket(InputPacket *pkt, const size_t thread_id) {
 ...
 */
void NetworkManager::SendErrorResponse(
...
void NetworkManager::SendReadyForQuery(NetworkTransactionStateType txn_status) {
  std::unique_ptr<OutputPacket> pkt(new OutputPacket());
