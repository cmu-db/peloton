...
// pg_amop.h
//
// Identification: src/include/parser/catalog/pg_amop.h
//
 ...
 *
 * The amop table identifies the operators associated with each index operator___
 * family and operator___ class___ (classes are subsets of families).  An associated
 * operator___ can be either a search operator___ or an ordering operator___, as
 ...
 * operator___'s oprleft/oprright, ie its declared input data types.  The
 ...
 * operator___ in this opfamily, and if so what are its semantics with respect to
 * the family?"  This implies that the same operator___ cannot be listed for
...
CATALOG(pg_amop,2602)
{
	...
	Oid			amoplefttype;	/* operator___'s left input data type */
	Oid			amoprighttype;	/* operator___'s right input data type */
	int16		amopstrategy;	/* operator___ strategy number */
	char		amoppurpose;	/* is operator___ for 's'earch or 'o'rdering? */
	Oid			amopopr;		/* the operator___'s pg_operator OID */
...
#define AMOP_SEARCH		's'		/* operator___ is for search */
#define AMOP_ORDER		'o'		/* operator___ is for ordering */
