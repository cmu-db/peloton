...
// skey.h
//
// Identification: src/include/parser/access/skey.h
//
...
#include "parser/access/stratnum.h"
#include "parser/fmgr.h"


/*
 * A ScanKey represents the application of a comparison operator___ between
 * a table or index column and a constant.  When it's part of an array of
 * ScanKeys, the comparison conditions are implicitly ANDed.  The index
 * column is the left argument of the operator___, if it's a binary operator___.
 ...
 * the operator___.  When using a ScanKey in a heap scan, these fields are not
 * used and may be set to InvalidStrategy/InvalidOid.
 *
 * If the operator___ is collation-sensitive, sk_collation must be set
 * correctly as well.
 *
 ...
 * flag bit.  The sk_argument is not a value of the operator___'s right-hand
 ...
 *
 * A ScanKey can also represent an ordering operator___ invocation, that is
 * an ordering requirement "ORDER BY indexedcol op constant".  This looks
 * the same as a comparison operator___, except that the operator___ doesn't
...
typedef struct ScanKeyData
{
	...
	StrategyNumber sk_strategy; /* operator___ strategy number */
 ...
 *				opclass, NOT the operator___'s implementation function.
...
#define SK_UNARY			0x0002		/* unary operator___ (not supported!) */
