...
// nbtree.h
//
// Identification: src/include/parser/access/nbtree.h
//
...
#include "parser/access/genam.h"
#include "parser/access/itup.h"
#include "parser/access/sdir.h"
#include "parser/access/xlogreader.h"
#include "parser/catalog/pg_index.h"
#include "parser/lib/stringinfo.h"
#include "parser/storage/bufmgr.h"
 ...
 *	do this is by generating a new___ OID for every insertion that we do in the
...
#define XLOG_BTREE_SPLIT_R		0x40	/* as above, new___ item on right */
#define XLOG_BTREE_SPLIT_L_ROOT 0x50	/* add tuple with split of root */
#define XLOG_BTREE_SPLIT_R_ROOT 0x60	/* as above, new___ item on right */
#define XLOG_BTREE_DELETE		0x70	/* delete leaf index tuples for a page */
#define XLOG_BTREE_UNLINK_PAGE	0x80	/* delete a half-dead page */
#define XLOG_BTREE_UNLINK_PAGE_META 0x90		/* same, and update metapage */
#define XLOG_BTREE_NEWROOT		0xA0	/* new___ root page */
 ...
 * XLogInsert would almost always think the right page is new___ and store its
 * whole page image.  The left page, however, is handled in the normal
 * incremental-update fashion.
 *
 * Note: the four XLOG_BTREE_SPLIT xl_info codes all use this data record.
 * The _L and _R variants indicate whether the inserted tuple went into the
 * left or right split page (and thus, whether newitemoff and the new___ item
 * are stored or not).  The _ROOT variants indicate that we are splitting
 * the root page, and thus that a newroot record rather than an insert or
 * split record should follow.  Note that a split record never carries a
 * metapage update --- we'll do that in the parent-level update.
 *
 * Backup Blk 0: original page / new___ left page
 *
 * The left page's data portion contains the new___ item, if it's the _L variant.
 * (In the _R variants, the new___ item is one of the right page's tuples.)
 * If level > 0, an IndexTuple representing the HIKEY of the left page
 * follows.  We don't need this on leaf pages, because it's the same as the
 * leftmost key in the new___ right page.
 *
...
typedef struct xl_btree_split
{
	...
	OffsetNumber newitemoff;	/* new___ item's offset (if placed on left page) */
} xl_btree_split;
 ...
 * Backup Blk 4: metapage (if rightsib becomes new___ fast root)
 */
 ...
 *
 * Backup Blk 0: new___ root page (2 tuples as payload, if splitting old root)
 * Backup Blk 1: left child (if splitting an old root)
 * Backup Blk 2: metapage
 */
typedef struct xl_btree_newroot
{
	BlockNumber rootblk;		/* location of new___ root (redundant with blk 0) */
...
/*
 *	When a new___ operator___ class___ is declared, we require that the user
 ...
 *
 *	To facilitate accelerated sorting, an operator___ class___ may choose to
 ...
 *	pairs from internal pages onto a private___ stack.  If we split a
 *	leaf, we use this stack to walk back up the tree and insert data
 *	into parent pages (and possibly to split them, too).  Lehman and
 *	Yao's update algorithm guarantees that under no circumstances can
 *	our private___ stack give us an irredeemably bad picture up the tree.
...
/*
 * BTScanOpaqueData is the btree-private___ state needed for an indexscan.
...
/*
 * We use some private___ sk_flags bits in preprocessed scan keys.  We're allowed
