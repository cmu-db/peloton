...
namespace peloton {
  namespace gc {
    ...
    class TxnGCManager : public GCManager {
    public:
      TxnGCManager(int thread_count)
        : is_running_(true),
      ...
      static TxnGCManager& GetInstance(int thread_count = 1) {
        static TxnGCManager gcManager(thread_count);
        return gcManager;
      }
