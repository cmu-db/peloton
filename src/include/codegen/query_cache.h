...
/* This is the implementation of query cache that maps an AbstractPlan with a
 * codegen
 * Query using LRU eviction policy. The cache is implemented as a singleton with
 * a list
 * and a map.
*/
class QueryCache {
 public:
  ...
  size_t GetSize() { return cache_map.size(); }

  void ClearCache() { cache_map.clear(); }
 ...
 private:
  ...
  std::list<
      std::pair<std::shared_ptr<planner::AbstractPlan>, std::unique_ptr<Query>>>
      query_list;

  std::map<std::shared_ptr<planner::AbstractPlan>, decltype(query_list.begin()),
           ComparePlan>
      cache_map;
