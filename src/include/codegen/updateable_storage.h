...
class UpdateableStorage {
 public:
  ...
  uint32_t Add(type::Type::TypeId type);
  ...
  codegen::Value Get(CodeGen &codegen, llvm::Value *area_start,
                     uint64_t index) const;

  // Get the value at a specific index into the storage area
  void Set(CodeGen &codegen, llvm::Value *area_start, uint64_t index,
