...
//===----------------------------------------------------------------------===//
// A container/context where all the LLVM code for a single query is stored. We
// create a code context instance for every query we see.  We keep instances
// of these around in the off chance that we see a query that requires a
// function we've previously JITed.
//===----------------------------------------------------------------------===//
class CodeContext {
