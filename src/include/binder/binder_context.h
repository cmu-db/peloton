...
// can bind to.
    ...
    public:
      ...
      void AddTable(const parser::TableRef* table_ref, concurrency::Transaction *consistentTxn);

      // Update the table alias map given a table reference (in the from clause)
      void AddTable(const std::string db_name, const std::string table_name, concurrency::Transaction *consistentTxn);
