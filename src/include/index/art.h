...
class AdaptiveRadixTree {
 public:
  using LoadKeyFunction = void (*)(TID tid, ARTKey &key,
 ...
 public:
  ...
  static CheckPrefixPessimisticResult CheckPrefixPessimistic(
      N *n, const ARTKey &k, uint32_t &level, uint8_t &nonMatchingKey,
      Prefix &nonMatchingPrefix, LoadKeyFunction loadKey, bool &needRestart,
      IndexMetadata *metadata);

  static PCCompareResults CheckPrefixCompare(const N *n, const ARTKey &k,
                                             uint8_t fillKey, uint32_t &level,
                                             LoadKeyFunction loadKey,
                                             bool &needRestart,
                                             IndexMetadata *metadata);

  static PCEqualsResults CheckPrefixEquals(
      const N *n, uint32_t &level, const ARTKey &start, const ARTKey &end,
 ...
 public:
  ...
  AdaptiveRadixTree(AdaptiveRadixTree &&t)
      : root_(t.root_), load_key_(t.load_key_), epoch_manager_(256) {}

  ~AdaptiveRadixTree();

  ThreadInfo &GetThreadInfo();

  TID Lookup(const ARTKey &k, ThreadInfo &threadEpocheInfo,
             std::vector<ItemPointer *> &result) const;

  bool LookupRange(const ARTKey &start, const ARTKey &end, ARTKey &continueKey,
                   std::vector<ItemPointer *> &result, std::size_t resultLen,
                   std::size_t &resultCount,
                   ThreadInfo &threadEpocheInfo) const;

  void FullScan(std::vector<ItemPointer *> &result, std::size_t &resultCount,
                ThreadInfo &threadEpocheInfo) const;

  void ScanAllLeafNodes(const N *node, std::vector<ItemPointer *> &result,
                        std::size_t &resultCount) const;

  void Insert(const ARTKey &k, TID tid, ThreadInfo &epocheInfo,
              bool &insertSuccess);

  bool Remove(const ARTKey &k, TID tid, ThreadInfo &epocheInfo);

  bool ConditionalInsert(const ARTKey &k, TID tid, ThreadInfo &epocheInfo,
