...
class IndexUtil {
 public:
  ...
  static void ConstructIntervals(
  ...
  static void FindMaxMinInColumns(
      oid_t leading_column_id, const std::vector<type::Value> &values,
      const std::vector<oid_t> &key_column_ids,
      const std::vector<ExpressionType> &expr_types,
      std::map<oid_t, std::pair<type::Value, type::Value>> &
          non_leading_columns);

  static bool HasNonOptimizablePredicate(
      const std::vector<ExpressionType> &expr_types);

  static bool FindValueIndex(
