...
class ArtIndex : public Index {
 ...
 public:
  ...
  bool DeleteEntry(const storage::Tuple *key, ItemPointer *location_ptr);
  ...
  static void WriteValueInBytes(type::Value value, char *c, int offset,
                                int length);

  static void WriteIndexedAttributesInKey(const storage::Tuple *tuple,
                                          ARTKey &index_key);
