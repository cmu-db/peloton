...
class SkipList {
 ...
 public:
  ...
  SkipList(bool start_gc_thread = true, bool unique_key = false,
  ...
  ~SkipList() {
    ...
    while (get_node_address(next) != nullptr) {
      if (!is_deleted_node(next)) {  // current node is not deleted
        node_count++;
        delete (current);
        LOG_TRACE("freed 1 node, size is %zu", sizeof(Node));
        current = next;
        next = current->next[0].load();
      } else {  // current node is marked as deleted, let GC thread to delete
                // that node
 ...
 public:
    ...
    if (node_level > old_head->level) {
      ...
      while (true) {
        ...
        if (old_head->level >= node_level) {
          ...
          // GC for new_head, because it won't be installed
          epoch->AddGarbageNode(new_head);
          break;
        }
        if (head.compare_exchange_strong(old_head, new_head)) {
          // GC for old_head if new_head is installed successfully
  ...
  retry:
      ...
      if (unique_key) {  // insert fail
        ...
        while (true) {
          ...
          if (!is_deleted_val_ptr(val_next)) {  // tail is not deleted
            ...
            // install fail, just run the loop again, retry insertion in the
            // value list
          }
        }
      }
    ...
    for (int i = 1; i < node_level; i++) {
      while (true) {
        ...
        // concurrent deletion of current node and concurrent insertion of the
        // same key
        if (right != nullptr && KeyCmpEqual(right->key, key))
          right = get_node_address(right->next[i].load());
        if (left->next[i].compare_exchange_strong(right, new_node)) break;
        // CAS fail
  ...
  bool ConditionalInsert(const KeyType &key, const ValueType &value,
    ...
    LOG_TRACE("Cond Insert join epoch");
    EpochNode *current_epoch = epoch->JoinEpoch();
    ValueNode *val_ptr = new ValueNode(value);
    epoch->memory_footprint.fetch_add(sizeof(ValueNode));
    int node_level = get_rand_level();
    ...
    if (succs[0] != nullptr && KeyCmpEqual(succs[0]->key, key)) {
      ...
      if (unique_key) {
        ...
        while (true) {
          ...
          // current successor exist
          old_val_ptr = get_tail_value_without_predicate(
              old_val_ptr, val_ptr->val, predicate, predicate_satisfied);
          if (old_val_ptr == nullptr) {  // there is an exist value in the value
                                         // list equal to val_ptr->val
            // GC ValueNode and Node
            epoch->AddGarbageValueNode(val_ptr);
  ...
  bool Delete(const KeyType &key, const ValueType &value) {
    ...
    Search(key, preds, succs, level);
    if (succs[0] == nullptr ||
        !KeyCmpEqual(succs[0]->key, key)) {  // key not exist
      delete[](preds);
      delete[](succs);
      LOG_TRACE("delete leave epoch");
      epoch->LeaveEpoch(current_epoch);
      return false;
    }
    ValueNode *old_val_ptr = succs[0]->val_ptr.load();
    // WARNING: we have to explicitly declare a variable of nullptr for CAS
    // operation
    ...
    if (unique_key) {  // key is unique
      if (old_val_ptr != nullptr && !is_deleted_val_ptr(old_val_ptr) &&
        ...
        // WARNING: we should guarantee we only use pointer with address WITHOUT
        // a delete bit
        ...
        while (true) {
          ...
          if (succs[0]->val_ptr.compare_exchange_strong(old_val_ptr,
                                                        empty_val_ptr)) {
      ...
      if (find_delete) {  // find value and logically delete it
        ...
        if (new_val_ptr != old_val_ptr) {
          ...
          while (!(succs[0]->val_ptr.compare_exchange_strong(old_val_ptr,
                                                             new_val_ptr))) {
            old_val_ptr = succs[0]->val_ptr.load();
          }
          if (new_val_ptr == nullptr) {  // all values are deleted, the key Node
    ...
    for (int i = level - 1; i >= 0; i--) {
      ...
      for (right = left_next;; right = right_next) {
        ...
        while (true) {
          ...
          if (right_next == nullptr || !is_deleted_node(right_next)) break;
          right = get_node_address(right_next);
        }
        // key <= right->key || right == nullptr means right is at the end of
        // the list
        if (right == nullptr || !KeyCmpLess(right->key, key)) break;
        // key > right->key
        left = right;
      ...
      if ((left_next != right) &&
          !(left->next[i].compare_exchange_strong(left_next, right)))
        goto retry;
      left_list[i] = left;
      right_list[i] = right;
    }
   ...
   public:
    ...
    inline void MoveAheadByOne() {
      ...
      if (val_ptr != nullptr)  // value chain is not exhausted yet
        return;
      // value chain is exhausted, find next exist node's key and exist value of
      // that key
      node_ptr = get_node_address(node_ptr->next[0].load());
      while (node_ptr != nullptr) {
        if (is_deleted_node(
                node_ptr->next[0].load())) {  // node is marked as deleted
          node_ptr = get_node_address(node_ptr->next[0].load());
        } else {  // node exist
  ...
  */
  ForwardIterator Begin() {
    LOG_TRACE("fwd begin join epoch");
    EpochNode *current_epoch = epoch->JoinEpoch();
    ...
    LOG_TRACE("fwd begin leave epoch");
    epoch->LeaveEpoch(current_epoch);
  ...
  size_t GetMemoryFootprint() { return epoch->memory_footprint; }

 private:
  // xingyuj1
  // value node is used to build linked list of ValueType to use CAS in SkipList
  // node
  class ValueNode {
   public:
    ValueType val;
    std::atomic<ValueNode *>
        next;  // support concurrent delete in duplicate key mode
    ValueNode(ValueType val) {
      this->val = val;
      next = nullptr;
    }
    ValueNode(ValueType val, ValueNode *next) {
      this->val = val;
      this->next = next;
    }
    ~ValueNode(){};
  ...
  /*
   * class Epoch - Referenced from BwtreeIndex's EpochManager class.
   *               Manages epoch-related behaviour for garbage collection
   *               Including join/leave epoch, create new epoch, add garbage
   *               node and gc.
   */
  ...
  class Epoch {
   public:
    /*
     * struct GarbageNode - Wrapper for nodes that are marked as garbage
     */
    struct GarbageNode {
      const Node *node;
      GarbageNode *next;
    ...
    /*
     * struct GarbageNode - Wrapper for value nodes that are marked as
     *                      garbage
     */
    ...
    /*
     * struct EpochNode - Node for representing an epoch.
     *                    stores current epoch's garbage nodes/value nodes.
    ...
    // current epoch and head epoch
    EpochNode *current;
    EpochNode *head;

    /*
     * Epoch() - constructor of Epoch class. Initialize data structures.
     */
    Epoch() {
      memory_value = 0;
      memory_footprint = 0;
    ...
    /*
     * Epoch() - destructor of Epoch class. Perform GC and free memory.
     */
    ~Epoch() {
      ...
      // Check if clean successful. If not, force clean data structure.
      if (head != nullptr) {
        ...
        for (EpochNode *epoch_node = head; epoch_node != nullptr;
          ...
          LOG_DEBUG("Active thread count: %d", epoch_node->thread_count.load());
    ...
    /*
     * CreateEpoch() - create new epoch and make it the current epoch.
     *                 any thread will now join this epoch.
     */
    void CreateEpoch() {
    ...
    /*
     * AddGarbageNode() - Add a node that has been marked as garbage
     *                    in the current epoch.
     */
    void AddGarbageNode(const Node *node) {
      // Create weapper and add to current epoch
      ...
      // Use CAS to append to the tail of linked list
      while (!epoch->garbage_list.compare_exchange_strong(new_garbage->next,
                                                          new_garbage)) {
    ...
    /*
     * AddGarbageValueNode() - Add a value node that has been marked as
     *                         garbage in the current epoch.
     */
    ...
    /*
     * JoinEpoch() - let the current thread join the current epoch.
     *               Current epoch's thread count +1
     */
    EpochNode *JoinEpoch() {
      int64_t count = -1;
      EpochNode *epoch = nullptr;

      // Count maybe less than 0, since we aggresively reduce this count
      // in the gc process. If count is less than 0, it means another thread
      // is trying to gc this Epoch. We should keep trying until a new epoch
    ...
    /*
     * LeaceEpoch() - let the current thread leave the current epoch.
     *                Current epoch's thread count -1
     */
    ...
    /*
     * ClearOldEpoch() - perform garbage collection.
     *                   Starting from the head of epoch chain, all old
     *                   epoch whose active thread count is 0 will be
     *                   cleaned.
     */
    size_t ClearOldEpoch() {
      LOG_DEBUG("Start to clear epoch");
      size_t total_memory_freed = 0;
      size_t total_memory_freed_value = 0;
      int retry = 0;
      LOG_DEBUG(
          "Before gc, total memory is %zu, total memory for valuenode is %zu",
          (size_t)memory_footprint, (size_t)memory_value);

      // Loop through all cleanable epochs
      while (true) {
        if (head == current) {
        ...
        // Retry to wait threads to leave current epoch being cleaned
        int count = head->thread_count.load();
        LOG_TRACE("Count of thread is: %d", count);
        PL_ASSERT(count >= 0);
        if (count != 0) {
          if (retry < 100) {
            retry++;
        ...
        // Check for head thread count now. Some thread may modify head
        // after current thread start cleaning. In this situation, abort
        ...
        // Pointer for garbage nodes. Need to clean both node and value node
        const GarbageNode *next_garbage_node = nullptr;
        const GarbageValueNode *next_garbage_value_node = nullptr;

        // Clean value node
        for (const GarbageValueNode *current_node =
                 head->garbage_value_list.load();
             current_node != nullptr; current_node = next_garbage_value_node) {
        ...
        // Clean node
        for (const GarbageNode *current_node = head->garbage_list.load();
             current_node != nullptr; current_node = next_garbage_node) {
      ...
      LOG_DEBUG(
          "Finish clearing epoch, memory freed: %zu, memory freed for value "
          "nodes: %zu",
          total_memory_freed, total_memory_freed_value);
      memory_footprint.fetch_sub(total_memory_freed);
      memory_value.fetch_sub(total_memory_freed_value);
      LOG_DEBUG(
          "After gc, total memory is %zu, total memory for valuenode is %zu",
          (size_t)memory_footprint, (size_t)memory_value);
  ...
  void delete_node(Node *p) {
    ...
    for (int i = p->level - 1; i >= 0; i--) {
      while (true) {
        next = p->next[i].load();
        if (is_deleted_node(next)) break;
        if (p->next[i].compare_exchange_strong(next,
                                               delete_node_address(next))) {
  ...
  bool delete_value(ValueNode *val_ptr, const ValueType &value) {
    ...
    while (v != nullptr) {
      if (ValueCmpEqual(v->val, value)) {
        ...
        while (true) {
          v_next = v->next.load();
          if (is_deleted_val_ptr(v_next)) {  // ValueNode is deleted by other
                                             // thread, skip this node
            break;
          }
          if (v->next.compare_exchange_strong(v_next,
                                              delete_val_address(v_next))) {
            // mark delete bit
