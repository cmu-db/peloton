...
// buffered_io.h
//
// Identification: src/include/network/buffered_io.h
//
...
class ReadBuffer : public Buffer {
 public:
   ...
   * or 4 bytes). It is assumed that the bytes in the buffer are in network
   ...
   * @param len Length of the integer, either 1, 2, or 4 bytes.
   * @return value of integer switched from network byte order
   */
  int ReadInt(uint8_t len);
