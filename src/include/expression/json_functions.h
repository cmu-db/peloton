...
// json_functions.h
//
// Identification: src/include/expression/json_functions.h
//
...
class JsonDocument {
 public:
  JsonDocument(const char *docChars, int32_t lenDoc)
  ...
  bool Get(const char *pathChars, int32_t lenPath,
    ...
    for (std::vector<JsonPathNode>::const_iterator cit = path.begin();
         cit != path.end(); ++cit) {
  ...
  void set(const char *pathChars, int32_t lenPath, const char *valueChars,
           int32_t lenValue) {
    // translate database nulls into JSON nulls, because that's really all that
    // makes
    ...
    std::vector<JsonPathNode> path = resolveJsonPath(
        pathChars, lenPath, true /*enforceArrayIndexLimitForSet*/);
    // the non-const version of the Json::Value [] operator creates a new, null
    // node on attempted
 ...
 private:
  ...
  std::vector<JsonPathNode> resolveJsonPath(
      const char *pathChars, int32_t lenPath,
      ...
      if (expectArrayIndex) {
        ...
        if (c == '-') {
          ...
          if (!readChar(c)) {
            ThrowInvalidPathError(
                "Unexpected termination (unterminated array access)");
          }
        }
        if (c < '0' || c > '9') {
        ...
        while (readChar(c)) {
          ...
          if (enforceArrayIndexLimitForSet) {
            // T.Is 500000 is a mostly arbitrary maximum JSON array index
            // enforced for practical
            // purposes. We enforce this up front to avoid excessive delays,
            // ridiculous short-term
            // memory growth, and/or bad_alloc errors that the jsoncpp library
            // could produce
            // essentially for nothing since our supported JSON document columns
            // are typiCally not
            // wide enough to hold the string representations of arrays this
            // large.
            if (arrayIndex > 500000) {
              if (neg) {
                // other than the special '-1' case, negative indices aren't
                // allowed
                ThrowInvalidPathError("Array index less than -1");
              }
              ThrowInvalidPathError(
                  "Array index greater than the maximum allowed value of "
                  "500000");
            }
          } else {
            if (arrayIndex > static_cast<int64_t>(INT32_MAX)) {
              ...
              ThrowInvalidPathError(
                  "Array index greater than the maximum integer value");
            }
          }
        }
        if (!terminated) {
        ...
        // handle the case of empty field names. for example, Getting the first
        ...
        do {
          if (c == '\\') {
            if (!readChar(c) ||
                (c != '[' && c != ']' && c != '.' && c != '\\')) {
              ThrowInvalidPathError(
                  "Unescaped backslash (double escaping required for path)");
            }
          } else if (c == '.') {
            expectField = true;
            break;
  ...
  void ThrowInvalidPathError(const char *err) const {
    char msg[1024];
    snprintf(msg, sizeof(msg), "Invalid JSON path: %s [position %d]", err,
             m_pos);
    throw Exception(msg);
  }
...
inline Value Value::Call<FUNC_VOLT_FIELD>(const std::vector<Value> &arguments) {
  ...
  const char *docChars =
      reinterpret_cast<char *>(docNVal.GetObjectValueWithoutNull());
  JsonDocument doc(docChars, lenDoc);

  int32_t lenPath = pathNVal.GetObjectLengthWithoutNull();
  const char *pathChars =
      reinterpret_cast<char *>(pathNVal.GetObjectValueWithoutNull());
  std::string result;
...
inline Value Value::Call<FUNC_VOLT_ARRAY_ELEMENT>(
  ...
  return GetTempStringValue(serializedValue.c_str(),
                            serializedValue.length() - 1);
}

/** implement the 1-argument SQL ARRAY_LENGTH function */
template <>
inline Value Value::CallUnary<FUNC_VOLT_ARRAY_LENGTH>() const {
  if (IsNull()) {
...
inline Value Value::Call<FUNC_VOLT_SET_FIELD>(
  ...
  const char *valueChars =
      reinterpret_cast<char *>(valueNVal.GetObjectValueWithoutNull());
  try {
