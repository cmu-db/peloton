...
// bitwise_functions.h
//
// Identification: src/include/expression/bitwise_functions.h
//
...
template <>
inline Value Value::CallUnary<FUNC_VOLT_BITNOT>() const {
...
template <>
inline Value Value::CallUnary<FUNC_VOLT_HEX>() const {
  if (GetValueType() != VALUE_TYPE_BIGINT) {
    // The parser should enforce this for us, but just in case...
...
template <>
inline Value Value::CallUnary<FUNC_VOLT_BIN>() const {
  if (GetValueType() != VALUE_TYPE_BIGINT) {
    // The parser should enforce this for us, but just in case...
...
inline Value Value::Call<FUNC_BITAND>(const std::vector<Value> &arguments) {
  ...
  if (lval.GetValueType() != VALUE_TYPE_BIGINT ||
      rval.GetValueType() != VALUE_TYPE_BIGINT) {
...
inline Value Value::Call<FUNC_VOLT_BIT_SHIFT_LEFT>(
  ...
  if (lval.GetValueType() != VALUE_TYPE_BIGINT) {
    throw Exception(
        "unsupported non-BigInt type for SQL BIT_SHIFT_LEFT function");
  }
  ...
  if (shifts < 0) {
    throw Exception("unsupported negative value for bit shifting");
  }
  ...
  if (result == INT64_NULL) {
    throw Exception(
        "Application of bithise function BIT_SHIFT_LEFT would produce "
        "INT64_MIN, "
        "which.Is reserved for SQL NULL values.");
  }
...
inline Value Value::Call<FUNC_VOLT_BIT_SHIFT_RIGHT>(
  ...
  if (lval.GetValueType() != VALUE_TYPE_BIGINT) {
    throw Exception(
        "unsupported non-BigInt type for SQL BIT_SHIFT_RIGHT function");
  }
  ...
  if (result == INT64_NULL) {
    throw Exception(
        "Application of bithise function BIT_SHIFT_RIGHT would produce "
        "INT64_MIN, "
        "which.Is reserved for SQL NULL values.");
  }
