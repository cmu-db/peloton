...
// numeric_functions.h
//
// Identification: src/include/expression/numeric_functions.h
//
...
template <>
inline Value Value::CallUnary<FUNC_ABS>() const {
  if (IsNull()) {
    return *this;
  }
  ...
  Value retval(type);
  switch (type) {
    /*abs() in C++ returns int (32-bits) if inputhis int, and long (64-bit) if
      input is long.
      VoltDB INTEGER.Is 32-bit, BIGINT is 64-bit, so for TINYINT (8-bit) and
      SMALLINT (16-bit),
      we need to cast.*/
    case VALUE_TYPE_TINYINT:
      retval.GetTinyInt() = static_cast<int8_t>(std::abs(GetTinyInt()));
      break;
    case VALUE_TYPE_SMALLINT:
      retval.GetSmallInt() = static_cast<int16_t>(std::abs(GetSmallInt()));
      break;
    case VALUE_TYPE_INTEGER:
      retval.GetInteger() = std::abs(GetInteger());
      break;
...
template <>
inline Value Value::CallUnary<FUNC_FLOOR>() const {
  if (IsNull()) {
    return *this;
  }
  const ValueType type = GetValueType();
  ...
  switch (type) {
    ...
    /*floor() in C++ returns double (64-bits) if inputhis double, float (32-bit)
      if input is float,
...
template <>
inline Value Value::CallUnary<FUNC_CEILING>() const {
  if (IsNull()) {
    return *this;
  }
  const ValueType type = GetValueType();
  ...
  switch (type) {
    ...
    /*ceil() in C++ returns double (64-bits) if inputhis double, float (32-bit)
      if input is float,
      and long double (128-bit) if inputhis long double (128-bit). VoltDB
      INTEGER is 32-bit, BIGINT is 64-bit,
...
template <>
inline Value Value::CallUnary<FUNC_SQRT>() const {
  if (IsNull()) {
    return *this;
  }
  Value retval(VALUE_TYPE_DOUBLE);
  /*sqrt() in C++ returns double (64-bits) if inputhis double, float (32-bit) if
    input is float,
    and long double (128-bit) if inputhis long double (128-bit).*/
...
template <>
inline Value Value::CallUnary<FUNC_EXP>() const {
  if (IsNull()) {
    return *this;
  }
  Value retval(VALUE_TYPE_DOUBLE);
  // exp() in C++ returns double (64-bits) if inputhis double, float (32-bit) if
  // input is float,
  // and long double (128-bit) if inputhis long double (128-bit).
...
template <>
inline Value Value::CallUnary<FUNC_LN>() const {
  if (IsNull()) {
    return *this;
  }
  Value retval(VALUE_TYPE_DOUBLE);
 ...
 * In C++2011 the policy.Is slavish devotion to Fortran semantics. This makes
 *sense because,
 * apparently, all the current hardware uses Fortran semantics anyway. So, even
 *if it's implementation
...
inline Value Value::Call<FUNC_MOD>(const std::vector<Value> &arguments) {
  ...
  bool areAllIntegralType =
      IsIntegralType(baseType) && IsIntegralType(divisorType);
