...
class TimestampCheckpointManager : public CheckpointManager {
 ...
 protected:
  ...
  // read table data without tile group and write it down to checkpoint data file
  ...
  void CheckpointingStorageObject(FileHandle &file_handle, concurrency::TransactionContext *txn);
  ...
  bool RecoverStorageObject(FileHandle &file_handle, concurrency::TransactionContext *txn);
  ...
  // this function is provided for initialized catalog table not including default value
  // return: inserted tuple count into table
  oid_t RecoverTableDataWithoutTileGroup(storage::DataTable *table,FileHandle &file_handle,
  		concurrency::TransactionContext *txn);
  ...
  oid_t RecoverTableDataWithDuplicateCheck(storage::DataTable *table, FileHandle &file_handle,
  		concurrency::TransactionContext *txn);
		...
		for(auto sub_dir_name : StringUtil::Split(dir_name, '/')) {
			...
			if (LoggingUtil::CheckDirectoryExistence(sub_dir_path.c_str()) == false) {
				...
				if (LoggingUtil::CreateDirectory(sub_dir_path.c_str(), 0700) == false) {
					LOG_ERROR("Cannot create directory in this checkpoints: %s", dir_name.c_str());
					break;
				}
			}
		}
  ...
  void CreateCheckpointDirectory(const std::string &dir_name) {
  	...
  	if (LoggingUtil::CheckDirectoryExistence(checkpoint_dir.c_str()) == false) {
    	LOG_TRACE("Create checkpoint directory %s", checkpoint_dir.c_str());
  		...
  		LOG_TRACE("Found checkpoint directory %s, and delete all old files", checkpoint_dir.c_str());
  ...
  std::string GetMetadataFileFullPath(const eid_t &epoch_id) {
  	return checkpoint_base_dir_ + "/" + std::to_string(epoch_id) + "/" + metadata_filename_prefix_;
  }
  std::string GetWorkingMetadataFileFullPath() {
  	return checkpoint_base_dir_ + "/" + checkpoint_working_dir_name_ + "/" + metadata_filename_prefix_;
  }

  // remove old checkpoints except for the designated epoch id (latest checkpointing time)
