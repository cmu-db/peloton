...
class PhyLogLogger {
 public:
  ...
  void StartRecovery(const size_t checkpoint_eid, const size_t persist_eid,
  ...
  size_t GetPersistEpochId() const { return persist_epoch_id_; }
 ...
 private:
  ...
  void PersistLogBuffer(FileHandle &file_handle,
  ...
  void GetSortedLogFileIdList(const size_t checkpoint_eid,
                              const size_t persist_eid);

  void RunRecoveryThread(const size_t thread_id, const size_t checkpoint_eid,
                         const size_t persist_eid);

  void RunSecIndexRebuildThread(const size_t logger_count);

  void RebuildSecIndexForTable(const size_t logger_count,
                               storage::DataTable *table);

  bool ReplayLogFile(const size_t thread_id, FileHandle &file_handle,
                     size_t checkpoint_eid, size_t pepoch_eid);
