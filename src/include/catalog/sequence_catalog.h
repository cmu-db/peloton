...
class SequenceCatalogObject {
 public:
  SequenceCatalogObject(oid_t seq_oid, oid_t db_oid, oid_t namespace_oid,
                        const std::string &name, const int64_t seqstart,
                        const int64_t seqincrement, const int64_t seqmax,
                        const int64_t seqmin, const bool seqcycle,
                        const int64_t seqval,
  ...
  oid_t GetSequenceOid() const { return (seq_oid_); }

  oid_t GetDatabaseOid() const { return (db_oid_); }

  oid_t GetNamespaceOid() const { return (namespace_oid_); }

  std::string GetName() const { return (seq_name_); }

  int64_t GetStart() const { return (seq_start_); }

  int64_t GetIncrement() const { return (seq_increment_); }

  int64_t GetMax() const { return (seq_max_); }

  int64_t GetMin() const { return (seq_min_); }

  int64_t GetCacheSize() const { return (seq_cache_); }

  bool GetAllowCycle() const { return (seq_cycle_); }
  ...
  int64_t GetCurrVal() const { return (seq_prev_val_); }
...
class SequenceCatalog : public AbstractCatalog {
 public:
   ...
   */
  bool InsertSequence(concurrency::TransactionContext *txn, oid_t database_oid,
                      oid_t namespace_oid, std::string sequence_name,
   ...
   * otherwise.
   */
  ResultType DropSequence(concurrency::TransactionContext *txn,
   ...
   * @param   namespace_oid the namespace oid for the sequence
   * @param   sequence_name the name of the sequence
   * @return  a SequenceCatalogObject if the sequence is found, nullptr
   * otherwise
   */
  ...
  /**
   * @brief   get sequence oid from pg_sequence given sequence_name and
   * database_oid
   * @param   txn           current transaction
   * @param   database_oid  the database_oid for the sequence
   * @param   namespace_oid the namespace oid for the sequence
   * @param   sequence_name the name of the sequence
   * @return  the oid_t of the sequence if the sequence is found, INVALID_OI
   * otherwise
   */
  oid_t GetSequenceOid(concurrency::TransactionContext *txn, oid_t database_oid,
                       oid_t namespace_oid, const std::string &sequence_name);
   ...
   * @return
   */
  bool UpdateNextVal(concurrency::TransactionContext *txn, oid_t database_oid,
                     oid_t namespace_oid, const std::string &sequence_name,
                     int64_t nextval);
  ...
  enum IndexId { PRIMARY_KEY = 0, DATABASE_NAMESPACE_SEQNAME_KEY = 1 };
  ...
  void ValidateSequenceArguments(int64_t seq_increment, int64_t seq_max,
    ...
    if (seq_min > seq_max) {
      throw SequenceException(StringUtil::Format(
          "MINVALUE (%ld) must be no greater than MAXVALUE (%ld)", seq_min,
          seq_max));
    }

    if (seq_increment == 0) {
      throw SequenceException(StringUtil::Format("INCREMENT must not be zero"));
    }

    if (seq_increment > 0 && seq_start < seq_min) {
      throw SequenceException(StringUtil::Format(
          "START value (%ld) cannot be less than MINVALUE (%ld)", seq_start,
          seq_min));
    }

    if (seq_increment < 0 && seq_start > seq_max) {
      throw SequenceException(StringUtil::Format(
          "START value (%ld) cannot be greater than MAXVALUE (%ld)", seq_start,
          seq_max));
    }
  };
};
