...
class raw_foreign_key_info {
 public:
  raw_foreign_key_info(oid_t source_table_id, oid_t sink_table_id,
                       std::vector<int> source_column_offsets,
                       std::vector<int> sink_column_offsets, char update_action,
                       char delete_action, std::string fk_name)
 ...
 private:
  ...
  // Refer::
  // http://www.postgresql.org/docs/9.4/static/catalog-pg-constraint.html
  // foreign key action
  char update_action;
  char delete_action;
