...
class LSPIIndexTuner{
 ...
 private:
  ...
  std::unique_ptr<CompressedIndexConfiguration> index_config_;
  ...
  /**
   * Constructs the feature vector representing the SQL query running on the current
   ...
   * where f(query) is first recommended_index(query)(0->n), then drop_index(query)(n->2*n)
   ...
   * // TODO: not in f(query) should split into:  (i)!f(query) && belongs(config) (ii) !(f(query) && belongs(config))?
   */
  void ConstructQueryConfigFeature(std::unique_ptr<boost::dynamic_bitset<>>& add_candidates,
