...
class IntegerParentType : public NumericType {
 public:
  ...
  virtual CmpBool CompareGreaterThan(const Value& left,
                                     const Value& right) const override = 0;
  virtual CmpBool CompareGreaterThanEquals(
      const Value& left, const Value& right) const override = 0;

  virtual Value CastAs(const Value& val,
                       const Type::TypeId type_id) const override = 0;
  ...
  virtual void SerializeTo(const Value& val,
                           SerializeOutput& out) const override = 0;
  virtual void SerializeTo(const Value& val, char* storage, bool inlined,
                           AbstractPool* pool) const override = 0;

  // Deserialize a value of the given type from the given storage space.
  virtual Value DeserializeFrom(
      const char* storage, const bool inlined,
      AbstractPool* pool = nullptr) const override = 0;
  virtual Value DeserializeFrom(
      SerializeInput& in, AbstractPool* pool = nullptr) const override = 0;
