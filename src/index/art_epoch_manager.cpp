...
namespace peloton {
namespace index {
ARTEpochManager::ARTEpochManager(size_t start_GC_threshhold):start_GC_threshhold_(start_GC_threshhold) {
  ...
  thread_info_list_ = reinterpret_cast<PaddedThreadInfo *>(
    (reinterpret_cast<size_t>(original_pointer_) + CACHE_LINE_SIZE - 1) & CACHE_LINE_MASK);
