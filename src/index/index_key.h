  ...
  std::string debug(const catalog::Schema *keySchema) const {
    ...
    for (int ii = 0; ii < GetColumnCount; ii++) {
      switch(keySchema->GetType(ii)) {
        ...
        default:
          throw NotImplementedException("We currently only support a specific set of column index sizes...");
          break;
      }
    }
...
class IntsComparator {
 public:
  ...
  IntsComparator(catalog::Schema *keySchema) : m_keySchema(keySchema) {}
...
class IntsEqualityChecker {
 public:
  ...
  IntsEqualityChecker(catalog::Schema *keySchema) : m_keySchema(keySchema) {}
...
class GenericKey {
 public:
  ...
  inline void setFromTuple(const storage::Tuple *tuple, const int *indices, const catalog::Schema *keySchema) {
    storage::Tuple keyTuple(keySchema, reinterpret_cast<void*>(data));
...
class GenericComparator {
 public:
  ...
  GenericComparator(catalog::Schema *keySchema) : m_schema(keySchema) {}

  inline bool operator()(const GenericKey<keySize> &lhs, const GenericKey<keySize> &rhs) const {
    storage::Tuple lhTuple(m_schema, reinterpret_cast<const void*>(&lhs));
    storage::Tuple rhTuple(m_schema, reinterpret_cast<const void*>(&rhs));
    // lexographical compare could be faster for fixed N
...
class GenericEqualityChecker {
 public:
  /** Type information passed to the constuctor as it's not in the key itself */
  GenericEqualityChecker(catalog::Schema *keySchema) : m_schema(keySchema) {}

  inline bool operator()(const GenericKey<keySize> &lhs, const GenericKey<keySize> &rhs) const {
    storage::Tuple lhTuple(m_schema, reinterpret_cast<const void*>(&rhs));
    storage::Tuple rhTuple(m_schema, reinterpret_cast<const void*>(&rhs));
    return (lhTuple == rhTuple);
  }
...
{
  /** Type information passed to the constuctor as it's not in the key itself */
  GenericHasher(catalog::Schema *keySchema) : m_schema(keySchema) {}

  /** Generate a 64-bit number for the key value */
  inline size_t operator()(GenericKey<keySize> const &p) const {
    storage::Tuple pTuple(m_schema, reinterpret_cast<const void*>(&p));
...
class TupleKey {
 public:
  ...
  storage::Tuple getTupleForComparison() const {
    return storage::Tuple(m_keyTuple, m_keySchema);
  }
...
class TupleKeyComparator {
 public:
  TupleKeyComparator(catalog::Schema *keySchema) : m_schema(keySchema) {
  }
...
class TupleKeyEqualityChecker {
 public:
  TupleKeyEqualityChecker(catalog::Schema *keySchema) : m_schema(keySchema) {
  }
