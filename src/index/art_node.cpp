...
NTypes N::GetType() const {
  return static_cast<NTypes>(
      type_version_lock_obsolete_.load(std::memory_order_relaxed) >> 62);
}
...
void N::UpgradeToWriteLockOrRestart(uint64_t &version, bool &needRestart) {
  if (type_version_lock_obsolete_.compare_exchange_strong(version,
                                                          version + 0b10)) {
...
void N::WriteUnlock() { type_version_lock_obsolete_.fetch_add(0b10); }
...
void N::InsertGrow(curN *n, uint64_t v, N *parentNode, uint64_t parentVersion,
...
void N::InsertAndUnlock(N *node, uint64_t v, N *parentNode,
                        uint64_t parentVersion, uint8_t keyParent, uint8_t key,
                        N *val, bool &needRestart, ThreadInfo &threadInfo) {
  switch (node->GetType()) {
    case NTypes::N4: {
      auto n = static_cast<N4 *>(node);
      InsertGrow<N4, N16>(n, v, parentNode, parentVersion, keyParent, key, val,
                          needRestart, threadInfo);
      break;
    }
    case NTypes::N16: {
      auto n = static_cast<N16 *>(node);
      InsertGrow<N16, N48>(n, v, parentNode, parentVersion, keyParent, key, val,
                           needRestart, threadInfo);
      break;
    }
    case NTypes::N48: {
      auto n = static_cast<N48 *>(node);
      InsertGrow<N48, N256>(n, v, parentNode, parentVersion, keyParent, key,
                            val, needRestart, threadInfo);
      break;
    }
    case NTypes::N256: {
      auto n = static_cast<N256 *>(node);
      InsertGrow<N256, N256>(n, v, parentNode, parentVersion, keyParent, key,
                             val, needRestart, threadInfo);
      break;
    }
  }
...
void N::RemoveAndShrink(curN *n, uint64_t v, N *parentNode,
                        uint64_t parentVersion, uint8_t keyParent, uint8_t key,
...
void N::RemoveAndUnlock(N *node, uint64_t v, uint8_t key, N *parentNode,
                        uint64_t parentVersion, uint8_t keyParent,
                        bool &needRestart, ThreadInfo &threadInfo) {
  switch (node->GetType()) {
    case NTypes::N4: {
      auto n = static_cast<N4 *>(node);
      RemoveAndShrink<N4, N4>(n, v, parentNode, parentVersion, keyParent, key,
                              needRestart, threadInfo);
      break;
    }
    case NTypes::N16: {
      auto n = static_cast<N16 *>(node);
      RemoveAndShrink<N16, N4>(n, v, parentNode, parentVersion, keyParent, key,
                               needRestart, threadInfo);
      break;
    }
    case NTypes::N48: {
      auto n = static_cast<N48 *>(node);
      RemoveAndShrink<N48, N16>(n, v, parentNode, parentVersion, keyParent, key,
                                needRestart, threadInfo);
      break;
    }
    case NTypes::N256: {
      auto n = static_cast<N256 *>(node);
      RemoveAndShrink<N256, N48>(n, v, parentNode, parentVersion, keyParent,
...
void N::RemoveLockedNodeAndUnlock(N *node, uint8_t key, N *parentNode,
                                  uint64_t parentVersion, uint8_t keyParent,
                                  bool &needRestart, ThreadInfo &threadInfo) {
  switch (node->GetType()) {
    case NTypes::N4: {
      auto n = static_cast<N4 *>(node);
      RemoveLockedNodeAndShrink<N4, N4>(n, parentNode, parentVersion, keyParent,
                                        key, needRestart, threadInfo);
      break;
    }
    case NTypes::N16: {
      auto n = static_cast<N16 *>(node);
      RemoveLockedNodeAndShrink<N16, N4>(n, parentNode, parentVersion,
                                         keyParent, key, needRestart,
                                         threadInfo);
      break;
    }
    case NTypes::N48: {
      auto n = static_cast<N48 *>(node);
      RemoveLockedNodeAndShrink<N48, N16>(n, parentNode, parentVersion,
                                          keyParent, key, needRestart,
                                          threadInfo);
      break;
    }
    case NTypes::N256: {
      auto n = static_cast<N256 *>(node);
      RemoveLockedNodeAndShrink<N256, N48>(n, parentNode, parentVersion,
                                           keyParent, key, needRestart,
                                           threadInfo);
      break;
    }
...
template <typename curN, typename smallerN>
void N::RemoveLockedNodeAndShrink(curN *n, N *parentNode,
                                  uint64_t parentVersion, uint8_t keyParent,
...
uint32_t N::GetPrefixLength() const { return prefix_count_; }

bool N::HasPrefix() const { return prefix_count_ > 0; }

uint32_t N::GetCount() const { return count_; }

const uint8_t *N::GetPrefix() const { return prefix_; }
...
void N::AddPrefixBefore(N *node, uint8_t key) {
  uint32_t prefixCopyCount =
      std::min(max_stored_prefix_length, node->GetPrefixLength() + 1);
  memmove(this->prefix_ + prefixCopyCount, this->prefix_,
          std::min(this->GetPrefixLength(),
                   max_stored_prefix_length - prefixCopyCount));
  memcpy(this->prefix_, node->prefix_,
...
N *N::SetLeaf(TID tid) {
  ...
  return reinterpret_cast<N *>((uint64_t)(value_list) |
                               (static_cast<uint64_t>(1) << 63));
}
