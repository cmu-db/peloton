...
Index *IndexFactory::GetIndex(catalog::IndexCatalogObject *index_catalog_object) {
  LOG_TRACE("Creating index %s", index_catalog_object->GetName().c_str());

  // The size of the key in bytes
  const auto key_size = index_catalog_object->key_schema->GetLength();
  LOG_TRACE("key_size : %d", key_size);
  LOG_TRACE("Indexed column count: %ld",
            index_catalog_object->key_schema->GetIndexedColumns().size());
  ...
  bool ints_only = true;
  for (auto column : index_catalog_object->key_schema->GetColumns()) {
  ...
  auto index_type = index_catalog_object->GetIndexType();
  Index *index = nullptr;
  ...
  if (index_type == IndexType::BWTREE) {
    if (ints_only) {
      index = IndexFactory::GetBwTreeIntsKeyIndex(index_catalog_object);
    } else {
      index = IndexFactory::GetBwTreeGenericKeyIndex(index_catalog_object);
    }
  ...
  // -----------------------
    ...
    if (ints_only) {
      index = IndexFactory::GetSkipListIntsKeyIndex(index_catalog_object);
    } else {
      index = IndexFactory::GetSkipListGenericKeyIndex(index_catalog_object);
    }
...
Index *IndexFactory::GetBwTreeIntsKeyIndex(catalog::IndexCatalogObject *index_catalog_object) {
  // Our new Index!
  Index *index = nullptr;
  ...
  if (key_size <= sizeof(uint64_t)) {
    ...
    index =
        new BWTreeIndex<CompactIntsKey<1>, ItemPointer *,
                        ...
                        ItemPointerHashFunc>(index_catalog_object);
...
#ifdef LOG_TRACE_ENABLED
  LOG_TRACE("%s", IndexFactory::GetInfo(index_catalog_object, comparatorType).c_str());
#endif
  return (index);
}

Index *IndexFactory::GetBwTreeGenericKeyIndex(catalog::IndexCatalogObject *index_catalog_object) {
  // Our new Index!
  Index *index = nullptr;
  ...
  if (key_size <= 4) {
    ...
    index =
        new BWTreeIndex<GenericKey<4>, ItemPointer *, FastGenericComparator<4>,
                        ...
                        ItemPointerComparator, ItemPointerHashFunc>(index_catalog_object);
  } else if (key_size <= 8) {
#ifdef LOG_TRACE_ENABLED
...
Index *IndexFactory::GetSkipListIntsKeyIndex(catalog::IndexCatalogObject *index_catalog_object) {
  // Our new Index!
  Index *index = nullptr;
...
Index *IndexFactory::GetSkipListGenericKeyIndex(catalog::IndexCatalogObject *index_catalog_object) {
  // Our new Index!
  Index *index = nullptr;
  ...
  if (key_size <= 4) {
    ...
    index =
        new SkipListIndex<GenericKey<4>, ItemPointer *,
                          ...
                          ItemPointerComparator>(index_catalog_object);
  } else if (key_size <= 8) {
#ifdef LOG_TRACE_ENABLED
    comparatorType = "GenericKey<8>";
#endif
...
std::string IndexFactory::GetInfo(catalog::IndexCatalogObject *index_catalog_object,
                                  std::string comparatorType) {
  std::ostringstream os;
  os << "Index '" << index_catalog_object->GetName() << "' => "
     << IndexTypeToString(index_catalog_object->GetIndexType()) << "::" << comparatorType
     << "(";
  bool first = true;
  for (auto column : index_catalog_object->key_schema->GetColumns()) {
    if (first == false) os << ", ";
