...
SmallintType::SmallintType() : IntegerParentType(Type::SMALLINT) {}
...
Value SmallintType::Sqrt(const Value &val) const {
  ...
  if (val.IsNull()) return ValueFactory::GetDecimalValue(PELOTON_DECIMAL_NULL);
...
Value SmallintType::OperateNull(const Value &left UNUSED_ATTRIBUTE,
...
void SmallintType::SerializeTo(const Value &val, char *storage,
...
Value SmallintType::CastAs(const Value &val, const Type::TypeId type_id) const {
  switch (type_id) {
    case Type::TINYINT: {
