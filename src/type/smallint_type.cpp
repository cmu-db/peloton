...
Value SmallintType::OperateNull(const Value& left UNUSED_ATTRIBUTE, const Value &right) const {
  ...
  case TypeId::PARAMETER_OFFSET:
    return ValueFactory::GetParameterOffsetValue((int32_t) PELOTON_VALUE_INT32_NULL);
  case TypeId::BIGINT:
...
Value SmallintType::CastAs(const Value& val,
  ...
  case TypeId::TINYINT: {
    ...
    if (val.GetAs<int16_t>() > PELOTON_VALUE_INT8_MAX ||
        val.GetAs<int16_t>() < PELOTON_VALUE_INT8_MIN)
