...
Value DecimalType::CastAs(const Value& val, const TypeId type_id) const {
  switch (type_id) {
    case TypeId::TINYINT: {
      ...
      if (val.GetAs<double>() > PELOTON_VALUE_INT8_MAX ||
          val.GetAs<double>() < PELOTON_VALUE_INT8_MIN)
    ...
    case TypeId::SMALLINT: {
      if (val.IsNull()) return ValueFactory::GetNullValueByType(type_id);
      if (val.GetAs<double>() > PELOTON_VALUE_INT16_MAX ||
          val.GetAs<double>() < PELOTON_VALUE_INT16_MIN)
        throw Exception(ExceptionType::OUT_OF_RANGE,
          "Numeric value out of range.");
    ...
    case TypeId::INTEGER: {
      if (val.IsNull()) return ValueFactory::GetNullValueByType(type_id);
      if (val.GetAs<double>() > PELOTON_VALUE_INT32_MAX ||
          val.GetAs<double>() < PELOTON_VALUE_INT32_MIN)
        throw Exception(ExceptionType::OUT_OF_RANGE,
          "Numeric value out of range.");
    ...
    case TypeId::BIGINT: {
      if (val.IsNull()) return ValueFactory::GetNullValueByType(type_id);
      if (val.GetAs<double>() > PELOTON_VALUE_INT64_MAX ||
          val.GetAs<double>() < PELOTON_VALUE_INT64_MIN)
        throw Exception(ExceptionType::OUT_OF_RANGE,
          "Numeric value out of range.");
