...
void DecentralizedEpochManager::ExitEpoch(const size_t thread_id,
...
eid_t DecentralizedEpochManager::GetExpiredEpochId() {
  ...
  // for all the local epoch contexts, obtain the minimum max committed epoch
  // id.
  for (auto &local_epoch_itr : local_epochs_) {
    // the centralized epoch manager must notify each local epoch
    // the current global epoch.
    eid_t local_expired_eid =
        local_epoch_itr.second->GetExpiredEpochId(current_global_epoch_id_);
