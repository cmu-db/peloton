...
namespace brain {
  ...
  // This interfaces with the optimizer to get the cost & physical plan of the query.
  // @parsed_sql_query: SQL statement
  // @index_set: set of indexes to be examined
  std::unique_ptr<optimizer::OptimizerPlanInfo>
    WhatIfIndex::GetCostAndPlanTree(parser::SQLStatement *parsed_sql_query,
  ...
  void WhatIfIndex::GetTablesUsed(parser::SQLStatement *parsed_statement, std::vector<std::string> &table_names) {
      ...
      case StatementType::SELECT:
        ...
        switch (sql_statement.select_stmt->from_table->type) {
          ...
          case TableReferenceType::JOIN:
            table_names.push_back(sql_statement.select_stmt->from_table->join->left.get()->GetTableName().c_str());
            break;
          ...
          case TableReferenceType::CROSS_PRODUCT:
            ...
            for (auto it = table_cp_list->begin(); it != table_cp_list->end(); it++) {
