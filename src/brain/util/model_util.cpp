...
void ModelUtil::GetBatches(const BaseForecastModel& model,
  ...
  if(time_major) {
    ...
    int batch_offset = 0;
    for(batch_offset = 0;
        batch_offset < max_rows_in - samples_per_batch;
        batch_offset += samples_per_batch) {
      std::vector<matrix_eig> data_batch_eig, target_batch_eig;
      ModelUtil::GetBatch(model, mat, batch_offset, bsz,
                          data_batch_eig, target_batch_eig, time_major);
      data.push_back(data_batch_eig);
      target.push_back(target_batch_eig);
    }
    ...
    if(rem_bsz > 0) {
      std::vector<matrix_eig> data_batch_eig, target_batch_eig;
      ModelUtil::GetBatch(model, mat, batch_offset, rem_bsz,
                          data_batch_eig, target_batch_eig, time_major);
      data.push_back(data_batch_eig);
      target.push_back(target_batch_eig);
    }
...
void ModelUtil::GetBatches(const BaseForecastModel &model,
  ...
  int batch_offset = 0;
  for(batch_offset = 0;
      batch_offset < mat.rows() - samples_per_batch;
      batch_offset += samples_per_batch) {
...
void ModelUtil::GenerateFeatureMatrix(const BaseForecastModel &model,
  ...
  for (size_t idx = 0; idx <= timesteps - model.GetBPTT();
       idx++) {
