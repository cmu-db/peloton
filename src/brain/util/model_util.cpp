...
void ModelUtil::GetBatch(const BaseForecastModel *model, const matrix_eig &mat,
  ...
  size_t seq_len = std::min<size_t>(
      bptt, samples_per_input - model->GetHorizon() - batch_offset);
  ...
  for (size_t input_idx = 0; input_idx < bsz; input_idx++) {
    ...
    matrix_eig target_batch =
        mat.block(row_idx + model->GetSegment(), 0, seq_len, mat.cols());
...
void ModelUtil::GenerateFeatureMatrix(const BaseForecastModel *model,
  ...
  for (size_t idx = 0; idx < timesteps - model->GetHorizon() - regress_dim + 1;
       idx++) {
