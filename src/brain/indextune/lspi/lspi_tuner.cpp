...
void LSPIIndexTuner::Tune(const std::vector<std::string>& queries,
  ...
  for(size_t i = 0; i < num_queries; i++) {
    ...
    ConstructQueryConfigFeature(add_candidates[i], drop_candidates[i], query_config_feat);
    rlse_model_->Update(query_config_feat, query_latencies[i]);
  }
  // Step 3: Iterate through the queries - Per query obtain optimal add/drop candidates
  // Step 4:
}

void LSPIIndexTuner::ConstructQueryConfigFeature(std::unique_ptr<boost::dynamic_bitset<>>& add_candidates,
...
void LSPIIndexTuner::ConstructConfigFeature(peloton::vector_eig &config_vec) const {
