...
void LSPIIndexTuner::Tune(
  ...
  const boost::dynamic_bitset<> &curr_config_set = *index_config_->GetCurrentIndexConfig();
  ...
  for (size_t i = 0; i < num_queries; i++) {
    CompressedIndexConfigUtil::AddCandidates(*index_config_, queries[i], add_candidate_set);
    add_candidate_sets.push_back(std::move(add_candidate_set));
    CompressedIndexConfigUtil::DropCandidates(*index_config_, queries[i], drop_candidate_set);
  ...
  for (size_t i = 0; i < num_queries; i++) {
    ...
    CompressedIndexConfigUtil::ConstructQueryConfigFeature(curr_config_set,
        add_candidate_sets[i], drop_candidate_sets[i], query_config_feat);
    rlse_model_->Update(query_config_feat, query_latencies[i]);
  }
  // Step 3: Iterate through the queries/latencies and obtain a new optimal config
...
void LSPIIndexTuner::FindOptimalConfig(double max_cost,
  ...
  while (index_id_rec != boost::dynamic_bitset<>::npos) {
    if(!optimal_config_set.test(index_id_rec)) {
      ...
      CompressedIndexConfigUtil::ConstructQueryConfigFeature(
          hypothetical_config, add_candidate_set,
          drop_candidate_set, query_config_vec);
