...
TimeSeriesLinearReg::TimeSeriesLinearReg(int regress_dim, int horizon, int segment)
    : BaseForecastModel(horizon, segment),
      regress_dim_(regress_dim) {}
...
**/
TimeSeriesKernelReg::TimeSeriesKernelReg(int regress_dim, int horizon, int segment)
    : BaseForecastModel(horizon, segment),
      regress_dim_(regress_dim) {}
...
float TimeSeriesKernelReg::TrainEpoch(matrix_eig &data) {
  ...
  kernel_x_ = X;
  kernel_y_ = y;
  matrix_eig kernel = (-EigenUtil::PairwiseEuclideanDist(X, kernel_x_)).array().exp();
