...
namespace peloton {
  namespace planner {
    ...
    AnalyzePlan::AnalyzePlan(std::string table_name, concurrency::Transaction *consistentTxn) :
        table_name_(table_name) {
      target_table_ = catalog::Catalog::GetInstance()->GetTableWithName(
          DEFAULT_DB_NAME, table_name, consistentTxn);
    }

    AnalyzePlan::AnalyzePlan(std::string table_name, std::vector<char*> column_names, concurrency::Transaction *consistentTxn) :
        table_name_(table_name),
    ...
    AnalyzePlan::AnalyzePlan(parser::AnalyzeStatement *analyze_stmt, concurrency::Transaction *consistentTxn) {
