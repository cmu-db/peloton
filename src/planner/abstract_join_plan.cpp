...
void AbstractJoinPlan::PerformBinding(BindingContext &context) {
  ...
  HandleSubplanBinding(/*is_left*/ false, inputs);
  HandleSubplanBinding(/*is_left*/ true, inputs);
  ...
  for (oid_t left_input_col : left_inputs) {
    left_attributes_.push_back(inputs[0]->Find(left_input_col));
  }
  const auto &right_inputs = input_cols[1];
  for (oid_t right_input_col : right_inputs) {
    right_attributes_.push_back(inputs[1]->Find(right_input_col));
  }
