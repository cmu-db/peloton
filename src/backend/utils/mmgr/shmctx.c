 ...
 *
 * IDENTIFICATION
 *	  $Header: /db/cvsroot/postgres/src/backend/utils/mmgr/shmctx.c,v 1.13.2.1 2003/07/16 02:05:29 sailesh Exp $
 *
 *-------------------------------------------------------------------------
 */
...
SHMContextInit(void)
{
	...
	elog(WARNING, "Init TopSharedmemorycontext : %d", getpid());

	if (mm_query_segment)
		return;
	pid = getpid();
	sprintf(mmquerysegname, "/tmp/shm_query_segment_%d", pid);

	mm_query_segment = mm_create(100 * 1024 * 1024, mmquerysegname);
	if (!mm_query_segment)
	{
	...
	elog(WARNING, "Finished initializing TopSharedmemorycontext");
}
...
SHMContextStats(MemoryContext context)
{
	...
	(*context->methods->stats) (context, 0);
	for (child = context->firstchild; child != NULL; child = child->nextchild)
