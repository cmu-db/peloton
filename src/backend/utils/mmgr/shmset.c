 ...
 *
 * IDENTIFICATION
 *	  $Header: /db/cvsroot/postgres/src/backend/utils/mmgr/shmset.c,v 1.11.2.1 2003/07/16 02:05:29 sailesh Exp $
 *
 * by: owenc@eecs.berkeley.edu
 *
 * this is a shared memory context for postgres based on aset.c and a shared
 * memory management library.
 *	For all services such as
 * shared memory allocation, and locking this allocator relies on the open
 * source mm shared memory management library.	This library abstracts the
 * details of shared memory allocation and locking, and may choose to
 * implement shared memory using a number of methods such as IPC,mmap,or files
 * In this way the library provides a  set of services that is highly portable
 * across a number of platforms.
 *
 * for more information on the mm library which was developed for use by
 * apache please see http
 */
