...
typename Cache<Key, Value>::iterator Cache<Key, Value>::insert(
    const Entry &entry) {
  ALWAYS_ASSERT(list_.size() == map_.size());
  ALWAYS_ASSERT(list_.size() <= this->capacity_);
  ...
  if (map_itr == map_.end()) {
    ...
    ALWAYS_ASSERT(ret.second); /* should not fail */
  ...
  ALWAYS_ASSERT(list_.size() == map_.size());
  ALWAYS_ASSERT(list_.size() <= capacity_);
  return cache_itr;
}
...
typename Cache<Key, Value>::size_type Cache<Key, Value>::size() const {
  ALWAYS_ASSERT(map_.size() == list_.size());
  return map_.size();
}
