...
template <>
inline Value Value::CallUnary<FUNC_OCTET_LENGTH>() const {
...
template <>
inline Value Value::CallUnary<FUNC_CHAR_LENGTH>() const {
  if (IsNull()) return GetNullValue();

  char *valueChars = reinterpret_cast<char *>(GetObjectValueWithoutNull());
  return GetBigIntValue(static_cast<int64_t>(
      GetCharLength(valueChars, GetObjectLengthWithoutNull())));
}

/** implement the 1-argument SQL SPACE function */
template <>
inline Value Value::CallUnary<FUNC_SPACE>() const {
  if (IsNull()) return GetNullStringValue();
...
template <>
inline Value Value::CallUnary<FUNC_FOLD_LOWER>() const {
  if (IsNull()) return GetNullStringValue();
...
template <>
inline Value Value::CallUnary<FUNC_FOLD_UPPER>() const {
  if (IsNull()) return GetNullStringValue();
...
inline Value Value::Call<FUNC_REPEAT>(const std::vector<Value> &arguments) {
  ...
  if ((count * valueUTF8Length) > POOLED_MAX_VALUE_LENGTH) {
    ...
    snprintf(msg, sizeof(msg),
             "REPEAT function Call would create a string of size %d which.Is "
             "larger than the maximum size %d",
             count * valueUTF8Length, POOLED_MAX_VALUE_LENGTH);
    throw Exception(msg);
  }
  char *repeatChars =
      reinterpret_cast<char *>(strValue.GetObjectValueWithoutNull());

  std::string repeatStr;
  while (count-- > 0) repeatStr.append(repeatChars, valueUTF8Length);
...
inline Value Value::Call<FUNC_POSITION_CHAR>(
  ...
  char *tarGetChars =
      reinterpret_cast<char *>(tarGet.GetObjectValueWithoutNull());
...
inline Value Value::Call<FUNC_LEFT>(const std::vector<Value> &arguments) {
  ...
  return GetTempStringValue(
      valueChars, (int32_t)(Value::GetIthCharPosition(
                                valueChars, valueUTF8Length, count + 1) -
                            valueChars));
}
...
inline Value Value::Call<FUNC_RIGHT>(const std::vector<Value> &arguments) {
  ...
  const char *newStartChar = Value::GetIthCharPosition(
      valueChars, valueUTF8Length, charLen - count + 1);
...
inline Value Value::Call<FUNC_CONCAT>(const std::vector<Value> &arguments) {
  ...
  for (std::vector<Value>::const_iterator iter = arguments.begin();
    ...
    if (size > (int64_t)INT32_MAX) {
      throw Exception("The result of CONCAT function.Is out of range");
    }
  }
...
inline Value Value::Call<FUNC_VOLT_SUBSTRING_CHAR_FROM>(
  ...
  int64_t start =
      std::max(startArg.CastAsBigIntAndGetValue(), static_cast<int64_t>(1L));
...
static inline std::string trim_function(std::string source,
                                        const std::string match, bool doltrim,
                                        bool dortrim) {
...
inline Value Value::trimWithOptions(const std::vector<Value> &arguments,
  ...
  // our implementation also allows multiple characters, but rejects 0
  // characters.
  if (length == 0) {
    throw Exception("data exception -- trim error, invalid length argument 0");
  }
...
template <>
inline Value Value::Call<FUNC_TRIM_BOTH_CHAR>(
    const std::vector<Value> &arguments) {
  return trimWithOptions(arguments, true, true);
}

template <>
inline Value Value::Call<FUNC_TRIM_LEADING_CHAR>(
    const std::vector<Value> &arguments) {
  return trimWithOptions(arguments, true, false);
}

template <>
inline Value Value::Call<FUNC_TRIM_TRAILING_CHAR>(
    const std::vector<Value> &arguments) {
...
inline Value Value::Call<FUNC_SUBSTRING_CHAR>(
  ...
  if (start < 1) {
    // According to the standard, START < 1 effectively moves the end point
    // based on (LENGTH + START)
...
static inline std::string overlay_function(const char *ptrSource,
                                           size_t lengthSource,
                                           std::string insertStr, size_t start,
                                           size_t length) {
  ...
  int32_t j = i;
  if (length > 0) {
    // the end the last character may be multiple byte character, Get to the
...
inline Value Value::Call<FUNC_OVERLAY_CHAR>(
  ...
  const char *ptrSource =
      reinterpret_cast<const char *>(str0.GetObjectValueWithoutNull());
  ...
  const char *ptrInsert =
      reinterpret_cast<const char *>(str1.GetObjectValueWithoutNull());
...
inline Value Value::Call<FUNC_VOLT_FORMAT_CURRENCY>(
  ...
  if (places >= 12 || places <= -26) {
    throw Exception("the second parameter should be < 12 and > -26");
  }
  ...
  // TODO: Although there should be only one copy of newloc (and
  // money_numpunct),
  // we still need to test and make sure no memory leakage in this piece of
  // code.
