...
static inline void micros_to_date(int64_t epoch_micros_in,
  ...
  boost::posix_time::ptime input_ptime =
      EPOCH + boost::posix_time::microseconds(epoch_micros_in);
  date_out = input_ptime.date();
}

/** Convert from epoch_micros to time **/
static inline void micros_to_time(int64_t epoch_micros_in,
...
static inline void micros_to_date_and_time(
    int64_t epoch_micros_in, boost::gregorian::date &date_out,
    boost::posix_time::time_duration &time_out) {
  if (epoch_micros_in < GREGORIAN_EPOCH) {
    throw peloton::Exception(
        "Value out of range. Cannot convert dates prior to the year 1583");
...
static inline int64_t epoch_microseconds_from_components(
    unsigned short int year, unsigned short int month = 1,
  ...
  boost::posix_time::time_duration goal_time =
      boost::posix_time::time_duration(hour, minute, second);
  boost::posix_time::ptime goal_ptime =
      boost::posix_time::ptime(goal_date, goal_time);
  ...
  int64_t epoch_seconds =
      goal_duration.ticks() / goal_duration.ticks_per_second();
...
template <>
inline Value Value::CallUnary<FUNC_EXTRACT_YEAR>() const {
  if (IsNull()) {
    return *this;
  }
...
template <>
inline Value Value::CallUnary<FUNC_EXTRACT_MONTH>() const {
  if (IsNull()) {
    return *this;
  }
  int64_t epoch_micros = GetTimestamp();
...
template <>
inline Value Value::CallUnary<FUNC_EXTRACT_DAY>() const {
  if (IsNull()) {
    return *this;
  }
  int64_t epoch_micros = GetTimestamp();
...
template <>
inline Value Value::CallUnary<FUNC_EXTRACT_DAY_OF_WEEK>() const {
  if (IsNull()) {
    return *this;
  }
  int64_t epoch_micros = GetTimestamp();
  boost::gregorian::date as_date;
  micros_to_date(epoch_micros, as_date);
  return GetTinyIntValue(
      (int8_t)(as_date.day_of_week() + 1));  // Have 0-based, want 1-based.
}
...
template <>
inline Value Value::CallUnary<FUNC_EXTRACT_WEEKDAY>() const {
  if (IsNull()) {
    return *this;
  }
  int64_t epoch_micros = GetTimestamp();
...
template <>
inline Value Value::CallUnary<FUNC_EXTRACT_WEEK_OF_YEAR>() const {
  if (IsNull()) {
    return *this;
  }
  int64_t epoch_micros = GetTimestamp();
...
template <>
inline Value Value::CallUnary<FUNC_EXTRACT_DAY_OF_YEAR>() const {
  if (IsNull()) {
    return *this;
  }
  int64_t epoch_micros = GetTimestamp();
...
template <>
inline Value Value::CallUnary<FUNC_EXTRACT_QUARTER>() const {
  if (IsNull()) {
    return *this;
  }
  int64_t epoch_micros = GetTimestamp();
...
template <>
inline Value Value::CallUnary<FUNC_EXTRACT_HOUR>() const {
  if (IsNull()) {
    return *this;
  }
  int64_t epoch_micros = GetTimestamp();
...
template <>
inline Value Value::CallUnary<FUNC_EXTRACT_MINUTE>() const {
  if (IsNull()) {
    return *this;
  }
  int64_t epoch_micros = GetTimestamp();
...
template <>
inline Value Value::CallUnary<FUNC_EXTRACT_SECOND>() const {
  if (IsNull()) {
    return *this;
  }
  int64_t epoch_micros = GetTimestamp();
...
template <>
inline Value Value::CallUnary<FUNC_SINCE_EPOCH_SECOND>() const {
  if (IsNull()) {
    return *this;
  }
  int64_t epoch_micros = GetTimestamp();
...
template <>
inline Value Value::CallUnary<FUNC_SINCE_EPOCH_MILLISECOND>() const {
  if (IsNull()) {
    return *this;
  }
  int64_t epoch_micros = GetTimestamp();
...
template <>
inline Value Value::CallUnary<FUNC_SINCE_EPOCH_MICROSECOND>() const {
  if (IsNull()) {
    return *this;
  }
  int64_t epoch_micros = GetTimestamp();
...
template <>
inline Value Value::CallUnary<FUNC_TO_TIMESTAMP_SECOND>() const {
  if (IsNull()) {
    return *this;
  }
...
template <>
inline Value Value::CallUnary<FUNC_TO_TIMESTAMP_MILLISECOND>() const {
  if (IsNull()) {
    return *this;
  }
...
template <>
inline Value Value::CallUnary<FUNC_TO_TIMESTAMP_MICROSECOND>() const {
  if (IsNull()) {
    return *this;
  }
...
template <>
inline Value Value::CallUnary<FUNC_TRUNCATE_YEAR>() const {
  if (IsNull()) {
    return *this;
  }
  int64_t epoch_micros = GetTimestamp();
  ...
  int64_t truncate_epoch_micros =
      epoch_microseconds_from_components(as_date.year());
  return GetTimestampValue(truncate_epoch_micros);
}

/** implement the timestamp TRUNCATE to QUARTER function **/
template <>
inline Value Value::CallUnary<FUNC_TRUNCATE_QUARTER>() const {
  if (IsNull()) {
    return *this;
  }
  int64_t epoch_micros = GetTimestamp();
  ...
  int64_t truncate_epoch_micros =
      epoch_microseconds_from_components(as_date.year(), quarter_start_month);
  return GetTimestampValue(truncate_epoch_micros);
}

/** implement the timestamp TRUNCATE to MONTH function **/
template <>
inline Value Value::CallUnary<FUNC_TRUNCATE_MONTH>() const {
  if (IsNull()) {
    return *this;
  }
  int64_t epoch_micros = GetTimestamp();
  ...
  micros_to_date(epoch_micros, as_date);
  int64_t truncate_epoch_micros =
      epoch_microseconds_from_components(as_date.year(), as_date.month());
  return GetTimestampValue(truncate_epoch_micros);
}

/** implement the timestamp TRUNCATE to DAY function **/
template <>
inline Value Value::CallUnary<FUNC_TRUNCATE_DAY>() const {
  if (IsNull()) {
    return *this;
  }
  int64_t epoch_micros = GetTimestamp();
  boost::gregorian::date as_date;
  micros_to_date(epoch_micros, as_date);
  int64_t truncate_epoch_micros = epoch_microseconds_from_components(
...
template <>
inline Value Value::CallUnary<FUNC_TRUNCATE_HOUR>() const {
  if (IsNull()) {
    return *this;
  }
  int64_t epoch_micros = GetTimestamp();
  ...
  int64_t truncate_epoch_micros = epoch_microseconds_from_components(
      as_date.year(), as_date.month(), as_date.day(), as_time.hours());
  return GetTimestampValue(truncate_epoch_micros);
}

/** implement the timestamp TRUNCATE to MINUTE function **/
template <>
inline Value Value::CallUnary<FUNC_TRUNCATE_MINUTE>() const {
  if (IsNull()) {
    return *this;
  }
  int64_t epoch_micros = GetTimestamp();
...
template <>
inline Value Value::CallUnary<FUNC_TRUNCATE_SECOND>() const {
  if (IsNull()) {
    return *this;
  }
  int64_t epoch_micros = GetTimestamp();
  ...
  int64_t truncate_epoch_micros = epoch_microseconds_from_components(
      as_date.year(), as_date.month(), as_date.day(), as_time.hours(),
      as_time.minutes(), as_time.seconds());
  return GetTimestampValue(truncate_epoch_micros);
}

/** implement the timestamp TRUNCATE to MILLIS function **/
template <>
inline Value Value::CallUnary<FUNC_TRUNCATE_MILLISECOND>() const {
  if (IsNull()) {
    return *this;
  }
  int64_t epoch_micros = GetTimestamp();
...
template <>
inline Value Value::CallUnary<FUNC_TRUNCATE_MICROSECOND>() const {
  if (IsNull()) {
    return *this;
  }
  int64_t epoch_micros = GetTimestamp();
...
template <>
inline Value Value::CallConstant<FUNC_CURRENT_TIMESTAMP>() {
  // TODO: Fix this
  // ExecutorContext * context = voltdb::ExecutorContext::GetExecutorContext();
  // int64_t currentTimeMil.Is = context->currentUniqueId() >> (COUNTER_BITS +
  // PARTITIONID_BITS);
