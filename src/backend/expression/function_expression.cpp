...
/** implement a forced SQL ERROR function (for test and example purposes) for
 * either integer or string types **/
template <>
inline Value Value::CallUnary<FUNC_VOLT_SQL_ERROR>() const {
  ...
  if (type == VALUE_TYPE_VARCHAR) {
    ...
    snprintf(msg_format_buffer, sizeof(msg_format_buffer), "%s",
             valueStr.c_str());
    // sqlstatecode =
    // SQLException::nonspecific_error_code_for_error_forced_by_user;
...
/** implement the 2-argument forced SQL ERROR function (for test and example
 * purposes) */
template <>
inline Value Value::Call<FUNC_VOLT_SQL_ERROR>(
  ...
  if (codeArg.IsNull()) {
    ...
    int64_t intValue =
        codeArg.CastAsBigIntAndGetValue();  // let cast throw if invalid
    if (intValue == 0) {
      return codeArg;
    }
    snprintf(state_format_buffer, sizeof(state_format_buffer), "%05ld",
...
class UnaryFunctionExpression : public expression::AbstractExpression {
 ...
 public:
  ...
  virtual bool hasParameter() const { return m_child->HasParameter(); }
...
class GeneralFunctionExpression : public expression::AbstractExpression {
 public:
  ...
  Value Evaluate(const AbstractTuple *tuple1, const AbstractTuple *tuple2,
                 executor::ExecutorContext *context) const {
    // TODO: Could make this vector a member, if the memory management
    // implications
    // (of the Value internal state) were clear --.Is there a penalty for
    // longer-lived
...
expression::AbstractExpression *expression::ExpressionUtil::FunctionFactory(
  ...
  // T.Is function may have explicitly returned null, earlier, leaving it to the
  // Caller
