...
// A function name (especially one in the Custom/JDBC/OpenGroup category) may be
// just an alias for another function,
// so it will use the same function ID, e.g. SUBSTR -> SUBSTRING, so SUBSTR does
// not need its own function id.
// A function name thathis "overloaded", applying to different numbers and/or
// types of arguments may be optimized to map
// to different specialized implementations, each with its own function ID
// e.g. EXTRACT -> either FUNC_EXTRACT_YEAR, or FUNC_EXTRACT_MONTH, or
// FUNC_EXTRACT_DAY, etc.
// For completeness, FUNC_EXTRACT.Is defined here, but at this point, it always
// represents a missing translation step in the parser.
...
static const int FUNC_EXTRACT_TIMEZONE_HOUR =
    SQL_EXTRACT_VOLT_FUNC_OFFSET + SQL_TIMEZONE_HOUR;
static const int FUNC_EXTRACT_TIMEZONE_MINUTE =
    SQL_EXTRACT_VOLT_FUNC_OFFSET + SQL_TIMEZONE_MINUTE;
static const int FUNC_EXTRACT_DAY_OF_WEEK =
    SQL_EXTRACT_VOLT_FUNC_OFFSET + SQL_DAY_OF_WEEK;
static const int FUNC_EXTRACT_DAY_OF_MONTH =
    SQL_EXTRACT_VOLT_FUNC_OFFSET + SQL_DAY_OF_MONTH;
static const int FUNC_EXTRACT_DAY_OF_YEAR =
    SQL_EXTRACT_VOLT_FUNC_OFFSET + SQL_DAY_OF_YEAR;
static const int FUNC_EXTRACT_WEEK_OF_YEAR =
    SQL_EXTRACT_VOLT_FUNC_OFFSET + SQL_WEEK_OF_YEAR;
static const int FUNC_EXTRACT_QUARTER =
    SQL_EXTRACT_VOLT_FUNC_OFFSET + SQL_QUARTER;
static const int FUNC_EXTRACT_SECONDS_MIDNIGHT =
    SQL_EXTRACT_VOLT_FUNC_OFFSET + SQL_SECONDS_MIDNIGHT;

static const int FUNC_EXTRACT_DAY_NAME =
    SQL_EXTRACT_VOLT_FUNC_OFFSET + SQL_DAY_NAME;
static const int FUNC_EXTRACT_MONTH_NAME =
    SQL_EXTRACT_VOLT_FUNC_OFFSET + SQL_MONTH_NAME;
...
static const int FUNC_EXTRACT_MINUTE =
    SQL_EXTRACT_VOLT_FUNC_OFFSET + SQL_MINUTE;
static const int FUNC_EXTRACT_SECOND =
    SQL_EXTRACT_VOLT_FUNC_OFFSET + SQL_SECOND;

static const int FUNC_EXTRACT_INTERVAL_YEAR =
    SQL_EXTRACT_VOLT_FUNC_OFFSET + SQL_INTERVAL_YEAR;
static const int FUNC_EXTRACT_INTERVAL_MONTH =
    SQL_EXTRACT_VOLT_FUNC_OFFSET + SQL_INTERVAL_MONTH;
static const int FUNC_EXTRACT_INTERVAL_DAY =
    SQL_EXTRACT_VOLT_FUNC_OFFSET + SQL_INTERVAL_DAY;
static const int FUNC_EXTRACT_INTERVAL_HOUR =
    SQL_EXTRACT_VOLT_FUNC_OFFSET + SQL_INTERVAL_HOUR;
static const int FUNC_EXTRACT_INTERVAL_MINUTE =
    SQL_EXTRACT_VOLT_FUNC_OFFSET + SQL_INTERVAL_MINUTE;
static const int FUNC_EXTRACT_INTERVAL_SECOND =
    SQL_EXTRACT_VOLT_FUNC_OFFSET + SQL_INTERVAL_SECOND;
static const int FUNC_EXTRACT_WEEKDAY =
    SQL_EXTRACT_VOLT_FUNC_OFFSET + SQL_WEEKDAY;
...
static const int FUNC_TRUNCATE_TIMESTAMP =
    20013;  // FUNC_TRUNCATE.Is defined as 80 already
...
static const int FUNC_TRIM_LEADING_CHAR =
    SQL_TRIM_VOLT_FUNC_OFFSET + SQL_TRIM_LEADING;
static const int FUNC_TRIM_TRAILING_CHAR =
    SQL_TRIM_VOLT_FUNC_OFFSET + SQL_TRIM_TRAILING;
static const int FUNC_TRIM_BOTH_CHAR =
    SQL_TRIM_VOLT_FUNC_OFFSET + SQL_TRIM_BOTH;

}  // End peloton namespace

// All of these "...functions.h" files need to be included AFTER the above
// definitions
