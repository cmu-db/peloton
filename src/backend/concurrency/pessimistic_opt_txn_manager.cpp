...
//                         Peloton
//
// pessimistic_opt_txn_manager.cpp
//
// Identification: src/backend/concurrency/pessimistic_opt_txn_manager.cpp
//
// Copyright (c) 2015-16, Carnegie Mellon University Database Group
//
//===----------------------------------------------------------------------===//

#include "pessimistic_opt_txn_manager.h"
...
void PessimisticOptTxnManager::ReleaseReadLock(
...
PessimisticOptTxnManager &PessimisticOptTxnManager::GetInstance() {
  static PessimisticOptTxnManager txn_manager;
  return txn_manager;
}
...
VisibilityType PessimisticOptTxnManager::IsVisible(
    const storage::TileGroupHeader *const tile_group_header,
    const oid_t &tuple_id) {
...
bool PessimisticOptTxnManager::IsOwner(
    const storage::TileGroupHeader *const tile_group_header,
    const oid_t &tuple_id) {
...
// transaction.
// this function is called by update/delete executors.
bool PessimisticOptTxnManager::IsOwnable(
    const storage::TileGroupHeader *const tile_group_header,
    const oid_t &tuple_id) {
  auto tuple_txn_id = tile_group_header->GetTransactionId(tuple_id);
...
bool PessimisticOptTxnManager::AcquireOwnership(
    const storage::TileGroupHeader *const tile_group_header,
  ...
  // if a transaction $T$ invokes this function, then the tuple's end commit id should be max_cid.
  // however, on the time the transaction $T$ reads the tuple,
  // it is possible that some other transactions are trying to update this version
  // by inserting a new version.
  // The transaction $T$ thinks that it reads an older version but eventually this version returns to be the latest version.

  // so we check whether we have inserted this tuple into our rw_set. 
  ...
  if (tuple_map != rw_set.end()) {
    if (tuple_map->second.find(tuple_id) != tuple_map->second.end()) {
      // It was already accessed, so release read lock.
      ReleaseReadLock(tile_group_header, tuple_id);

      // Mark the tuple as released
      pessimistic_opt_released_rdlock[tile_group_id].insert(tuple_id);
    }
  }
...
void PessimisticOptTxnManager::YieldOwnership(const oid_t &tile_group_id,
  const oid_t &tuple_id) {
...
bool PessimisticOptTxnManager::PerformRead(const ItemPointer &location) {
  ...
  if (tile_group_header->GetEndCommitId(tuple_id) == MAX_CID) {
    // Try to acquire read lock.
    // No one is holding the write lock.
    GetSpinlockField(tile_group_header, tuple_id)->Lock();
    
    // FIXME: why need this fence???
...
bool PessimisticOptTxnManager::PerformInsert(const ItemPointer &location) {
  oid_t tile_group_id = location.block;
  oid_t tuple_id = location.offset;
...
void PessimisticOptTxnManager::PerformUpdate(const ItemPointer &old_location,
...
void PessimisticOptTxnManager::PerformUpdate(const ItemPointer &location) {
  oid_t tile_group_id = location.block;
  oid_t tuple_id = location.offset;
...
void PessimisticOptTxnManager::PerformDelete(const ItemPointer &old_location,
                                          const ItemPointer &new_location) {
...
void PessimisticOptTxnManager::PerformDelete(const ItemPointer &location) {
...
Result PessimisticOptTxnManager::CommitTransaction() {
    ...
    for (auto &tile_group_entry : rw_set) {
      ...
      for (auto &tuple_entry : tile_group_entry.second) {
        ...
        if (tuple_entry.second == RW_TYPE_READ) {
          // Release read locks
          if (pessimistic_opt_released_rdlock.find(tile_group_id) ==
                  pessimistic_opt_released_rdlock.end() ||
              pessimistic_opt_released_rdlock[tile_group_id].find(tuple_slot) ==
                  pessimistic_opt_released_rdlock[tile_group_id].end()) {
...
Result PessimisticOptTxnManager::AbortTransaction() {
