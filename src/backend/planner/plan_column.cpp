...
int PlanColumn::GetGuid() const {
  return m_guid;
}

std::string PlanColumn::GetColumnName() const {
  return m_name;
}

ValueType PlanColumn::GetType() const {
  return m_type;
}

size_t PlanColumn::GetSize() const {
  return m_size;
}

std::string PlanColumn::GetInputColumnName() const {
...
expression::AbstractExpression* PlanColumn::GetExpression() {
  ...
  json_spirit::Value column_expression_value = find_value(m_col_object, "EXPRESSION");
  if (column_expression_value == json_spirit::Value::null) {
    throw PlannerException("PlanColumn::getExpression: Can't find EXPRESSION value");
  }

  json_spirit::Object column_expression_object = column_expression_value.get_obj();
  expression = expression::AbstractExpression::CreateExpressionTree(column_expression_object);
