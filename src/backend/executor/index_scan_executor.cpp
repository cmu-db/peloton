...
bool IndexScanExecutor::DInit() {
  ...
  if (runtime_keys_.size() != 0) {
    ALWAYS_ASSERT(runtime_keys_.size() == values_.size());
...
bool IndexScanExecutor::ExecPrimaryIndexLookup() {
  ...
  ALWAYS_ASSERT(index_->GetIndexType() == INDEX_CONSTRAINT_TYPE_PRIMARY_KEY);
  ...
  for (auto tuple_location_ptr : tuple_location_ptrs) {
    ...
    while (true) {
      ...
      // if the tuple is not visible.
        ...
        ALWAYS_ASSERT(tuple_location.IsNull() == false);
        ...
        if (old_end_cid <= max_committed_cid) {
          ALWAYS_ASSERT(tile_group_header->GetTransactionId(old_item.offset) == INITIAL_TXN_ID || tile_group_header->GetTransactionId(old_item.offset) == INVALID_TXN_ID);
