...
#include "backend/benchmark/tpcc/loader.h"
...
// Tuple id counter
oid_t tpcc_tuple_counter = -1000000;

oid_t tpcc_max_scan_length = 1000;

oid_t tpcc_bulk_insert_count = 1000;
...
expression::AbstractExpression *CreateScanPredicate(const int bound) {
  ...
  Value upper_bound = ValueFactory::GetIntegerValue(bound + tpcc_max_scan_length);
...
static int GetBound() {
  ...
  auto bound = rand() % (tuple_count - tpcc_max_scan_length);
