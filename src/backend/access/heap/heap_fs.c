...
static HTSU_Result fs_heap_lock_updated_tuple(Relation rel, HeapTuple tuple,
...
initscan(HeapScanDesc scan, ScanKey key, bool is_rescan)
{
	...
	else if (allow_sync && synchronize_seqscans)
	{
		...
		scan->rs_startblock = fs_ss_get_location(scan->rs_rd, scan->rs_nblocks);
	}
	else
	{
...
void
fs_heap_setscanlimits(HeapScanDesc scan, BlockNumber startBlk, BlockNumber numBlks)
{
...
/*
 * fs_heapgetpage - subroutine for fs_heapgettup()
 *
 ...
 */
static void
fs_heapgetpage(HeapScanDesc scan, BlockNumber page)
{
	Buffer		buffer;
	Snapshot	snapshot;
	Page		dp;
	 ...
	 */
	fs_heap_page_prune_opt(scan->rs_rd, buffer);
 ...
 */
static void
fs_heapgettup(HeapScanDesc scan,
		   ScanDirection dir,
		   int nkeys,
		   ScanKey key)
{
 ...
 */
static void
fs_heapgettup_pagemode(HeapScanDesc scan,
					ScanDirection dir,
					int nkeys,
					ScanKey key)
{
 ...
 */
Relation
fs_relation_open(Oid relationId, LOCKMODE lockmode)
{
 ...
 */
Relation
fs_try_relation_open(Oid relationId, LOCKMODE lockmode)
{
	elog(ERROR, "%s %d %s : function not implemented", __FILE__, __LINE__, __func__);
	return NULL;
}
 ...
 */
Relation
fs_relation_openrv(const RangeVar *relation, LOCKMODE lockmode)
{
	elog(ERROR, "%s %d %s : function not implemented", __FILE__, __LINE__, __func__);
	return NULL;
}
 ...
 */
Relation
fs_relation_openrv_extended(const RangeVar *relation, LOCKMODE lockmode,
						 bool missing_ok)
{
	elog(ERROR, "%s %d %s : function not implemented", __FILE__, __LINE__, __func__);
	return NULL;
 ...
 */
void
fs_relation_close(Relation relation, LOCKMODE lockmode)
{
	elog(ERROR, "%s %d %s : function not implemented", __FILE__, __LINE__, __func__);
}
 ...
 */
Relation
fs_heap_open(Oid relationId, LOCKMODE lockmode)
{
	elog(ERROR, "%s %d %s : function not implemented", __FILE__, __LINE__, __func__);
	return NULL;
}
 ...
 */
Relation
fs_heap_openrv(const RangeVar *relation, LOCKMODE lockmode)
{
	elog(ERROR, "%s %d %s : function not implemented", __FILE__, __LINE__, __func__);
	return NULL;
}
 ...
 */
Relation
fs_heap_openrv_extended(const RangeVar *relation, LOCKMODE lockmode,
					 bool missing_ok)
{
	elog(ERROR, "%s %d %s : function not implemented", __FILE__, __LINE__, __func__);
	return NULL;
...
HeapScanDesc
fs_heap_beginscan_catalog(Relation relation, int nkeys, ScanKey key)
{
...
HeapScanDesc
fs_heap_beginscan_strat(Relation relation, Snapshot snapshot,
					 int nkeys, ScanKey key,
 ...
 */
void
fs_heap_rescan(HeapScanDesc scan,
			ScanKey key)
{
	/*
	 * unpin scan buffers
 ...
 */
void
fs_heap_endscan(HeapScanDesc scan)
{
...
#ifdef HEAPDEBUGALL
#define HEAPDEBUG_1 \
	elog(DEBUG2, "fs_heap_getnext([%s,nkeys=%d],dir=%d) called", \
		 RelationGetRelationName(scan->rs_rd), scan->rs_nkeys, (int) direction)
#define HEAPDEBUG_2 \
	elog(DEBUG2, "fs_heap_getnext returning EOS")
#define HEAPDEBUG_3 \
	elog(DEBUG2, "fs_heap_getnext returning tuple")
#else
#define HEAPDEBUG_1
#define HEAPDEBUG_2
#define HEAPDEBUG_3
...
HeapTuple
fs_heap_getnext(HeapScanDesc scan, ScanDirection direction)
{
	/* Note: no locking manipulations needed */
	...
	if (scan->rs_pageatatime)
		fs_heapgettup_pagemode(scan, direction,
							scan->rs_nkeys, scan->rs_key);
	else
		fs_heapgettup(scan, direction, scan->rs_nkeys, scan->rs_key);
 ...
 */
bool
fs_heap_fetch(Relation relation,
		   Snapshot snapshot,
		   HeapTuple tuple,
		   Buffer *userbuf,
		   bool keep_buf,
 ...
 */
bool
fs_heap_hot_search_buffer(ItemPointer tid, Relation relation, Buffer buffer,
 ...
 */
bool
fs_heap_hot_search(ItemPointer tid, Relation relation, Snapshot snapshot,
				bool *all_dead)
{
	bool		result;
	Buffer		buffer;
	...
	buffer = ReadBuffer(relation, ItemPointerGetBlockNumber(tid));
	LockBuffer(buffer, BUFFER_LOCK_SHARE);
	result = fs_heap_hot_search_buffer(tid, relation, buffer, snapshot,
 ...
 */
void
fs_heap_get_latest_tid(Relation relation,
					Snapshot snapshot,
					ItemPointer tid)
{
	BlockNumber blk;
 ...
 */
void
fs_FreeBulkInsertState(BulkInsertState bistate)
{
 ...
 */
Oid
fs_heap_insert(Relation relation, HeapTuple tup, CommandId cid,
 ...
 */
void
fs_heap_multi_insert(Relation relation, HeapTuple *tuples, int ntuples,
 ...
 */
Oid
fs_simple_heap_insert(Relation relation, HeapTuple tup)
{
	return fs_heap_insert(relation, tup, GetCurrentCommandId(true), 0, NULL);
}
 ...
 */
HTSU_Result
fs_heap_delete(Relation relation, ItemPointer tid,
 ...
 */
void
fs_simple_heap_delete(Relation relation, ItemPointer tid)
{
	HTSU_Result result;
	HeapUpdateFailureData hufd;

	result = fs_heap_delete(relation, tid,
 ...
 */
HTSU_Result
fs_heap_update(Relation relation, ItemPointer otid, HeapTuple newtup,
			CommandId cid, Snapshot crosscheck, bool wait,
 ...
 */
static bool
fs_heap_tuple_attr_equals(TupleDesc tupdesc, int attrnum,
							 ...
							 HeapTuple oldtup, HeapTuple newtup)
{
	...
	for (;;)
	{
		...
		changed = !fs_heap_tuple_attr_equals(RelationGetDescr(relation),
 ...
 */
void
fs_simple_heap_update(Relation relation, ItemPointer otid, HeapTuple tup)
{
	HTSU_Result result;
	HeapUpdateFailureData hufd;
	LockTupleMode lockmode;

	result = fs_heap_update(relation, otid, tup,
						 GetCurrentCommandId(true), InvalidSnapshot,
						 true /* wait for commit */ ,
						 &hufd, &lockmode);
	switch (result)
 ...
 */
HTSU_Result
fs_heap_lock_tuple(Relation relation, HeapTuple tuple,
		...
		if (mode == LockTupleKeyShare)
		{
			...
			if (!(infomask2 & HEAP_KEYS_UPDATED))
			{
				...
				if (follow_updates && updated)
				{
					...
					res = fs_heap_lock_updated_tuple(relation, tuple, &t_ctid,
 ...
 */
static HTSU_Result
fs_heap_lock_updated_tuple_rec(Relation rel, ItemPointer tid, TransactionId xid,
							LockTupleMode mode)
{
	...
	for (;;)
	{
		...
		if (!fs_heap_fetch(rel, SnapshotAny, &mytup, &buf, false, NULL))
		{
			/*
 ...
 */
static HTSU_Result
fs_heap_lock_updated_tuple(Relation rel, HeapTuple tuple, ItemPointer ctid,
						TransactionId xid, LockTupleMode mode)
{
	if (!ItemPointerEquals(&tuple->t_self, ctid))
	{
		...
		return fs_heap_lock_updated_tuple_rec(rel, ctid, xid, mode);
	}
 ...
 */
void
fs_heap_inplace_update(Relation relation, HeapTuple tuple)
{
	Buffer		buffer;
	Page		page;
	OffsetNumber offnum;
 ...
 */
bool
fs_heap_freeze_tuple(HeapTupleHeader tuple, TransactionId cutoff_xid,
				  TransactionId cutoff_multi)
{
	xl_heap_freeze_tuple frz;
	bool		do_freeze;
 ...
 */
bool
fs_heap_tuple_needs_freeze(HeapTupleHeader tuple, TransactionId cutoff_xid,
