...
/**
 * @brief Transform a PG ProjectionInfo structure to a Peloton ProjectInfo object.
 *
...
const planner::ProjectInfo* PlanTransformer::BuildProjectInfo(const ProjectionInfo *pg_pi, oid_t column_count) {
  ...
  if (pg_pi->pi_numSimpleVars > 0)
  {
    ...
    if (pg_pi->pi_directMap) // Sequential direct map
    {
      ...
      {
        oid_t   tuple_idx = (varSlotOffsets[i] == offsetof(ExprContext, ecxt_innertuple) ? 1 : 0 );
        ...
        if(!(isnull[i])){   // Non null, direct map
          direct_map_list
          .emplace_back(out_col_id,
                        planner::ProjectInfo::DirectMap::second_type(tuple_idx, in_col_id));
        }
