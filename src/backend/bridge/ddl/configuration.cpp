...
ConfigManager::BuildConfigMap() {
  ...
  while (ConfigureNamesBool[count].gen.name != NULL) {
    ...
    auto val =
        ::GetConfigOption(ConfigureNamesBool[count].gen.name, false, false);
  ...
  while (ConfigureNamesInt[count].gen.name != NULL) {
    ...
    auto val =
        ::GetConfigOption(ConfigureNamesInt[count].gen.name, false, false);
    temp.value = std::string(val);
  ...
  while (ConfigureNamesReal[count].gen.name != NULL) {
    temp.type = REAL_TYPE;
    auto val =
        ::GetConfigOption(ConfigureNamesReal[count].gen.name, false, false);
    temp.value = std::string(val);
  ...
  while (ConfigureNamesString[count].gen.name != NULL) {
    ...
    auto val =
        ::GetConfigOption(ConfigureNamesString[count].gen.name, false, false);
    if (val == NULL)
      temp.value = "NULL";
    else
      temp.value = std::string(val);
  ...
  while (ConfigureNamesEnum[count].gen.name != NULL) {
    temp.type = ENUM_TYPE;
    auto val =
        ::GetConfigOption(ConfigureNamesEnum[count].gen.name, false, false);
    temp.value = std::string(val);
...
void ConfigManager::SetConfigOption(std::string option_name,
                                    std::string option_value) {
  ::SetConfigOption(option_name.c_str(), option_value.c_str(), PGC_USERSET,
                    PGC_S_USER);
}
