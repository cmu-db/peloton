...
private:
  ...
  static void
  GetRawForeignKeys(std::vector<raw_foreignkey_info *> &raw_foreignkeys);

  static raw_table_info *
  GetRawTable(oid_t table_oid, std::string table_name,
              std::vector<raw_column_info *> raw_columns);

  static raw_index_info *
  GetRawIndex(oid_t index_oid, std::string index_name,
              std::vector<raw_column_info *> raw_columns);

  static std::vector<raw_column_info *>
  GetRawColumn(Oid tuple_oid, char relation_kind, Relation pg_attribute_rel);
  ...
  CreateColumns(raw_column_info **raw_columns, oid_t column_count);
