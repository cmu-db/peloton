 ...
 *
 * /n-store/src/stats/stats_source.h
 *
 *-------------------------------------------------------------------------
 */
...
public:
	 ...
	 * An updated stat is requested from the derived class by calling updateStatsTuple
	 *
	...
	friend std::ostream& operator<< (std::ostream& os, const StatsSource& stats_source);
...
protected:
	 ...
	 * to obtain columns contributed by ancestors and then append the columns they will be
	 ...
	 * Necessary because classes in the inheritance hierarchy can vary the number of columns
