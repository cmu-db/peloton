...
void bt_putid(unsigned char *dest, uid id) {
  ...
  while (i--) dest[i] = (unsigned char)id, id >>= 8;
}
...
uid bt_getid(unsigned char *src) {
  ...
  for (i = 0; i < BtId; i++) id <<= 8, id |= *src++;

  return id;
}

uid bt_newdup(BtDb *bt) {
  return __sync_fetch_and_add((unsigned long long *)bt->mgr->pagezero->dups,
                              1) +
         1;
}
...
BTERR bt_readpage(BtMgr *mgr, BtPage page, uid page_no) {
  if (pread(mgr->idx, page, mgr->page_size, page_no << mgr->page_bits) <
      mgr->page_size) {
...
BtPage bt_mappage(BtDb *bt, BtLatchSet *latch) {
  BtPage page =
      (BtPage)(((uid)latch->entry << bt->mgr->page_bits) + bt->mgr->pagepool);
...
BtLatchSet *bt_pinlatch(BtDb *bt, uid page_no, uint loadit) {
  ...
  if (slot < bt->mgr->latchtotal) {
    latch = bt->mgr->latchsets + slot;
    if (bt_latchlink(bt, hashidx, slot, page_no, loadit)) return NULL;
    bt_spinreleasewrite(bt->mgr->hashtable[hashidx].latch);
    return latch;
  }
  ...
  while (1) {
    ...
    if (!bt_spinwritetry(bt->mgr->hashtable[idx].latch)) continue;
    ...
    if (latch->next) bt->mgr->latchsets[latch->next].prev = latch->prev;
...
BtMgr *bt_mgr(char *name, uint bits, uint nodemax) {
  ...
  *amt = 0;
  ...
  mgr->nlatchpage = (nodemax / 16 * sizeof(BtHashEntry) + mgr->page_size - 1) /
                    mgr->page_size;
  mgr->latchhash =
      ((uid)mgr->nlatchpage << mgr->page_bits) / sizeof(BtHashEntry);

  mgr->nlatchpage += nodemax;  // size of the buffer pool in pages
  mgr->nlatchpage +=
      (sizeof(BtLatchSet) * nodemax + mgr->page_size - 1) / mgr->page_size;
  ...
  for (lvl = MIN_lvl; lvl--;) {
    slotptr(pagezero->alloc, 1)->off =
        mgr->page_size - 3 - (lvl ? BtId + sizeof(BtVal) : sizeof(BtVal));
...
mgrlatch:
  ...
  mgr->pagezero =
      (BtPageZero *)mmap(0, mgr->page_size, flag, MAP_ANONYMOUS | MAP_PRIVATE,
                         mgr->idx, ALLOC_page << mgr->page_bits);
  ...
  mgr->hashtable =
      (BtHashEntry *)mmap(0, (uid)mgr->nlatchpage << mgr->page_bits, flag,
  ...
  mgr->latchsets =
      (BtLatchSet *)(mgr->pagepool - (uid)mgr->latchtotal * sizeof(BtLatchSet));
...
void bt_lockpage(BtDb *bt __attribute__((unused)), BtLock mode,
                 BtLatchSet *latch) {
  switch (mode) {
    case BtLockRead:
...
void bt_unlockpage(__attribute__((unused)) BtDb *bt, BtLock mode,
                   BtLatchSet *latch) {
  switch (mode) {
    case BtLockRead:
      latch->readwr->Unlock();
...
int bt_loadpage(BtDb *bt, BtPageSet *set, char *key, uint lvl, BtLock lock) {
  ...
  do {
    ...
    if (!(set->latch = bt_pinlatch(bt, page_no, 1))) return 0;

    // obtain access lock using lock chaining with Access mode

    if (page_no > ROOT_page) bt_lockpage(bt, BtLockAccess, set->latch);
    ...
    if (set->page->free) return bt->err = BTERR_struct, 0;

    if (page_no > ROOT_page) bt_unlockpage(bt, BtLockAccess, set->latch);

    // re-read and re-lock root after determining actual level of root

    if (set->page->lvl != drill) {
      if (set->latch->page_no != ROOT_page) return bt->err = BTERR_struct, 0;
...
BTERR bt_fixfence(BtDb *bt, BtPageSet *set, uint lvl, uint unique) {
  ...
  if (bt_deletekey(bt, ptr->key, lvl + 1, unique)) return (BTERR)bt->err;
...
BTERR bt_deletepage(BtDb *bt, BtPageSet *set, uint unique) {
  ...
  if (right->page->kill) return (BTERR)(bt->err = BTERR_struct);
...
BTERR bt_deletekey(BtDb *bt, char *key, uint lvl, uint unique) {
  ...
  while ((found = !keycmp(ptr, key, bt->key_schema))) {
    ...
    if (unique || slot++ == set->page->cnt) break;
  ...
  if (!set->page->act) return bt_deletepage(bt, set, unique);
...
BtKey *bt_foundkey(BtDb *bt) { return (BtKey *)(bt->key); }
...
int bt_findkey(BtDb *bt, char *key, uint keylen, unsigned char *value,
  ...
  if ((slot = bt_loadpage(bt, set, key, 0, BtLockRead))) do {
      ptr = keyptr(set->page, slot);
...
uint bt_cleanpage(BtDb *bt, BtPageSet *set, uint keylen, uint slot,
  ...
  if (page->min >= (max + 2) * sizeof(BtSlot) + sizeof(*page) + keylen +
                       sizeof(BtKey) + vallen + sizeof(BtVal))
    return slot;
  ...
  while (cnt++ < max) {
    ...
    if (cnt < max || bt->frame->lvl)
      if (slotptr(bt->frame, cnt)->dead) continue;
  ...
  if (page->min >= (idx + 2) * sizeof(BtSlot) + sizeof(*page) + keylen +
                       sizeof(BtKey) + vallen + sizeof(BtVal))
    return newslot;
...
BTERR bt_splitroot(BtDb *bt, BtPageSet *root, BtLatchSet *right) {
  ...
  if (bt_newpage(bt, left, root->page)) return (BTERR)bt->err;
...
uint bt_splitpage(BtDb *bt, BtPageSet *set) {
  ...
  while (cnt++ < max) {
    if (cnt < max || set->page->lvl)
      if (slotptr(set->page, cnt)->dead) continue;
  ...
  if (bt_newpage(bt, right, bt->frame)) return 0;
  ...
  if (slotptr(bt->frame, max)->type == Librarian) max--;
...
BTERR bt_splitkeys(BtDb *bt, BtPageSet *set, BtLatchSet *right, uint unique) {
  ...
  if (set->latch->page_no == ROOT_page) return bt_splitroot(bt, set, right);
...
BTERR bt_insertslot(BtDb *bt, BtPageSet *set, uint slot, char *key, uint keylen,
                    unsigned char *value, uint vallen, uint type,
                    uint release) {
  uint idx, librarian;
  BtSlot *node;
  BtKey *ptr;
  ...
  if (slot > 1)
    if (slotptr(set->page, slot - 1)->type == Librarian) slot--;
...
BTERR bt_insertkey(BtDb *bt, char *key, uint keylen, uint lvl, void *value,
  ...
  while (1) {  // find the page and slot for the current key
    ...
    if (slotptr(set->page, slot)->type == Librarian)
      if (!keycmp(ptr, key, bt->key_schema)) ptr = keyptr(set->page, ++slot);
    ...
    if ((unique && (len != ins->len || compare)) || (!unique)) {
      ...
      return bt_insertslot(bt, set, slot, ins->key, ins->len,
                           (unsigned char *)value, vallen, type, 1);
    }
...
uint bt_atomicpage(BtDb *bt, BtPage source, AtomicTxn *locks, uint src,
  ...
  do {
    ...
    if ((slot = bt_findslot(set->page, key->key, bt->key_schema))) {
      if (slotptr(set->page, slot)->type == Librarian) slot++;
...
BTERR bt_atomicinsert(BtDb *bt, BtPage source, AtomicTxn *locks, uint src) {
  ...
  while ((slot = bt_atomicpage(bt, source, locks, src, set))) {
    if ((slot = bt_cleanpage(bt, set, key->len, slot, val->len)))
      return bt_insertslot(bt, set, slot, key->key, key->len, val->value,
                           val->len, slotptr(source, src)->type, 0);
...
int bt_atomictxn(BtDb *bt, BtPage source, uint unique) {
  ...
  for (src = 0; src++ < source->cnt;) {
    ...
    if ((samepage = src > 1)) {
      if ((samepage = !bt_getid(set->page->right) ||
                      keycmp(keyptr(set->page, set->page->cnt), key->key,
                             bt->key_schema) >= 0))
    ...
    if (!slot) {
      if ((slot = bt_loadpage(bt, set, key->key, 0,
                              (BtLock)(BtLockRead | BtLockAtomic))))
        set->latch->split = 0;
      else
        return -1;
    }
  ...
  if (source->cnt) bt_unlockpage(bt, BtLockRead, set->latch);
  ...
  for (src = source->cnt; src; src--) {
    ...
    for (idx = src; idx < samepage; idx++) switch (slotptr(source, idx)->type) {
        case Delete:
          if (bt_atomicdelete(bt, source, locks, idx)) return -1;
          break;

        case Duplicate:
        case Unique:
          if (bt_atomicinsert(bt, source, locks, idx)) return -1;
          break;
      }
    ...
    if (bt_deletekey(bt, ptr->key, 1, unique)) return -1;
  ...
  if ((leaf = head)) do {
      ...
      switch (leaf->type) {
        ...
        case 2:  // free page
          if (bt_atomicfree(bt, set, unique)) return -1;

          break;
      }

      if (!leaf->nounlock) bt_unlockpage(bt, BtLockParent, set->latch);
...
goleft:
  if (!(next = bt_getid(bt->cursor->left))) return 0;
...
findourself:
  ...
  if (bt->cursor->kill) goto findourself;
...
BtKey *bt_key(BtDb *bt, uint slot) { return keyptr(bt->cursor, slot); }

BtVal *bt_val(BtDb *bt, uint slot) { return valptr(bt->cursor, slot); }
