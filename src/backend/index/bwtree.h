...
/// Print out debug information to std::cout if BWTREE_DEBUG is defined.
#define BWTREE_PRINT(x)          do { if (debug) (std::cout << x << std::endl); } while(0)

/// Assertion only if BWTREE_DEBUG is defined. This is not used in verify().
#define BWTREE_ASSERT(x)         do { assert(x); } while(0)

#else

/// Print out debug information to std::cout if BWTREE_DEBUG is defined.
#define BWTREE_PRINT(x)          do { } while(0)

/// Assertion only if BWTREE_DEBUG is defined. This is not used in verify().
#define BWTREE_ASSERT(x)         do { } while(0)
...
#ifndef BWTREE_FRIENDS
/// The macro BWTREE_FRIENDS can be used by outside class to access the B+
/// tree internals. This was added for wxBTreeDemo to be able to draw the
/// tree.
#define BWTREE_FRIENDS           friend class BWTree_friend;
#endif
...
struct BWTree_default_set_traits
{
    /// If true, the tree will self verify it's invariants after each insert()
    /// or erase(). The header must have been compiled with BWTREE_DEBUG defined.
    static const bool   selfverify = false;

    /// If true, the tree will print out debug information and a tree dump
    /// during insert() or erase() operation. The header must have been
    /// compiled with BWTREE_DEBUG defined and key_type must be std::ostream
    /// printable.
    static const bool   debug = false;

    /// Number of slots in each leaf of the tree. Estimated so that each node
    /// has a size of about 256 bytes.
    static const int    leafslots = BWTREE_MAX( 8, 256 / (sizeof(_Key)) );

    /// Number of slots in each inner node of the tree. Estimated so that each node
    /// has a size of about 256 bytes.
    static const int    innerslots = BWTREE_MAX( 8, 256 / (sizeof(_Key) + sizeof(void*)) );

    /// As of stx-BWTree-0.9, the code does linear search in find_lower() and
...
struct BWTree_default_map_traits
{
    /// If true, the tree will self verify it's invariants after each insert()
    /// or erase(). The header must have been compiled with BWTREE_DEBUG defined.
    static const bool   selfverify = false;
    ...
    /// Number of slots in each leaf of the tree. Estimated so that each node
    /// has a size of about 256 bytes.
    static const int    leafslots = BWTREE_MAX( 8, 256 / (sizeof(_Key) + sizeof(_Data)) );
 ...
 *
 * This class is specialized into BWTree_set, BWTree_multiset, BWTree_map and
 * BWTree_multimap using default template parameters and facade functions.
 */
template <typename _Key, typename _Data,
          typename _Value = std::pair<_Key, _Data>,
          typename _Compare = std::less<_Key>,
          typename _Traits = BWTree_default_map_traits<_Key, _Data>,
...
public:
    ...
    /// Eighth template parameter: boolean indicator whether the BWTree is used
...
public:
    ...
    typedef BWTree<key_type, data_type, value_type, key_compare,
                  traits, allow_duplicates, allocator_type, used_as_set> BWTree_self;
...
public:
    ...
    /// with BWTREE_DEBUG and the key type must be std::ostream printable.
...
private:
    ...
    struct leaf_node : public node
    {
        ...
        {
            BWTREE_ASSERT(used_as_set == false);
            BWTREE_ASSERT(slot < node::slotuse);
        ...
        {
            BWTREE_ASSERT(used_as_set == true);
            BWTREE_ASSERT(slot < node::slotuse);
            slotkey[slot] = key;
        }
    };
...
private:
    ...
    struct BWTree_pair_to_value<value_type, value_type>
    {
    ...
    /// iterators
    typedef BWTree_pair_to_value<value_type, pair_type> pair_to_value_type;
...
public:
    ...
    public:
        ...
        /// The key type of the BWTree. Returned by key().
        typedef typename BWTree::key_type                key_type;

        /// The data type of the BWTree. Returned by data().
        typedef typename BWTree::data_type               data_type;

        /// The value type of the BWTree. Returned by operator*().
        typedef typename BWTree::value_type              value_type;

        /// The pair type of the BWTree.
        typedef typename BWTree::pair_type               pair_type;
    ...
    private:
        ...
        typename BWTree::leaf_node*      currnode;
        ...
        /// Also friendly to the base BWTree class, because erase_iter() needs
        /// to read the currnode and currslot values directly.
        friend class BWTree<key_type, data_type, value_type, key_compare,
    ...
    public:
        ...
        inline iterator(typename BWTree::leaf_node *l, unsigned short s)
    ...
    private:
        ...
        /// The currently referenced leaf node of the tree
        const typename BWTree::leaf_node*        currnode;
    ...
    public:
        ...
        inline const_iterator(const typename BWTree::leaf_node *l, unsigned short s)
            : currnode(l), currslot(s)
        { }
    ...
    public:
        ...
        inline reverse_iterator(typename BWTree::leaf_node *l, unsigned short s)
            : currnode(l), currslot(s)
        { }
    ...
    public:
        ...
        inline const_reverse_iterator(const typename BWTree::leaf_node *l, unsigned short s)
            : currnode(l), currslot(s)
        { }
...
public:
    ...
    struct tree_stats
    {
        ...
        /// Base B+ tree parameter: The number of key/data slots in each leaf
        static const unsigned short     leafslots = BWTree_self::leafslotmax;

        /// Base B+ tree parameter: The number of key slots in each inner node.
        static const unsigned short     innerslots = BWTree_self::innerslotmax;
...
public:
    ...
    /// comparison function
    explicit inline BWTree(const allocator_type &alloc = allocator_type())
    ...
    /// comparison object
    explicit inline BWTree(const key_compare &kcf,
    ...
    inline BWTree(InputIterator first, InputIterator last,
                 const allocator_type &alloc = allocator_type())
        : m_root(NULL), m_headleaf(NULL), m_tailleaf(NULL), m_allocator(alloc)
    {
        insert(first, last);
    ...
    template <class InputIterator>
    inline BWTree(InputIterator first, InputIterator last, const key_compare &kcf,
                 const allocator_type &alloc = allocator_type())
        : m_root(NULL), m_headleaf(NULL), m_tailleaf(NULL),
          m_key_less(kcf), m_allocator(alloc)
    {
...
public:
    ...
    protected:
        ...
        /// Constructor called from BWTree::value_comp()
        inline value_compare(key_compare kc)
            : key_comp(kc)
        { }

        /// Friendly to the BWTree class so it may call the constructor
        friend class BWTree<key_type, data_type, value_type, key_compare,
                           traits, allow_duplicates, allocator_type, used_as_set>;
...
public:
    ...
    void clear()
    {
        ...
        BWTREE_ASSERT(m_stats.itemcount == 0);
    }
...
private:
    ...
    inline int find_lower(const node_type *n, const key_type& key) const
    {
        if ( 0 && sizeof(n->slotkey) > traits::binsearch_threshold )
        {
            ...
            BWTREE_PRINT("BWTree::find_lower: on " << n << " key " << key << " -> " << lo << " / " << hi);
            ...
            if (selfverify)
            {
                ...
                BWTREE_PRINT("BWTree::find_lower: testfind: " << i);
                BWTREE_ASSERT(i == lo);
            }
    ...
    inline int find_upper(const node_type *n, const key_type& key) const
    {
        if ( 0 && sizeof(n->slotkey) > traits::binsearch_threshold )
        {
            ...
            BWTREE_PRINT("BWTree::find_upper: on " << n << " key " << key << " -> " << lo << " / " << hi);
            ...
            if (selfverify)
            {
                ...
                BWTREE_PRINT("BWTree::find_upper testfind: " << i);
                BWTREE_ASSERT(i == hi);
            }
...
public:
    ...
    inline bool operator==(const BWTree_self &other) const
    {
...
public:
    ...
    inline BWTree_self& operator= (const BWTree_self &other)
    {
        if (this != &other)
        {
            clear();
    ...
    inline BWTree(const BWTree_self &other)
        : m_root(NULL), m_headleaf(NULL), m_tailleaf(NULL),
          m_stats( other.m_stats ),
          m_key_less( other.key_comp() ),
          m_allocator( other.get_allocator() )
    ...
    */
                                             ...
                                             key_type* splitkey, node** splitnode)
    {
        if (!n->isleafnode())
        {
            ...
            BWTREE_PRINT("BWTree::insert_descend into " << inner->childid[slot]);
            ...
            if (newchild)
            {
                BWTREE_PRINT("BWTree::insert_descend newchild with key " << newkey << " node " << newchild << " at slot " << slot);

                if (inner->isfull())
                {
                    split_inner_node(inner, splitkey, splitnode, slot);

                    BWTREE_PRINT("BWTree::insert_descend done split_inner: putslot: " << slot << " putkey: " << newkey << " upkey: " << *splitkey);
                    ...
                    BWTREE_PRINT("BWTree::insert_descend switch: " << slot << " > " << inner->slotuse+1);
                    ...
                    if (slot == inner->slotuse+1 && inner->slotuse < (*splitnode)->slotuse)
                    {
                        ...
                        BWTREE_ASSERT(inner->slotuse + 1 < innerslotmax);
                    ...
                    else if (slot >= inner->slotuse+1)
                    {
                        ...
                        BWTREE_PRINT("BWTree::insert_descend switching to splitted node " << inner << " slot " << slot);
                    }
                }

                // move items and put pointer to child node into correct slot
                BWTREE_ASSERT(slot >= 0 && slot <= inner->slotuse);
        ...
        else // n->isleafnode() == true
        {
            ...
            BWTREE_ASSERT(slot >= 0 && slot <= leaf->slotuse);
    ...
    {
        BWTREE_ASSERT(leaf->isfull());

        unsigned int mid = (leaf->slotuse >> 1);

        BWTREE_PRINT("BWTree::split_leaf_node on " << leaf);
    ...
    {
        BWTREE_ASSERT(inner->isfull());

        unsigned int mid = (inner->slotuse >> 1);

        BWTREE_PRINT("BWTree::split_inner: mid " << mid << " addslot " << addslot);
        ...
        BWTREE_PRINT("BWTree::split_inner_node on " << inner << " into two nodes " << mid << " and " << inner->slotuse - (mid + 1) << " sized");
...
public:
    ...
    void bulk_load(Iterator ibegin, Iterator iend)
    {
        ...
        BWTREE_PRINT("BWTree::bulk_load, level 0: " << m_stats.itemcount << " items into " << num_leaves << " leaves with up to " << ((iend - ibegin + num_leaves-1) / num_leaves) << " items per leaf.");
        ...
        BWTREE_ASSERT( it == iend && num_items == 0 );

        // if the BWTree is so small to fit into one leaf, then we're done.
        if (m_headleaf == m_tailleaf) {
            m_root = m_headleaf;
            return;
        }

        BWTREE_ASSERT( m_stats.leaves == num_leaves );

        // create first level of inner nodes, pointing to the leaves.
        size_t num_parents = (num_leaves + (innerslotmax+1)-1) / (innerslotmax+1);

        BWTREE_PRINT("BWTree::bulk_load, level 1: " << num_leaves << " leaves in " << num_parents << " inner nodes with up to " << ((num_leaves + num_parents-1) / num_parents) << " leaves per inner node.");
        ...
        BWTREE_ASSERT( leaf == NULL && num_leaves == 0 );
        ...
        for (int level = 2; num_parents != 1; ++level)
        {
            ...
            BWTREE_PRINT("BWTree::bulk_load, level " << level << ": " << num_children << " children in " << num_parents << " inner nodes with up to " << ((num_children + num_parents-1) / num_parents) << " children per inner node.");
...
private:
    ...
    struct result_t
    {
        ...
        inline result_t(result_flags_t f = BWTree_ok)
            : flags(f), lastkey()
        { }
        ...
        inline result_t& operator|= (const result_t &other)
        {
            ...
            if (other.has(BWTree_update_lastkey))
...
public:
    ...
    {
        BWTREE_PRINT("BWTree::erase_one(" << key << ") on BWTree size " << size());
        ...
        if (!result.has(BWTree_not_found))
            --m_stats.itemcount;
        ...
        return !result.has(BWTree_not_found);
    }
    ...
    {
        BWTREE_PRINT("BWTree::erase_iter(" << iter.currnode << "," << iter.currslot << ") on BWTree size " << size());
...
private:
                               ...
                               inner_node *parent, unsigned int parentslot)
    {
        if (curr->isleafnode())
        {
            ...
            {
                BWTREE_PRINT("Could not find key " << key << " to erase.");

                return BWTree_not_found;
            }

            BWTREE_PRINT("Found key in leaf " << curr << " at slot " << slot);
            ...
            if (slot == leaf->slotuse)
            {
                ...
                {
                    BWTREE_ASSERT(parent->childid[parentslot] == curr);
                ...
                else
                {
                    if (leaf->slotuse >= 1)
                    {
                        BWTREE_PRINT("Scheduling lastkeyupdate: key " << leaf->slotkey[leaf->slotuse - 1]);
                        myres |= result_t(BWTree_update_lastkey, leaf->slotkey[leaf->slotuse - 1]);
                    }
                    else
                    {
                        BWTREE_ASSERT(leaf == m_root);
                    }
                }
            }
            ...
            if (leaf->isunderflow() && !(leaf == m_root && leaf->slotuse >= 1))
            {
                ...
                if (leftleaf == NULL && rightleaf == NULL)
                {
                    ...
                    BWTREE_ASSERT(m_stats.itemcount == 1);
                    BWTREE_ASSERT(m_stats.leaves == 0);
                    BWTREE_ASSERT(m_stats.innernodes == 0);
        ...
        else // !curr->isleafnode()
        {
            ...
            BWTREE_PRINT("erase_one_descend into " << inner->childid[slot]);
            ...
            if (result.has(BWTree_update_lastkey))
            {
                if (parent && parentslot < parent->slotuse)
                {
                    BWTREE_PRINT("Fixing lastkeyupdate: key " << result.lastkey << " into parent " << parent << " at parentslot " << parentslot);
                ...
                {
                    BWTREE_PRINT("Forwarding lastkeyupdate: key " << result.lastkey);
                    myres |= result_t(BWTree_update_lastkey, result.lastkey);
                }
            }

            if (result.has(BWTree_fixmerge))
            {
                ...
                BWTREE_ASSERT(inner->childid[slot]->slotuse == 0);
            ...
            if (inner->isunderflow() && !(inner == m_root && inner->slotuse >= 1))
            {
                ...
                {
                    BWTREE_ASSERT(inner == m_root);
                    BWTREE_ASSERT(inner->slotuse == 0);
     ...
     * first search for that leaf node in the suBWTree containing all pairs of
     * the same key. This suBWTree can be very large, even the whole tree,
                                ...
                                inner_node *parent, unsigned int parentslot)
    {
        if (curr->isleafnode())
        {
            ...
            {
                BWTREE_PRINT("Could not find iterator (" << iter.currnode << "," << iter.currslot << ") to erase. Invalid leaf node?");
            ...
            BWTREE_PRINT("Found iterator in leaf " << curr << " at slot " << slot);
        ...
        else // !curr->isleafnode()
        {
            ...
            while (slot <= inner->slotuse)
            {
                ...
                BWTREE_PRINT("erase_iter_descend into " << inner->childid[slot]);
    ...
    {
        BWTREE_PRINT("Merge leaf nodes " << left << " and " << right << " with common parent " << parent << ".");
        (void)parent;

        BWTREE_ASSERT(left->isleafnode() && right->isleafnode());
        BWTREE_ASSERT(parent->level == 1);

        BWTREE_ASSERT(left->slotuse + right->slotuse < leafslotmax);
    ...
    {
        BWTREE_PRINT("Merge inner nodes " << left << " and " << right << " with common parent " << parent << ".");

        BWTREE_ASSERT(left->level == right->level);
        BWTREE_ASSERT(parent->level == left->level + 1);

        BWTREE_ASSERT(parent->childid[parentslot] == left);

        BWTREE_ASSERT(left->slotuse + right->slotuse < innerslotmax);
        ...
        if (selfverify)
        {
            ...
            BWTREE_ASSERT(leftslot < parent->slotuse);
            BWTREE_ASSERT(parent->childid[leftslot] == left);
            BWTREE_ASSERT(parent->childid[leftslot+1] == right);

            BWTREE_ASSERT(parentslot == leftslot);
        }
    ...
    static result_t shift_left_leaf(leaf_node *left, leaf_node *right, inner_node *parent, unsigned int parentslot)
    {
        ...
        BWTREE_ASSERT(left->nextleaf == right);
        BWTREE_ASSERT(left == right->prevleaf);

        BWTREE_ASSERT(left->slotuse < right->slotuse);
        BWTREE_ASSERT(parent->childid[parentslot] == left);

        unsigned int shiftnum = (right->slotuse - left->slotuse) >> 1;

        BWTREE_PRINT("Shifting (leaf) " << shiftnum << " entries to left " << left << " from right " << right << " with common parent " << parent << ".");

        BWTREE_ASSERT(left->slotuse + shiftnum < leafslotmax);
        ...
        if (parentslot < parent->slotuse) {
            ...
            return result_t(BWTree_update_lastkey, left->slotkey[left->slotuse - 1]);
        }
    }
    ...
    static void shift_left_inner(inner_node *left, inner_node *right, inner_node *parent, unsigned int parentslot)
    {
        ...
        BWTREE_PRINT("Shifting (inner) " << shiftnum << " entries to left " << left << " from right " << right << " with common parent " << parent << ".");

        BWTREE_ASSERT(left->slotuse + shiftnum < innerslotmax);
        ...
        if (selfverify)
        {
            ...
            BWTREE_ASSERT(leftslot == parentslot);
        }
    ...
    static void shift_right_leaf(leaf_node *left, leaf_node *right, inner_node *parent, unsigned int parentslot)
    {
        ...
        BWTREE_PRINT("Shifting (leaf) " << shiftnum << " entries to right " << right << " from left " << left << " with common parent " << parent << ".");
        ...
        BWTREE_ASSERT(right->slotuse + shiftnum < leafslotmax);
    ...
    static void shift_right_inner(inner_node *left, inner_node *right, inner_node *parent, unsigned int parentslot)
    {
        ...
        BWTREE_ASSERT(right->slotuse + shiftnum < innerslotmax);
...
public:
    ...
    /// function requires that the header is compiled with BWTREE_DEBUG and that
    ...
    void verify_node(const node* n, key_type* minkey, key_type* maxkey, tree_stats &vstats) const
    {
        ...
        else // !n->isleafnode()
        {
            ...
            for(unsigned short slot = 0; slot <= inner->slotuse; ++slot)
            {
                ...
                BWTREE_PRINT("verify subnode " << subnode << ": " << subminkey << " - " << submaxkey);
...
private:
    ...
    struct dump_header
    {
        /// "stx-BWTree", just to stop the restore() function from loading garbage
        char            signature[12];
        ...
        inline void fill()
        {
            ...
            version = 0;
            key_type_size = sizeof(typename BWTree_self::key_type);
            data_type_size = sizeof(typename BWTree_self::data_type);
            leafslots = BWTree_self::leafslotmax;
            innerslots = BWTree_self::innerslotmax;
            allow_duplicates = BWTree_self::allow_duplicates;
        }
...
public:
    ...
    bool restore(std::istream &is)
    {
        ...
        {
            BWTREE_PRINT("BWTree::restore: file header does not match instantiation signature.");
            return false;
        }
...
private:
    ...
    {
        BWTREE_PRINT("dump_node " << n << std::endl);
    ...
    node* restore_node(std::istream &is)
    {
        ...
        if (nu.top.isleafnode())
        {
            ...
            *newleaf = nu.leaf;
            ...
            if (m_headleaf == NULL) {
                BWTREE_ASSERT(newleaf->prevleaf == NULL);
                m_headleaf = m_tailleaf = newleaf;
            }
            else {
                newleaf->prevleaf = m_tailleaf;
