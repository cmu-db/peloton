...
#include "order_by_executor.h"
#include "planner/order_by_node.h"
 ...
 */
OrderByExecutor::OrderByExecutor(planner::AbstractPlanNode *node, Transaction* transaction)
...
bool OrderByExecutor::DExecute(){
  ...
  storage::Tile *ptile = storage::TileFactory::GetTile(INVALID_OID, INVALID_OID, INVALID_OID, INVALID_OID,
                                                        nullptr, backend_, *input_schema_, nullptr, tile_size);
  for(size_t id = 0;id < tile_size;id++){
    ...
    for(oid_t col = 0; col < input_schema_->GetColumnCount(); col++){
      ptile->SetValue(input_tiles_[source_tile_id]->GetValue(source_tuple_id, col),
                      id,
                      col);
    }
  }
  ...
  std::unique_ptr<LogicalTile> ltile(LogicalTileFactory::WrapTiles(singleton, true));
...
bool OrderByExecutor::DoSort(){
  ...
  for(oid_t tile_id = 0; tile_id < input_tiles_.size(); tile_id++){
    for(oid_t tuple_id : *input_tiles_[tile_id]){
      ...
      sort_buffer_.emplace_back(
          sort_buffer_entry_t(ItemPointer(tile_id, tuple_id),
                              std::move(tuple))
                              );
    }
  }
    ...
    bool operator()(const storage::Tuple* ta, const storage::Tuple* tb){
      for(oid_t id = 0; id < descend_flags.size(); id++){
        if(!descend_flags[id]){
          ...
          else if(ta->GetValue(id).OpGreaterThan(tb->GetValue(id)).IsTrue()){
            return false;
          }
        }
        else{
          ...
          else if(tb->GetValue(id).OpGreaterThan(ta->GetValue(id)).IsTrue()){
            return false;
          }
        }
      }
