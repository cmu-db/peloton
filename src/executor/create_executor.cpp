...
bool CreateExecutor::DExecute() {
  ...
  switch (node.GetCreateType()) {
    case CreateType::DB: {
      result = CreateDatabase(node, current_txn);
      break;
    }
    case CreateType::TABLE: {
      result = CreateTable(node, current_txn);
      break;
    }
    case CreateType::INDEX: {
      result = CreateIndex(node, current_txn);
      break;
    }
    case CreateType::TRIGGER: {
      result = CreateTrigger(node, current_txn);
      break;
    }
    default: {
      std::string error =
          StringUtil::Format("Invalid Create type %s",
                             CreateTypeToString(node.GetCreateType()).c_str());
    ...
    if (node.GetForeignKeys().empty() == false) {
      ...
      for (auto fk : node.GetForeignKeys()) {
        ...
        auto catalog_fk = new catalog::ForeignKey(
            sink_table->GetOid(), sink_col_ids, source_col_ids, fk.upd_action,
            fk.del_action, fk.constraint_name);
...
bool CreateExecutor::CreateIndex(const planner::CreatePlan &node,
  ...
  ResultType result = catalog::Catalog::GetInstance()->CreateIndex(
      DEFAULT_DB_NAME,  // TODO: Remove this!
      table_name, key_attrs, index_name, unique_flag, index_type, txn);
  txn->SetResult(result);
  ...
  if (result == ResultType::SUCCESS) {
    ...
    LOG_TRACE("Creating index failed. Result is: %s",
              ResultTypeToString(result).c_str());
    return (false);
  }
}
