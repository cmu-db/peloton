...
DeleteTranslator::DeleteTranslator(const planner::DeletePlan &delete_plan,
                                   ...
                                   Pipeline &pipeline)
    : OperatorTranslator(context, pipeline), delete_plan_(delete_plan), table_(*delete_plan_.GetTable())  {
...
void DeleteTranslator::Consume(ConsumerContext &context, RowBatch::Row &row) const {
  ...
  codegen.CallFunc(_DeleteWrapper::GetFunction(codegen), {tile_group_id, tuple_id, txn, table_ptr});
}

/**
* @brief Delete wrapper.
* @return true on success, false otherwise.
*/
bool DeleteTranslator::wrapper(oid_t tile_group_id, oid_t tuple_id, concurrency::Transaction *txn, storage::DataTable *table) {
  ...
  std::shared_ptr<storage::TileGroup> tile_group = table->GetTileGroup(tile_group_id);
  storage::TileGroupHeader *tile_group_header = tile_group->GetHeader();

  bool is_owner = transaction_manager.IsOwner(txn, tile_group_header, tuple_id);
  bool is_written = transaction_manager.IsWritten(txn, tile_group_header, tuple_id);
  ...
  if (is_owner && is_written) {
    ...
    if (is_ownable) {
      ...
      bool acquire_ownership_success = is_owner ||
                                       transaction_manager.AcquireOwnership(txn, tile_group_header, tuple_id);
...
const std::string& DeleteTranslator::_DeleteWrapper::GetFunctionName() {
  static const std::string deleteWrapperFnName = "_ZN7peloton7codegen16DeleteTranslator7wrapperEjjPNS_11concurrency11TransactionEPNS_7storage9DataTableE";
  return deleteWrapperFnName;
}

llvm::Function* DeleteTranslator::_DeleteWrapper::GetFunction(CodeGen& codegen) {
  ...
  llvm::FunctionType *int32_type = llvm::FunctionType::get(codegen.Int32Type(), false);
  llvm::FunctionType *pointer_type = llvm::FunctionType::get(codegen.CharPtrType(), false);
  std::vector<llvm::Type*> fn_args{int32_type, int32_type, pointer_type, pointer_type};
