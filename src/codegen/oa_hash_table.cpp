...
OAHashTable::OAHashTable(CodeGen &codegen,
  ...
  hash_entry_size_ = sizeof(utils::OAHashTable::HashEntry) +
                     key_storage_.MaxPackedSize() + value_size_;
}
...
OAHashTable::HashTablePos OAHashTable::GetNextEntry(CodeGen &codegen,
  ...
  If wrap_back{codegen, codegen->CreateICmpEQ(next_index, hash_table_size)};
  {
...
std::pair<llvm::Value *, llvm::Value *> OAHashTable::GetDataCountAndPointer(
  ...
  If is_entry_single_value{codegen, IsPtrEqualTo(codegen, kv_p, 1UL),
                           "singleValue"};
  {
...
//===----------------------------------------------------------------------===//
void OAHashTable::TranslateProbing(
    ...
    bool process_only_one_value, bool create_key_if_missing) const {
  ...
  Loop probe_loop{codegen,
                  status_neq_zero,
                  {{"entry", entry_ptr}, {"index", index}, {"kvl", kv_p}}};
  {
    ...
    If hash_match_branch{codegen, is_hash_match, "hashMatch"};
    {
      // Load the key from the HashEntry *
      std::vector<codegen::Value> entry_key{};
      llvm::Value *key_ptr = GetKeyPtr(codegen, entry_ptr);
      llvm::Value *data_ptr = key_storage_.Unpack(codegen, key_ptr, entry_key);

      // Check if the provided key matches what's in the HashEntry
      llvm::Value *is_key_match =
          ValueComparison::TestEquality(codegen, key, entry_key).GetValue();
      If key_match_branch{codegen, is_key_match, "keyMatch"};
      {
        if (process_value) {
  ...
  if (create_key_if_missing) {
    ...
    // First pack keys into the region in hash entry
    // Return value is a pointer to value region
    llvm::Value *dump_data_ptr = key_storage_.Pack(codegen, dump_key_ptr, key);
...
void OAHashTable::Init(CodeGen &codegen, llvm::Value *ht_ptr) const {
  ...
  auto *key_size = codegen.Const64(key_storage_.MaxPackedSize());
...
void OAHashTable::ProbeOrInsert(CodeGen &codegen, llvm::Value *ht_ptr,
  ...
  auto key_not_found = [&codegen, &insert_callback](llvm::Value *data_ptr) {
    insert_callback.Store(codegen, data_ptr);
  };

  TranslateProbing(codegen, ht_ptr, hash, key,
                   key_found,      // Key found then process it and break
                   key_not_found,  // Key not found then create entry
                   true,           // process value
                   true,           // process only one value
...
void OAHashTable::Iterate(CodeGen &codegen, llvm::Value *hash_table,
  ...
  Loop bucket_loop{codegen,
                   bucket_cond,
                   {{"entryIndex", entry_index}, {"entryPtr", entry_ptr}}};
  {
    ...
    If bucket_occupied{codegen, status_neq_zero, "bucketIsOccupied"};
    {
      ...
      Loop read_value_loop{codegen,
                           codegen.ConstBool(true),  // Always pass
                           {{"counter", val_index}, {"dataPtr", data_ptr}}};
      {
...
void OAHashTable::VectorizedIterate(
      ...
      codegen, num_buckets, size, {{"currEntryPtr", entry_ptr}}};
  {
    ...
    // Initial filter loop
    std::vector<Loop::LoopVariable> loop_vars = {{"pos", start},
                                                 {"selPos", codegen.Const32(0)},
                                                 {"currEntryPtr", entry_ptr}};
    Loop filter_loop{codegen, codegen.ConstBool(true), loop_vars};
    {
...
void OAHashTable::FindAll(CodeGen &codegen, llvm::Value *ht_ptr,
  ...
  TranslateProbing(codegen, ht_ptr, nullptr, key,
                   key_found,      // Key found then process it and break
                   key_not_found,  // Key not found then do nothing
                   true,           // process value
                   false,          // process only one value
...
void OAHashTable::OAHashTableAccess::ExtractBucketKeys(
  ...
  hash_table_.key_storage_.Unpack(codegen, key_ptr, key);
}
...
llvm::Value *OAHashTable::OAHashTableAccess::BucketValue(
  ...
  llvm::Value *key_ptr = hash_table_.GetKeyPtr(codegen, entry_ptr);
  uint32_t key_size = hash_table_.key_storage_.MaxPackedSize();
