...
// (3.2.1) Unpack the keys from the hash entry and check raw key equality
...
void CCHashTable::ProbeOrInsert(CodeGen &codegen, llvm::Value *ht_ptr,
  ...
  Loop chain_loop{codegen, end_condition, {{"iter", bucket}}};
  {
    ...
    If hash_match{codegen, codegen->CreateICmpEQ(entry_hash, hash_val),
                  "hashMatch"};
    {
      // (3.2.1) Unpack the keys from the hash entry
      llvm::Value *keys_ptr =
          codegen->CreateConstInBoundsGEP2_32(ht_entry_type, entry, 1, 0);
      std::vector<codegen::Value> hash_entry_keys;
      llvm::Value *values_area =
          key_storage_.Unpack(codegen, keys_ptr, hash_entry_keys);

      // (3.2.1)
      auto keys_are_equal =
          ValueComparison::TestEquality(codegen, key, hash_entry_keys);
  ...
  // (4)
  llvm::Value *value_size = insert_callback.PayloadSize(codegen);
  llvm::Value *keys_size = codegen.Const32(key_storage_.MaxPackedSize());
  ...
  // (5)
  llvm::Value *data_space_ptr = key_storage_.Pack(codegen, ptr, key);
  insert_callback.Store(codegen, data_space_ptr);
...
void CCHashTable::Insert(CodeGen &codegen, llvm::Value *ht_ptr,
  ...
  llvm::Value *ptr =
      codegen.CallFunc(store_func, {ht_ptr, hash_val, needed_bytes});
  // 'ptr' points to the space in the hash table.  Store keys first, calculate
  // where the
...
void CCHashTable::Iterate(CodeGen &codegen, llvm::Value *ht_ptr,
  ...
  Loop bucket_loop{codegen, bucket_cond, {{"bucketNum", bucket_num}}};
  {
                    ...
                    {{"entry", bucket}}};
    {
      ...
      auto *data_area_ptr = key_storage_.Unpack(codegen, entry_data, keys);
  ...
  Loop chain_loop{codegen, end_condition, {{"iter", bucket}}};
  {
                  ...
                  "hashMatch"};
    {
      ...
      llvm::Value *data_area =
          key_storage_.Unpack(codegen, iter_keys, entry_keys);

      auto keys_are_equal =
          ValueComparison::TestEquality(codegen, key, entry_keys);
      If key_match{codegen, keys_are_equal.GetValue(), "keyMatch"};
      {
        // (3.2.2) Call the probe callback since we found a matching key
