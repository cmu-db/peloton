...
// Constructor
HashTranslator::HashTranslator (
    const planner::HashPlan &hash_plan, CompilationContext &context,
    Pipeline &pipeline)
    : OperatorTranslator(context, pipeline),
      hash_plan_(hash_plan) {
  ...
  hash_table_id_ = runtime_state.RegisterState(
      "hash", OAHashTableProxy::GetType(codegen));
...
void HashTranslator::Consume (ConsumerContext &context,
  ...
  // TODO: removed lot's of prefetch code here
...
void HashTranslator::CollectHashKeys (
...
void HashTranslator::ConsumerProbe::ProcessEntry (
    UNUSED_ATTRIBUTE CodeGen &codegen, UNUSED_ATTRIBUTE llvm::Value *data_area) const {
  // The key already exists in the hash table, which means that we can just drop this row,
...
void HashTranslator::ConsumerInsert::StoreValue (
    UNUSED_ATTRIBUTE CodeGen &codegen, UNUSED_ATTRIBUTE llvm::Value *space) const {
  // It is the first time this key appears, so we just pass it on to the next operator
