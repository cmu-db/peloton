...
llvm::Value *RuntimeState::GetStatePtr(CodeGen &codegen,
...
llvm::Value *RuntimeState::GetStateValue(CodeGen &codegen,
                                         RuntimeState::StateID state_id) const {
  auto &state_info = state_slots_[state_id];
  if (state_info.local) {
...
llvm::Type *RuntimeState::ConstructType(CodeGen &codegen) {
