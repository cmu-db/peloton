...
void Sorter::Init(ComparisonFunction func, uint32_t tuple_size) {
  ...
  LOG_INFO("Initialed Sorter with initial size %lu KB for tuples of size %u...",
           kInitialBufferSize / 1024, tuple_size_);
}

// Store a tuple of the given size in this sorter. We return a buffer that
...
void Sorter::Resize() {
  ...
  std::memcpy(new_buffer_start, buffer_start_, buffer_pos_ - buffer_start_);
