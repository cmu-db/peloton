...
std::string TableUtil::GetInfo(DataTable *table) {
  ...
  for (oid_t tile_group_itr = 0; tile_group_itr < tile_group_count;
       tile_group_itr++) {
    if (tile_group_itr > 0) os << std::endl;
    ...
    //    inner << "Tile Group Id  : " << tile_group_itr
    //          << " Tuple Count : " << tile_tuple_count << "\n";
    inner << (*tile_group);  //  << table_id;

    os << StringUtil::Prefix(StringBoxUtil::Box(inner.str()), GETINFO_SPACER);
