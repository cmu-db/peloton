...
void GCManager::StartGC(int thread_id) {
  LOG_TRACE("Starting GC");
  this->is_running_ = true;
  gc_threads_[thread_id].reset(new std::thread(&GCManager::Running, this, thread_id));
}

void GCManager::StopGC(int thread_id) {
...
void GCManager::AddToRecycleMap(std::shared_ptr<GarbageContext> gc_ctx) {
...
void GCManager::Running(int thread_id) {
...
void GCManager::Reclaim(int thread_id, const cid_t &max_cid) {
  int gc_counter = 0;
...
void GCManager::Unlink(int thread_id, const cid_t &max_cid) {
  int tuple_counter = 0;
...
void GCManager::RecycleOldTupleSlot(const oid_t &table_id,
...
void GCManager::ClearGarbage(int thread_id) {
