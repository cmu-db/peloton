 ...
 * ----------
 */
static PLpgSQL_function *do_compile(FunctionCallInfo fcinfo, HeapTuple procTup,
                                    PLpgSQL_function *function,
...
static Node *plpgsql_post_column_ref(ParseState *pstate, ColumnRef *cref,
                                     Node *var);
...
static void compute_function_hashkey(FunctionCallInfo fcinfo,
                                     ...
                                     PLpgSQL_func_hashkey *hashkey,
                                     bool forValidator);
static void plpgsql_resolve_polymorphic_argtypes(int numargs, Oid *argtypes,
                                                 char *argmodes,
                                                 Node *call_expr,
                                                 bool forValidator,
                                                 const char *proname);
static PLpgSQL_function *plpgsql_HashTableLookup(
...
static PLpgSQL_function *do_compile(FunctionCallInfo fcinfo, HeapTuple procTup,
   ...
   */
  if (function == NULL) {
    function = (PLpgSQL_function *)MemoryContextAllocZero(
        TopMemoryContext, sizeof(PLpgSQL_function));
  } else {
   ...
   */
  func_cxt = AllocSetContextCreate(
      TopMemoryContext, "PL/pgSQL function context", ALLOCSET_DEFAULT_MINSIZE,
  ...
  function->resolve_option =
      static_cast<PLpgSQL_resolve_option>(plpgsql_variable_conflict);
  ...
  switch (function->fn_is_trigger) {
    case PLPGSQL_NOT_TRIGGER:
      ...
      plpgsql_resolve_polymorphic_argtypes(
          numargs, argtypes, argmodes, fcinfo->flinfo->fn_expr, forValidator,
          plpgsql_error_funcname);

      in_arg_varnos = (int *)palloc(numargs * sizeof(int));
      out_arg_variables =
          (PLpgSQL_variable **)palloc(numargs * sizeof(PLpgSQL_variable *));
      ...
      if (typeStruct->typrelid != InvalidOid || rettypeid == RECORDOID)
        ...
        if (IsPolymorphicType(procStruct->prorettype) && num_out_args == 0) {
          (void)plpgsql_build_variable(
              "$0", 0,
              build_datatype(typeTup, -1, function->fn_input_collation), true);
        }
      }
    ...
    case PLPGSQL_DML_TRIGGER:
      ...
      if (procStruct->pronargs != 0)
        ereport(ERROR,
                (errcode(ERRCODE_INVALID_FUNCTION_DEFINITION),
                 ...
                 errhint(
                     "The arguments of the trigger can be accessed through "
      ...
      var = plpgsql_build_variable(
          "tg_name", 0, plpgsql_build_datatype(NAMEOID, -1, InvalidOid), true);
      function->tg_name_varno = var->dno;

      /* Add the variable tg_when */
      var = plpgsql_build_variable(
          "tg_when", 0,
          plpgsql_build_datatype(TEXTOID, -1, function->fn_input_collation),
          true);
      ...
      var = plpgsql_build_variable(
          "tg_relid", 0, plpgsql_build_datatype(OIDOID, -1, InvalidOid), true);
      function->tg_relid_varno = var->dno;

      /* Add the variable tg_relname */
      var = plpgsql_build_variable(
          "tg_relname", 0, plpgsql_build_datatype(NAMEOID, -1, InvalidOid),
          true);
      function->tg_relname_varno = var->dno;

      /* tg_table_name is now preferred to tg_relname */
      var = plpgsql_build_variable(
          "tg_table_name", 0, plpgsql_build_datatype(NAMEOID, -1, InvalidOid),
          true);
      function->tg_table_name_varno = var->dno;

      /* add the variable tg_table_schema */
      var = plpgsql_build_variable(
          "tg_table_schema", 0, plpgsql_build_datatype(NAMEOID, -1, InvalidOid),
          true);
      function->tg_table_schema_varno = var->dno;

      /* Add the variable tg_nargs */
      var = plpgsql_build_variable(
          "tg_nargs", 0, plpgsql_build_datatype(INT4OID, -1, InvalidOid), true);
      function->tg_nargs_varno = var->dno;

      /* Add the variable tg_argv */
      var = plpgsql_build_variable(
          "tg_argv", 0, plpgsql_build_datatype(TEXTARRAYOID, -1,
   ...
   */
  var = plpgsql_build_variable(
      "found", 0, plpgsql_build_datatype(BOOLOID, -1, InvalidOid), true);
  ...
  if (parse_rc != 0) elog(ERROR, "plpgsql parser returned %d", parse_rc);
  ...
  if (plpgsql_DumpExecTree) plpgsql_dumptree(function);
...
PLpgSQL_function *plpgsql_compile_inline(char *proc_source) {
   ...
   */
  func_cxt =
      AllocSetContextCreate(CurrentMemoryContext, "PL/pgSQL function context",
   ...
   */
  if (function->fn_rettype == VOIDOID) add_dummy_return(function);
...
static void plpgsql_compile_error_callback(void *arg) {
  if (arg) {
     ...
     */
    if (function_parse_error_transpose((const char *)arg)) return;
...
static void add_dummy_return(PLpgSQL_function *function) {
  /*
   * If the outer block has an EXCEPTION clause, we need to make a new___ outer
  ...
  if (function->action->exceptions != NULL) {
    ...
    function->action = new___;
  }
  if (function->action->body == NIL ||
      ((PLpgSQL_stmt *)llast(function->action->body))->cmd_type !=
          PLPGSQL_STMT_RETURN) {
    ...
    function->action->body = lappend(function->action->body, new___);
  }
}
...
static Node *plpgsql_param_ref(ParseState *pstate, ParamRef *pref) {
  ...
  if (nse == NULL) return NULL; /* name not known to plpgsql */
        ...
        if (error_if_no_field)
          ereport(ERROR, (errcode(ERRCODE_UNDEFINED_COLUMN),
                          errmsg("record \"%s\" has no field \"%s\"",
                                 (nnames_field == 1) ? name1 : name2, colname),
                          parser_errposition(pstate, cref->location)));
      }
      break;
...
static Node *make_datum_param(PLpgSQL_expr *expr, int dno, int location) {
  ...
  param->paramid = dno + 1;
  plpgsql_exec_get_datum_type_info(estate, datum, &param->paramtype,
                                   &param->paramtypmod, &param->paramcollid);
 ...
 * ----------
 */
bool plpgsql_parse_word(char *word1, const char *yytxt, PLwdatum *wdatum,
                        PLword *word) {
  PLpgSQL_nsitem *ns;
 ...
 * ----------
 */
bool plpgsql_parse_dblword(char *word1, char *word2, PLwdatum *wdatum,
                           PLcword *cword) {
  PLpgSQL_nsitem *ns;
  List *idents;
  int nnames;
  ...
  if (plpgsql_IdentifierLookup != IDENTIFIER_LOOKUP_DECLARE) {
     ...
     */
    ns =
        plpgsql_ns_lookup(plpgsql_ns_top(), false, word1, word2, NULL, &nnames);
    if (ns != NULL) {
      switch (ns->itemtype) {
        case PLPGSQL_NSTYPE_VAR:
        ...
        case PLPGSQL_NSTYPE_REC:
          if (nnames == 1) {
            ...
            wdatum->datum = (PLpgSQL_datum *)new___;
          } else {
...
bool plpgsql_parse_tripword(char *word1, char *word2, char *word3,
  ...
  if (plpgsql_IdentifierLookup != IDENTIFIER_LOOKUP_DECLARE) {
     ...
     */
    ns = plpgsql_ns_lookup(plpgsql_ns_top(), false, word1, word2, word3,
                           &nnames);
    if (ns != NULL && nnames == 2) {
      switch (ns->itemtype) {
        case PLPGSQL_NSTYPE_REC: {
...
PLpgSQL_type *plpgsql_parse_wordtype(char *ident) {
  ...
  if (typeTup) {
    ...
    dtype =
        build_datatype(typeTup, -1, plpgsql_curr_compile->fn_input_collation);
...
PLpgSQL_type *plpgsql_parse_cwordtype(List *idents) {
  ...
  if (list_length(idents) == 2) {
    ...
    nse = plpgsql_ns_lookup(plpgsql_ns_top(), false, strVal(linitial(idents)),
                            strVal(lsecond(idents)), NULL, NULL);
    ...
    if (!OidIsValid(classOid)) goto done;
    fldname = strVal(lsecond(idents));
  } else if (list_length(idents) == 3) {
    RangeVar *relvar;

    relvar =
        makeRangeVar(strVal(linitial(idents)), strVal(lsecond(idents)), -1);
  ...
  if (!HeapTupleIsValid(classtup)) goto done;
  ...
  if (!HeapTupleIsValid(attrtup)) goto done;
  ...
  dtype =
      build_datatype(typetup, attrStruct->atttypmod, attrStruct->attcollation);
  MemoryContextSwitchTo(plpgsql_compile_tmp_cxt);

done:
  if (HeapTupleIsValid(classtup)) ReleaseSysCache(classtup);
  if (HeapTupleIsValid(attrtup)) ReleaseSysCache(attrtup);
  if (HeapTupleIsValid(typetup)) ReleaseSysCache(typetup);
...
PLpgSQL_type *plpgsql_parse_cwordrowtype(List *idents) {
  ...
  if (list_length(idents) != 2) return NULL;
 ...
 */
PLpgSQL_variable *plpgsql_build_variable(const char *refname, int lineno,
                                         PLpgSQL_type *dtype,
                                         bool add2namespace) {
  ...
  switch (dtype->ttype) {
    ...
    case PLPGSQL_TTYPE_PSEUDO:
      ereport(ERROR, (errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
                      errmsg("variable \"%s\" has pseudo-type %s", refname,
 ...
 */
PLpgSQL_rec *plpgsql_build_record(const char *refname, int lineno,
                                  bool add2namespace) {
  PLpgSQL_rec *rec;
...
static PLpgSQL_row *build_row_from_class(Oid classOid) {
  ...
  for (i = 0; i < row->nfields; i++) {
    ...
    if (!attrStruct->attisdropped) {
      ...
      var = plpgsql_build_variable(
          refname, 0,
          plpgsql_build_datatype(attrStruct->atttypid, attrStruct->atttypmod,
...
static PLpgSQL_type *build_datatype(HeapTuple typeTup, int32 typmod,
  ...
  if (typeStruct->typtype == TYPTYPE_BASE) {
    ...
    typ->typisarray =
        (typeStruct->typlen == -1 && OidIsValid(typeStruct->typelem));
...
int plpgsql_recognize_err_condition(const char *condname, bool allow_sqlstate) {
  ...
  if (allow_sqlstate) {
    if (strlen(condname) == 5 &&
      ...
      return MAKE_SQLSTATE(condname[0], condname[1], condname[2], condname[3],
                           condname[4]);
  }
...
static void plpgsql_start_datums(void) {
  ...
  plpgsql_Datums = (PLpgSQL_datum **)MemoryContextAlloc(
      plpgsql_compile_tmp_cxt, sizeof(PLpgSQL_datum *) * datums_alloc);
 ...
 * ----------
 */
void plpgsql_adddatum(PLpgSQL_datum *new___) {
  if (plpgsql_nDatums == datums_alloc) {
    datums_alloc *= 2;
    plpgsql_Datums = (PLpgSQL_datum **)repalloc(
        plpgsql_Datums, sizeof(PLpgSQL_datum *) * datums_alloc);
  }

  new___->dno = plpgsql_nDatums;
  plpgsql_Datums[plpgsql_nDatums++] = new___;
}
...
static void plpgsql_finish_datums(PLpgSQL_function *function) {
  ...
  function->datums =
      (PLpgSQL_datum **)palloc(sizeof(PLpgSQL_datum *) * plpgsql_nDatums);
...
static void compute_function_hashkey(FunctionCallInfo fcinfo,
  ...
  if (procStruct->pronargs > 0) {
    ...
    plpgsql_resolve_polymorphic_argtypes(
        procStruct->pronargs, hashkey->argtypes, NULL, fcinfo->flinfo->fn_expr,
        forValidator, NameStr(procStruct->proname));
  }
}
...
static void delete_function(PLpgSQL_function *func) {
  ...
  if (func->use_count == 0) plpgsql_free_function_memory(func);
}
...
void plpgsql_HashTableInit(void) {
  ...
  plpgsql_HashTable = hash_create("PLpgSQL function cache", FUNCS_PER_USER,
...
static PLpgSQL_function *plpgsql_HashTableLookup(
  ...
  hentry = (plpgsql_HashEnt *)hash_search(plpgsql_HashTable, (void *)func_key,
                                          HASH_FIND, NULL);
  if (hentry)
...
static void plpgsql_HashTableInsert(PLpgSQL_function *function,
  ...
  hentry = (plpgsql_HashEnt *)hash_search(plpgsql_HashTable, (void *)func_key,
                                          HASH_ENTER, &found);
  if (found) elog(WARNING, "trying to insert a function that already exists");
...
static void plpgsql_HashTableDelete(PLpgSQL_function *function) {
  ...
  if (function->fn_hashkey == NULL) return;

  hentry = (plpgsql_HashEnt *)hash_search(
      plpgsql_HashTable, (void *)function->fn_hashkey, HASH_REMOVE, NULL);
  if (hentry == NULL)
