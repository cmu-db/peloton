 ...
 * ----------
 */
void plpgsql_ns_init(void) { ns_top = NULL; }

/* ----------
 * plpgsql_ns_push			Create a new namespace level
 * ----------
 */
void plpgsql_ns_push(const char *label, enum PLpgSQL_label_types label_type) {
  if (label == NULL) label = "";
 ...
 * ----------
 */
PLpgSQL_nsitem *plpgsql_ns_top(void) { return ns_top; }
...
void plpgsql_ns_additem(int itemtype, int itemno, const char *name) {
  ...
  nse = (PLpgSQL_nsitem *)palloc(offsetof(PLpgSQL_nsitem, name) + strlen(name) +
                                 1);
 ...
 * ----------
 */
PLpgSQL_nsitem *plpgsql_ns_lookup(PLpgSQL_nsitem *ns_cur, bool localmode,
  ...
  while (ns_cur != NULL) {
    ...
    for (nsitem = ns_cur; nsitem->itemtype != PLPGSQL_NSTYPE_LABEL;
         nsitem = nsitem->prev) {
      if (strcmp(nsitem->name, name1) == 0) {
        if (name2 == NULL || nsitem->itemtype != PLPGSQL_NSTYPE_VAR) {
          if (names_used) *names_used = 1;
          return nsitem;
        }
      }
    }
    ...
    if (localmode) break; /* do not look into upper levels */

    ns_cur = nsitem->prev;
  }

  /* This is just to suppress possibly-uninitialized-variable warnings */
  if (names_used) *names_used = 0;
...
static void free_stmts(List *stmts) {
  ...
  foreach (s, stmts) { free_stmt((PLpgSQL_stmt *)lfirst(s)); }
}
...
static void free_assign(PLpgSQL_stmt_assign *stmt) { free_expr(stmt->expr); }
...
static void free_loop(PLpgSQL_stmt_loop *stmt) { free_stmts(stmt->body); }
...
static void free_open(PLpgSQL_stmt_open *stmt) {
  ...
  foreach (lc, stmt->params) { free_expr((PLpgSQL_expr *)lfirst(lc)); }
}

static void free_fetch(PLpgSQL_stmt_fetch *stmt) { free_expr(stmt->expr); }

static void free_close(PLpgSQL_stmt_close *stmt) {}

static void free_perform(PLpgSQL_stmt_perform *stmt) { free_expr(stmt->expr); }

static void free_exit(PLpgSQL_stmt_exit *stmt) { free_expr(stmt->cond); }

static void free_return(PLpgSQL_stmt_return *stmt) { free_expr(stmt->expr); }
...
void plpgsql_free_function_memory(PLpgSQL_function *func) {
  ...
  if (func->action) free_block(func->action);
  func->action = NULL;
   ...
   */
  if (func->fn_cxt) MemoryContextDelete(func->fn_cxt);
  func->fn_cxt = NULL;
}
...
static void dump_ind(void) {
  ...
  for (i = 0; i < dump_indent; i++) printf(" ");
}
  ...
  if (block->exceptions) {
    ...
    foreach (e, block->exceptions->exc_list) {
      ...
      for (cond = exc->conditions; cond; cond = cond->next) {
        if (cond != exc->conditions) printf(" OR ");
...
static void dump_case(PLpgSQL_stmt_case *stmt) {
  ...
  if (stmt->t_expr) dump_expr(stmt->t_expr);
  printf("\n");
  dump_indent += 6;
  foreach (l, stmt->case_when_list) {
    PLpgSQL_case_when *cwt = (PLpgSQL_case_when *)lfirst(l);
...
static void dump_foreach_a(PLpgSQL_stmt_foreach_a *stmt) {
  ...
  if (stmt->slice != 0) printf("SLICE %d ", stmt->slice);
  printf("IN ");
  dump_expr(stmt->expr);
  printf("\n");
...
static void dump_exit(PLpgSQL_stmt_exit *stmt) {
  ...
  if (stmt->label != NULL) printf(" label='%s'", stmt->label);
  if (stmt->cond != NULL) {
    printf(" WHEN ");
    dump_expr(stmt->cond);
  }
...
static void dump_raise(PLpgSQL_stmt_raise *stmt) {
  ...
  if (stmt->condname) printf(" condname='%s'", stmt->condname);
  if (stmt->message) printf(" message='%s'", stmt->message);
  printf("\n");
  dump_indent += 2;
  foreach (lc, stmt->params) {
    dump_ind();
...
static void dump_getdiag(PLpgSQL_stmt_getdiag *stmt) {
  ...
  foreach (lc, stmt->diag_items) {
    ...
    if (lc != list_head(stmt->diag_items)) printf(", ");
...
static void dump_expr(PLpgSQL_expr *expr) { printf("'%s'", expr->query); }
    ...
    switch (d->dtype) {
      case PLPGSQL_DTYPE_VAR: {
        ...
        printf("VAR %-16s type %s (typoid %u) atttypmod %d\n", var->refname,
               var->datatype->typname, var->datatype->typoid,
      ...
      case PLPGSQL_DTYPE_ROW: {
        ...
        for (i = 0; i < row->nfields; i++) {
          if (row->fieldnames[i])
            printf(" %s=var %d", row->fieldnames[i], row->varnos[i]);
        }
        printf("\n");
      } break;
      case PLPGSQL_DTYPE_REC:
