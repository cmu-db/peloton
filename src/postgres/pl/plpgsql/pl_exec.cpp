...
static void exec_prepare_plan(PLpgSQL_execstate *estate, PLpgSQL_expr *expr,
                              int cursorOptions);
...
static bool exec_eval_simple_expr(PLpgSQL_execstate *estate, PLpgSQL_expr *expr,
                                  Datum *result, bool *isNull, Oid *rettype,
                                  int32 *rettypmod);

static void exec_assign_expr(PLpgSQL_execstate *estate, PLpgSQL_datum *target,
                             PLpgSQL_expr *expr);
static void exec_assign_c_string(PLpgSQL_execstate *estate,
                                 PLpgSQL_datum *target, const char *str);
static void exec_assign_value(PLpgSQL_execstate *estate, PLpgSQL_datum *target,
                              Datum value, bool isNull, Oid valtype,
                              int32 valtypmod);
static void exec_eval_datum(PLpgSQL_execstate *estate, PLpgSQL_datum *datum,
                            Oid *typeid___, int32 *typetypmod, Datum *value,
                            bool *isnull);
static int exec_eval_integer(PLpgSQL_execstate *estate, PLpgSQL_expr *expr,
                             bool *isNull);
static bool exec_eval_boolean(PLpgSQL_execstate *estate, PLpgSQL_expr *expr,
                              bool *isNull);
static Datum exec_eval_expr(PLpgSQL_execstate *estate, PLpgSQL_expr *expr,
                            bool *isNull, Oid *rettype, int32 *rettypmod);
static int exec_run_select(PLpgSQL_execstate *estate, PLpgSQL_expr *expr,
                           long maxtuples, Portal *portalP, bool parallelOK);
...
static void exec_move_row(PLpgSQL_execstate *estate, PLpgSQL_rec *rec,
                          PLpgSQL_row *row, HeapTuple tup, TupleDesc tupdesc);
static HeapTuple make_tuple_from_row(PLpgSQL_execstate *estate,
                                     PLpgSQL_row *row, TupleDesc tupdesc);
...
static void exec_move_row_from_datum(PLpgSQL_execstate *estate,
                                     ...
                                     Datum value);
static char *convert_value_to_string(PLpgSQL_execstate *estate, Datum value,
                                     Oid valtype);
static Datum exec_cast_value(PLpgSQL_execstate *estate, Datum value,
                             bool *isnull, Oid valtype, int32 valtypmod,
...
Datum plpgsql_exec_function(PLpgSQL_function *func, FunctionCallInfo fcinfo,
   ...
   */
  estate.err_text =
      gettext_noop("while storing call arguments into local variables");
  for (i = 0; i < func->fn_nargs; i++) {
    ...
    switch (estate.datums[n]->dtype) {
      case PLPGSQL_DTYPE_VAR: {
        ...
        assign_simple_var(&estate, var, fcinfo->arg[i], fcinfo->argnull[i],
                          false);
        ...
        if (!var->isnull && var->datatype->typisarray) {
          if (VARATT_IS_EXTERNAL_EXPANDED_RW(DatumGetPointer(var->value))) {
            ...
            assign_simple_var(
                &estate, var,
                TransferExpandedObject(var->value, CurrentMemoryContext), false,
                true);
          } else if (VARATT_IS_EXTERNAL_EXPANDED_RO(
            ...
            assign_simple_var(
                &estate, var,
                expand_array(var->value, CurrentMemoryContext, NULL), false,
                true);
          }
        }
      } break;
      ...
      case PLPGSQL_DTYPE_ROW: {
        ...
        if (!fcinfo->argnull[i]) {
          ...
          exec_move_row_from_datum(&estate, NULL, row, fcinfo->arg[i]);
        } else {
    ...
    if (estate.retistuple) {
      ...
      switch (get_call_result_type(fcinfo, NULL, &tupdesc)) {
        case TYPEFUNC_COMPOSITE:
          ...
          if (tupmap) rettup = do_convert_tuple(rettup, tupmap);
      ...
      estate.retval = exec_cast_value(&estate, estate.retval, &fcinfo->isnull,
                                      estate.rettype, -1, func->fn_rettype, -1);
      ...
      if (!fcinfo->isnull && !func->fn_retbyval)
        estate.retval =
            SPI_datumTransfer(estate.retval, false, func->fn_rettyplen);
    }
  }
...
HeapTuple plpgsql_exec_trigger(PLpgSQL_function *func, TriggerData *trigdata) {
  ...
  assign_simple_var(&estate, var,
                    DirectFunctionCall1(
                        namein, CStringGetDatum(trigdata->tg_trigger->tgname)),
                    false, true);
  ...
  if (TRIGGER_FIRED_BEFORE(trigdata->tg_event))
    ...
    elog(ERROR,
         "unrecognized trigger execution time: not BEFORE, AFTER, or INSTEAD "
         "OF");
  ...
  assign_simple_var(&estate, var,
                    ObjectIdGetDatum(trigdata->tg_relation->rd_id), false,
                    false);

  var = (PLpgSQL_var *)(estate.datums[func->tg_relname_varno]);
  assign_simple_var(
      &estate, var,
      DirectFunctionCall1(namein, CStringGetDatum(RelationGetRelationName(
  ...
  assign_simple_var(&estate, var, Int16GetDatum(trigdata->tg_trigger->tgnargs),
                    false, false);
  ...
  if (trigdata->tg_trigger->tgnargs > 0) {
    ...
    assign_simple_var(&estate, var,
                      PointerGetDatum(construct_md_array(
                          elems, NULL, 1, dims, lbs, TEXTOID, -1, false, 'i')),
                      false, true);
  } else {
  ...
  if (estate.retisnull || !TRIGGER_FIRED_FOR_ROW(trigdata->tg_event))
    ...
    tupmap = convert_tuples_by_position(
        estate.rettupdesc, trigdata->tg_relation->rd_att,
        gettext_noop(
...
static void plpgsql_exec_error_callback(void *arg) {
  ...
  if (estate->err_text != NULL) {
    ...
    if (estate->err_stmt != NULL) {
       ...
       */
      errcontext("PL/pgSQL function %s line %d %s", estate->func->fn_signature,
                 estate->err_stmt->lineno, _(estate->err_text));
    } else {
      /*
       * translator: last %s is a phrase such as "while storing call
       * arguments into local variables"
       */
      errcontext("PL/pgSQL function %s %s", estate->func->fn_signature,
                 _(estate->err_text));
    }
  } else if (estate->err_stmt != NULL) {
    /* translator: last %s is a plpgsql statement type name */
    errcontext("PL/pgSQL function %s line %d at %s", estate->func->fn_signature,
...
static int exec_stmt_block(PLpgSQL_execstate *estate,
   ...
   */
  estate->err_text =
      gettext_noop("during statement block local variable initialization");
      ...
      case PLPGSQL_DTYPE_VAR: {
        ...
        if (var->default_val == NULL) {
          ...
          if (var->datatype->typtype == TYPTYPE_DOMAIN)
            exec_assign_value(estate, (PLpgSQL_datum *)var, (Datum)0, true,
                              UNKNOWNOID, -1);
    ...
    PG_CATCH();
    {
      ...
      foreach (e, block->exceptions->exc_list) {
        ...
        if (exception_matches_conditions(edata, exception->conditions)) {
          ...
          state_var =
              (PLpgSQL_var *)estate->datums[block->exceptions->sqlstate_varno];
          errm_var =
              (PLpgSQL_var *)estate->datums[block->exceptions->sqlerrm_varno];
  ...
  switch (rc) {
    ...
    case PLPGSQL_RC_EXIT:
       ...
       */
      if (estate->exitlabel == NULL) return PLPGSQL_RC_EXIT;
      if (block->label == NULL) return PLPGSQL_RC_EXIT;
      if (strcmp(block->label, estate->exitlabel) != 0) return PLPGSQL_RC_EXIT;
  ...
  if (stmt->is_stacked && estate->cur_error == NULL)
    ereport(
        ...
        (errcode(ERRCODE_STACKED_DIAGNOSTICS_ACCESSED_WITHOUT_ACTIVE_HANDLER),
         errmsg(
             "GET STACKED DIAGNOSTICS cannot be used outside an exception "
             "handler")));
  ...
  foreach (lc, stmt->diag_items) {
    ...
    switch (diag_item->kind) {
      case PLPGSQL_GETDIAG_ROW_COUNT:
        exec_assign_value(estate, var, UInt32GetDatum(estate->eval_processed),
                          false, INT8OID, -1);
        break;

      case PLPGSQL_GETDIAG_RESULT_OID:
        exec_assign_value(estate, var, ObjectIdGetDatum(estate->eval_lastoid),
                          false, OIDOID, -1);
        break;
...
static int exec_stmt_if(PLpgSQL_execstate *estate, PLpgSQL_stmt_if *stmt) {
  ...
  exec_eval_cleanup(estate);
  if (!isnull && value) return exec_stmts(estate, stmt->then_body);
  ...
  foreach (lc, stmt->elsif_list) {
    ...
    exec_eval_cleanup(estate);
    if (!isnull && value) return exec_stmts(estate, elif->stmts);
  }
...
static int exec_stmt_case(PLpgSQL_execstate *estate, PLpgSQL_stmt_case *stmt) {
  ...
  if (stmt->t_expr != NULL) {
    ...
    if (t_var->datatype->typoid != t_typoid ||
      ...
      t_var->datatype = plpgsql_build_datatype(
          t_typoid, t_typmod, estate->func->fn_input_collation);

    /* now we can assign to the variable */
    exec_assign_value(estate, (PLpgSQL_datum *)t_var, t_val, isnull, t_typoid,
                      t_typmod);
  ...
  /* We can now discard any value we had for the temp variable */
  if (t_var != NULL) assign_simple_var(estate, t_var, (Datum)0, true, false);

  /* SQL2003 mandates this error if there was no ELSE clause */
  if (!stmt->have_else)
    ereport(ERROR, (errcode(ERRCODE_CASE_NOT_FOUND), errmsg("case not found"),
...
static int exec_stmt_loop(PLpgSQL_execstate *estate, PLpgSQL_stmt_loop *stmt) {
  for (;;) {
    ...
    switch (rc) {
      ...
      case PLPGSQL_RC_EXIT:
        if (estate->exitlabel == NULL) return PLPGSQL_RC_OK;
        if (stmt->label == NULL) return PLPGSQL_RC_EXIT;
        if (strcmp(stmt->label, estate->exitlabel) != 0) return PLPGSQL_RC_EXIT;
        estate->exitlabel = NULL;
        return PLPGSQL_RC_OK;
...
static int exec_stmt_while(PLpgSQL_execstate *estate,
  ...
  for (;;) {
    ...
    switch (rc) {
      ...
      case PLPGSQL_RC_CONTINUE:
        if (estate->exitlabel == NULL) /* anonymous continue, so re-run loop */
          break;
        else if (stmt->label != NULL &&
                 strcmp(stmt->label, estate->exitlabel) == 0)
          /* label matches named continue, so re-run loop */
...
static int exec_stmt_fori(PLpgSQL_execstate *estate, PLpgSQL_stmt_fori *stmt) {
  ...
  value = exec_cast_value(estate, value, &isnull, valtype, valtypmod,
  ...
  for (;;) {
     ...
     */
    if (stmt->reverse) {
      if ((int32)(loop_value - step_value) > loop_value) break;
...
static int exec_stmt_forc(PLpgSQL_execstate *estate, PLpgSQL_stmt_forc *stmt) {
  ...
  if (!curvar->isnull) {
    ...
    if (SPI_cursor_find(curname) != NULL)
      ereport(ERROR, (errcode(ERRCODE_DUPLICATE_CURSOR),
  ...
  if (stmt->argquery != NULL) {
    ...
    set_args.row =
        (PLpgSQL_row *)(estate->datums[curvar->cursor_explicit_argrow]);
   ...
   */
  portal = SPI_cursor_open_with_paramlist(curname, query->plan, paramLI,
  ...
  if (paramLI) pfree(paramLI);

  /*
   * If cursor variable was NULL, store the generated portal name in it
   */
  if (curname == NULL) assign_text_var(estate, curvar, portal->name);
  ...
  if (curname == NULL) assign_simple_var(estate, curvar, (Datum)0, true, false);

  if (curname) pfree(curname);
...
static int exec_stmt_foreach_a(PLpgSQL_execstate *estate,
  ...
  if (loop_var->dtype == PLPGSQL_DTYPE_REC ||
    ...
    loop_var_elem_type =
        get_element_type(plpgsql_exec_get_datum_type(estate, loop_var));
  ...
  while (array_iterate(array_iterator, &value, &isnull)) {
    ...
    exec_assign_value(estate, loop_var, value, isnull, iterator_result_type,
                      iterator_result_typmod);

    /* In slice case, value is temporary; must free it to avoid leakage */
    if (stmt->slice > 0) pfree(DatumGetPointer(value));
...
static int exec_stmt_exit(PLpgSQL_execstate *estate, PLpgSQL_stmt_exit *stmt) {
  ...
  if (stmt->cond != NULL) {
    ...
    value = exec_eval_boolean(estate, stmt->cond, &isnull);
    exec_eval_cleanup(estate);
    if (isnull || value == false) return PLPGSQL_RC_OK;
  }
...
static int exec_stmt_return(PLpgSQL_execstate *estate,
   ...
   */
  if (estate->retisset) return PLPGSQL_RC_RETURN;
    ...
    switch (retvar->dtype) {
      case PLPGSQL_DTYPE_VAR: {
        ...
        if (estate->retistuple && !estate->retisnull)
          ereport(ERROR, (errcode(ERRCODE_DATATYPE_MISMATCH),
                          errmsg(
                              "cannot return non-composite value from function "
      ...
      case PLPGSQL_DTYPE_REC: {
        ...
        if (HeapTupleIsValid(rec->tup)) {
          if (estate->retistuple) {
            ...
            exec_eval_datum(estate, retvar, &estate->rettype, &rettypmod,
                            &estate->retval, &estate->retisnull);
        }
      } break;
  ...
  if (stmt->expr != NULL) {
    ...
    estate->retval = exec_eval_expr(estate, stmt->expr, &(estate->retisnull),
                                    &(estate->rettype), &rettypmod);
...
static int exec_stmt_return_next(PLpgSQL_execstate *estate,
  ...
  if (estate->tuple_store == NULL) exec_init_tuple_store(estate);
  ...
  if (stmt->retvarno >= 0) {
    ...
    switch (retvar->dtype) {
      case PLPGSQL_DTYPE_VAR: {
        ...
        retval =
            MakeExpandedObjectReadOnly(retval, isNull, var->datatype->typlen);

        /* coerce type if needed */
        retval = exec_cast_value(estate, retval, &isNull, var->datatype->typoid,
    ...
    if (estate->retistuple) {
      ...
      if (!isNull) {
        ...
        tuplestore_putvalues(estate->tuple_store, tupdesc, nulldatums,
                             nullflags);
        pfree(nulldatums);
        pfree(nullflags);
      }
      ...
      /* coerce type if needed */
      retval = exec_cast_value(estate, retval, &isNull, rettype, rettypmod,
                               tupdesc->attrs[0]->atttypid,
                               tupdesc->attrs[0]->atttypmod);
  ...
  if (HeapTupleIsValid(tuple)) {
    ...
    if (free_tuple) heap_freetuple(tuple);
  }
...
static int exec_stmt_return_query(PLpgSQL_execstate *estate,
  ...
  if (stmt->query != NULL) {
    ...
    portal = exec_dynquery_with_params(estate, stmt->dynquery, stmt->params,
                                       NULL, CURSOR_OPT_PARALLEL_OK);
  }

  tupmap = convert_tuples_by_position(
      portal->tupDesc, estate->rettupdesc,
  ...
  while (true) {
    ...
    for (i = 0; i < SPI_processed; i++) {
      ...
      if (tupmap) tuple = do_convert_tuple(tuple, tupmap);
      tuplestore_puttuple(estate->tuple_store, tuple);
      if (tupmap) heap_freetuple(tuple);
      processed++;
    }
  ...
  if (tupmap) free_conversion_map(tupmap);
...
static void exec_init_tuple_store(PLpgSQL_execstate *estate) {
   ...
   */
  if (!rsi || !IsA(rsi, ReturnSetInfo) ||
      (rsi->allowedModes & SFRM_Materialize) == 0 || rsi->expectedDesc == NULL)
    ereport(ERROR, (errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
                    errmsg(
                        "set-valued function called in context that cannot "
                        "accept a set")));
  ...
  estate->tuple_store = tuplestore_begin_heap(
...
#define SET_RAISE_OPTION_TEXT(opt, name)                                    \
  do {                                                                      \
    if (opt)                                                                \
      ereport(ERROR, (errcode(ERRCODE_SYNTAX_ERROR),                        \
                      errmsg("RAISE option already specified: %s", name))); \
    opt = pstrdup(extval);                                                  \
  } while (0)
...
static int exec_stmt_raise(PLpgSQL_execstate *estate,
  ...
  if (stmt->condname == NULL && stmt->message == NULL && stmt->options == NIL) {
    if (estate->cur_error != NULL) ReThrowError(estate->cur_error);
    /* oops, we're not inside a handler */
    ereport(
        ERROR,
        (errcode(ERRCODE_STACKED_DIAGNOSTICS_ACCESSED_WITHOUT_ACTIVE_HANDLER),
         errmsg(
             "RAISE without parameters cannot be used outside an exception "
             "handler")));
  }
  ...
  if (stmt->message) {
    ...
    for (cp = stmt->message; *cp; cp++) {
      ...
      if (cp[0] == '%') {
        ...
        paramvalue =
            exec_eval_expr(estate, (PLpgSQL_expr *)lfirst(current_param),
                           &paramisnull, &paramtypeid, &paramtypmod);
  ...
  foreach (lc, stmt->options) {
    ...
    optionvalue = exec_eval_expr(estate, opt->expr, &optionisnull,
                                 &optiontypeid, &optiontypmod);
    if (optionisnull)
      ereport(ERROR, (errcode(ERRCODE_NULL_VALUE_NOT_ALLOWED),
  ...
  ereport(
      stmt->elog_level,
      (err_code ? errcode(err_code) : 0, errmsg_internal("%s", err_message),
       ...
       (err_column != NULL)
           ...
           : 0,
       (err_constraint != NULL)
           ? err_generic_string(PG_DIAG_CONSTRAINT_NAME, err_constraint)
           : 0,
       (err_datatype != NULL)
           ? err_generic_string(PG_DIAG_DATATYPE_NAME, err_datatype)
           : 0,
       (err_table != NULL) ? err_generic_string(PG_DIAG_TABLE_NAME, err_table)
                           : 0,
       (err_schema != NULL)
           ? err_generic_string(PG_DIAG_SCHEMA_NAME, err_schema)
           : 0));

  if (condname != NULL) pfree(condname);
  if (err_message != NULL) pfree(err_message);
  if (err_detail != NULL) pfree(err_detail);
  if (err_hint != NULL) pfree(err_hint);
  if (err_column != NULL) pfree(err_column);
  if (err_constraint != NULL) pfree(err_constraint);
  if (err_datatype != NULL) pfree(err_datatype);
  if (err_table != NULL) pfree(err_table);
  if (err_schema != NULL) pfree(err_schema);
...
static int exec_stmt_assert(PLpgSQL_execstate *estate,
  ...
  if (!plpgsql_check_asserts) return PLPGSQL_RC_OK;
  ...
  if (isnull || !value) {
    ...
    if (stmt->message != NULL) {
      ...
      val = exec_eval_expr(estate, stmt->message, &isnull, &typeid___, &typmod);
      if (!isnull) message = convert_value_to_string(estate, val, typeid___);
...
static void plpgsql_estate_setup(PLpgSQL_execstate *estate,
  ...
  estate->datums =
      (PLpgSQL_datum **)palloc(sizeof(PLpgSQL_datum *) * estate->ndatums);
  ...
  if (simple_eval_estate) {
    ...
    estate->cast_hash = hash_create(
        "PLpgSQL private cast cache", 16, /* start small and extend */
    ...
    if (shared_cast_hash == NULL) {
      ...
      shared_cast_hash =
          hash_create("PLpgSQL cast cache", 16, /* start small and extend */
                      &ctl, HASH_ELEM | HASH_BLOBS | HASH_CONTEXT);
    }
...
static void exec_eval_cleanup(PLpgSQL_execstate *estate) {
  ...
  if (estate->eval_tuptable != NULL) SPI_freetuptable(estate->eval_tuptable);
  estate->eval_tuptable = NULL;

  /* Clear result of exec_eval_simple_expr (but keep the econtext) */
  if (estate->eval_econtext != NULL) ResetExprContext(estate->eval_econtext);
}
...
static void exec_prepare_plan(PLpgSQL_execstate *estate, PLpgSQL_expr *expr,
   ...
   */
  plan = SPI_prepare_params(expr->query, (ParserSetupHook)plpgsql_parser_setup,
                            (void *)expr, cursorOptions);
  if (plan == NULL) {
    ...
    switch (SPI_result) {
      ...
      default:
        elog(ERROR, "SPI_prepare_params failed for \"%s\": %s", expr->query,
             SPI_result_code_string(SPI_result));
    }
  }
  SPI_keepplan(plan);
  ...
  if (stmt->into) {
    ...
    if (n == 0) {
      if (stmt->strict) {
        ...
        ereport(
            ERROR,
            (errcode(ERRCODE_NO_DATA_FOUND), errmsg("query returned no rows"),
    ...
    if (SPI_tuptable != NULL)
      ereport(ERROR,
              (errcode(ERRCODE_SYNTAX_ERROR),
               ...
               (rc == SPI_OK_SELECT) ? errhint(
                                           "If you want to discard the results "
                                           "of a SELECT, use PERFORM instead.")
                                     : 0));
  }
...
static int exec_stmt_dynexecute(PLpgSQL_execstate *estate,
  ...
  if (stmt->params) {
    ...
    exec_res =
        SPI_execute_with_args(querystr, ppd->nargs, ppd->types, ppd->values,
                              ppd->nulls, estate->readonly_func, 0);
  } else
  ...
  switch (exec_res) {
    ...
    default:
      elog(ERROR, "SPI_execute failed executing query \"%s\": %s", querystr,
           SPI_result_code_string(exec_res));
      break;
  }
...
static int exec_stmt_dynfors(PLpgSQL_execstate *estate,
  ...
  portal =
      exec_dynquery_with_params(estate, stmt->query, stmt->params, NULL, 0);
...
static void exec_assign_value(PLpgSQL_execstate *estate, PLpgSQL_datum *target,
  ...
  switch (target->dtype) {
    case PLPGSQL_DTYPE_VAR: {
      ...
      newvalue =
          exec_cast_value(estate, value, &isNull, valtype, valtypmod,
                          var->datatype->typoid, var->datatype->atttypmod);

      if (isNull && var->notnull)
        ereport(ERROR, (errcode(ERRCODE_NULL_VALUE_NOT_ALLOWED),
                        errmsg(
                            "null value cannot be assigned to variable \"%s\" "
                            "declared NOT NULL",
                            var->refname)));
      ...
      if (!var->datatype->typbyval && !isNull) {
        if (var->datatype->typisarray &&
          ...
          newvalue = expand_array(newvalue, CurrentMemoryContext, NULL);
        } else {
    ...
    case PLPGSQL_DTYPE_RECFIELD: {
      ...
      if (!HeapTupleIsValid(rec->tup))
        ereport(ERROR,
                (errcode(ERRCODE_OBJECT_NOT_IN_PREREQUISITE_STATE),
                 errmsg("record \"%s\" is not assigned yet", rec->refname),
                 errdetail(
                     "The tuple structure of a not-yet-assigned record is "
                     "indeterminate.")));
      ...
      values[fno] = exec_cast_value(estate, value, &isNull, valtype, valtypmod,
                                    atttype, atttypmod);
      nulls[fno] = isNull;
       ...
       */
      newtup =
          heap_modify_tuple(rec->tup, rec->tupdesc, values, nulls, replaces);

      if (rec->freetup) heap_freetuple(rec->tup);
    ...
    case PLPGSQL_DTYPE_ARRAYELEM: {
      ...
      do {
        ...
        if (nsubscripts >= MAXDIM)
          ereport(ERROR, (errcode(ERRCODE_PROGRAM_LIMIT_EXCEEDED),
                          errmsg(
                              "number of array dimensions (%d) exceeds the "
                              "maximum allowed (%d)",
      ...
      exec_eval_datum(estate, target, &parenttypoid, &parenttypmod,
      ...
      if (arrayelem->parenttypoid != parenttypoid ||
        ...
        get_typlenbyvalalign(elemtypoid, &elemtyplen, &elemtypbyval,
                             &elemtypalign);
      ...
      for (i = 0; i < nsubscripts; i++) {
        ...
        subscriptvals[i] = exec_eval_integer(
            estate, subscripts[nsubscripts - 1 - i], &subisnull);
        if (subisnull)
          ereport(ERROR,
                  (errcode(ERRCODE_NULL_VALUE_NOT_ALLOWED),
      ...
      coerced_value =
          exec_cast_value(estate, value, &isNull, valtype, valtypmod,
                          arrayelem->elemtypoid, arrayelem->arraytypmod);
      ...
      if (oldarrayisnull)
        oldarraydatum =
            PointerGetDatum(construct_empty_array(arrayelem->elemtypoid));

      /*
       * Build the modified array value.
       */
      newarraydatum = array_set_element(
          oldarraydatum, nsubscripts, subscriptvals, coerced_value, isNull,
          arrayelem->arraytyplen, arrayelem->elemtyplen,
          arrayelem->elemtypbyval, arrayelem->elemtypalign);
       ...
       */
      exec_assign_value(estate, target, newarraydatum, false,
                        arrayelem->arraytypoid, arrayelem->arraytypmod);
      break;
    }
...
void plpgsql_exec_get_datum_type_info(PLpgSQL_execstate *estate,
                                      PLpgSQL_datum *datum, Oid *typeid___,
...
static int exec_eval_integer(PLpgSQL_execstate *estate, PLpgSQL_expr *expr,
  ...
  exprdatum = exec_cast_value(estate, exprdatum, isNull, exprtypeid, exprtypmod,
                              INT4OID, -1);
...
static Datum exec_eval_expr(PLpgSQL_execstate *estate, PLpgSQL_expr *expr,
   ...
   */
  if (expr->plan == NULL) exec_prepare_plan(estate, expr, 0);
    ...
    ereport(ERROR,
            (errcode(ERRCODE_SYNTAX_ERROR),
             errmsg_plural("query \"%s\" returned %d column",
                           ...
                           estate->eval_tuptable->tupdesc->natts, expr->query,
  ...
  if (estate->eval_processed != 1)
    ereport(ERROR,
            (errcode(ERRCODE_CARDINALITY_VIOLATION),
             errmsg("query \"%s\" returned more than one row", expr->query)));
...
static int exec_run_select(PLpgSQL_execstate *estate, PLpgSQL_expr *expr,
  ...
  if (portalP != NULL) {
    ...
    *portalP = SPI_cursor_open_with_paramlist(NULL, expr->plan, paramLI,
                                              estate->readonly_func);
    if (*portalP == NULL)
...
static bool exec_eval_simple_expr(PLpgSQL_execstate *estate, PLpgSQL_expr *expr,
   ...
   */
  if (expr->expr_simple_expr == NULL) return false;
  ...
  if (cplan->generation != expr->expr_simple_generation) {
    ...
    if (expr->rwparam >= 0) exec_check_rw_parameter(expr, expr->rwparam);
  ...
  *result = ExecEvalExpr(expr->expr_simple_state, econtext, isNull, NULL);
  ...
  if (paramLI && paramLI != estate->paramLI) pfree(paramLI);

  estate->paramLI->parserSetupArg = save_setup_arg;

  if (!estate->readonly_func) PopActiveSnapshot();
    ...
    while ((dno = bms_next_member(expr->paramnos, dno)) >= 0) {
      ...
      if (datum->dtype == PLPGSQL_DTYPE_VAR) {
        ...
        if (dno == expr->rwparam)
          ...
          prm->value = MakeExpandedObjectReadOnly(var->value, var->isnull,
                                                  var->datatype->typlen);
...
static void plpgsql_param_fetch(ParamListInfo params, int paramid) {
   ...
   * data than necessary.
   */
  if (!bms_is_member(dno, expr->paramnos)) return;
  ...
  exec_eval_datum(estate, datum, &prm->ptype, &prmtypmod, &prm->value,
                  &prm->isnull);
  ...
  if (datum->dtype == PLPGSQL_DTYPE_VAR && dno != expr->rwparam)
    prm->value = MakeExpandedObjectReadOnly(
        prm->value, prm->isnull, ((PLpgSQL_var *)datum)->datatype->typlen);
}
...
static void exec_move_row(PLpgSQL_execstate *estate, PLpgSQL_rec *rec,
  ...
  if (rec != NULL) {
    ...
    if (tupdesc) tupdesc = CreateTupleDescCopy(tupdesc);
  ...
  if (row != NULL) {
    ...
    for (fnum = 0; fnum < row->nfields; fnum++) {
      ...
      exec_assign_value(estate, (PLpgSQL_datum *)var, value, isnull, valtype,
                        valtypmod);
    }
...
static HeapTuple make_tuple_from_row(PLpgSQL_execstate *estate,
  ...
  if (natts != row->nfields) return NULL;
  ...
  for (i = 0; i < natts; i++) {
    ...
    exec_eval_datum(estate, estate->datums[row->varnos[i]], &fieldtypeid,
                    &fieldtypmod, &dvalues[i], &nulls[i]);
    if (fieldtypeid != tupdesc->attrs[i]->atttypid) return NULL;
...
static Datum exec_cast_value(PLpgSQL_execstate *estate, Datum value,
  ...
  if (valtype != reqtype || (valtypmod != reqtypmod && reqtypmod != -1)) {
    ...
    cast_entry =
        get_cast_hashentry(estate, valtype, valtypmod, reqtype, reqtypmod);
    if (cast_entry) {
      ExprContext *econtext = estate->eval_econtext;
      MemoryContext oldcontext;
      ...
      value = ExecEvalExpr(cast_entry->cast_exprstate, econtext, isnull, NULL);
...
static plpgsql_CastHashEntry *get_cast_hashentry(PLpgSQL_execstate *estate,
  ...
  cast_entry = (plpgsql_CastHashEntry *)hash_search(
      estate->cast_hash, (void *)&cast_key, HASH_FIND, NULL);
  ...
  if (cast_entry == NULL) {
    ...
    if (srctype == UNKNOWNOID || srctype == RECORDOID)
      ...
      cast_expr = coerce_to_target_type(NULL, (Node *)placeholder, srctype,
                                        dsttype, dsttypmod, COERCION_ASSIGNMENT,
    ...
    if (cast_expr == NULL) {
      ...
      if (dsttypmod != -1)
        cast_expr = coerce_to_target_type(NULL, cast_expr, dsttype, dsttype,
                                          dsttypmod, COERCION_ASSIGNMENT,
                                          COERCE_IMPLICIT_CAST, -1);
    }
    ...
    cast_entry = (plpgsql_CastHashEntry *)hash_search(
        estate->cast_hash, (void *)&cast_key, HASH_ENTER, &found);
  ...
  if (cast_entry->cast_expr == NULL) return NULL;
...
static bool exec_simple_check_node(Node *node) {
  ...
  switch (nodeTag(node)) {
    ...
    case T_ArrayRef: {
      ...
      if (!exec_simple_check_node((Node *)expr->refupperindexpr)) return FALSE;
      if (!exec_simple_check_node((Node *)expr->reflowerindexpr)) return FALSE;
      if (!exec_simple_check_node((Node *)expr->refexpr)) return FALSE;
      if (!exec_simple_check_node((Node *)expr->refassgnexpr)) return FALSE;
    ...
    case T_FuncExpr: {
      ...
      if (expr->funcretset) return FALSE;
      if (!exec_simple_check_node((Node *)expr->args)) return FALSE;
    ...
    case T_FieldStore: {
      ...
      if (!exec_simple_check_node((Node *)expr->arg)) return FALSE;
      if (!exec_simple_check_node((Node *)expr->newvals)) return FALSE;
    ...
    case T_CaseExpr: {
      ...
      if (!exec_simple_check_node((Node *)expr->arg)) return FALSE;
      if (!exec_simple_check_node((Node *)expr->args)) return FALSE;
      if (!exec_simple_check_node((Node *)expr->defresult)) return FALSE;
    ...
    case T_CaseWhen: {
      ...
      if (!exec_simple_check_node((Node *)when->expr)) return FALSE;
      if (!exec_simple_check_node((Node *)when->result)) return FALSE;
    ...
    case T_ArrayExpr: {
      ...
      if (!exec_simple_check_node((Node *)expr->elements)) return FALSE;
    ...
    case T_RowCompareExpr: {
      ...
      if (!exec_simple_check_node((Node *)expr->largs)) return FALSE;
      if (!exec_simple_check_node((Node *)expr->rargs)) return FALSE;
    ...
    case T_XmlExpr: {
      ...
      if (!exec_simple_check_node((Node *)expr->named_args)) return FALSE;
      if (!exec_simple_check_node((Node *)expr->args)) return FALSE;
    ...
    case T_List: {
      ...
      foreach (l, expr) {
        if (!exec_simple_check_node((Node *)lfirst(l))) return FALSE;
      }
...
static void exec_simple_check_plan(PLpgSQL_expr *expr) {
  ...
  if (list_length(plansources) != 1) return;
   ...
   */
  if (list_length(plansource->query_list) != 1) return;
  query = (Query *)linitial(plansource->query_list);

  /*
   * 2. It must be a plain SELECT query without any input tables
   */
  if (!IsA(query, Query)) return;
  if (query->commandType != CMD_SELECT) return;
  if (query->rtable != NIL) return;

  /*
   * 3. Can't have any subplans, aggregates, qual clauses either
   */
  if (query->hasAggs || query->hasWindowFuncs || query->hasSubLinks ||
      query->hasForUpdate || query->cteList || query->jointree->quals ||
      query->groupClause || query->havingQual || query->windowClause ||
   ...
   */
  if (list_length(query->targetList) != 1) return;
...
static void exec_simple_recheck_plan(PLpgSQL_expr *expr, CachedPlan *cplan) {
   ...
   */
  if (list_length(cplan->stmt_list) != 1) return;
  stmt = (PlannedStmt *)linitial(cplan->stmt_list);

  /*
   * 2. It must be a RESULT plan --> no scan's required
   */
  if (!IsA(stmt, PlannedStmt)) return;
  if (stmt->commandType != CMD_SELECT) return;
  plan = stmt->planTree;
  if (!IsA(plan, Result)) return;

  /*
   * 3. Can't have any subplan or qual clause, either
   */
  if (plan->lefttree != NULL || plan->righttree != NULL ||
      plan->initPlan != NULL || plan->qual != NULL ||
   ...
   */
  if (list_length(plan->targetlist) != 1) return;
   ...
   */
  if (!exec_simple_check_node((Node *)tle->expr)) return;
...
static void exec_check_rw_parameter(PLpgSQL_expr *expr, int target_dno) {
   ...
   */
  if (expr->expr_simple_expr == NULL) return;

  /*
   * If target variable isn't referenced by expression, no need to look
   * further.
   */
  if (!bms_is_member(target_dno, expr->paramnos)) return;
   ...
   */
  if (!(funcid == F_ARRAY_APPEND || funcid == F_ARRAY_PREPEND)) return;
  ...
  foreach (lc, fargs) {
    ...
    if (arg && IsA(arg, Param)) continue;
    /* Otherwise, argument expression must not reference target */
    if (contains_target_param(arg, &target_dno)) return;
  }
...
static bool contains_target_param(Node *node, int *target_dno) {
  ...
  return expression_tree_walker(
      node,
      reinterpret_cast<expression_tree_walker_fptr>(contains_target_param),
      (int *)target_dno);
}
...
static void plpgsql_create_econtext(PLpgSQL_execstate *estate) {
  ...
  entry = (SimpleEcontextStackEntry *)MemoryContextAlloc(
      TopTransactionContext, sizeof(SimpleEcontextStackEntry));
...
void plpgsql_xact_cb(XactEvent event, void *arg) {
  ...
  if (event == XACT_EVENT_COMMIT || event == XACT_EVENT_PREPARE) {
    ...
    if (shared_simple_eval_estate) FreeExecutorState(shared_simple_eval_estate);
 ...
 */
void plpgsql_subxact_cb(SubXactEvent event, SubTransactionId mySubid,
...
static void assign_simple_var(PLpgSQL_execstate *estate, PLpgSQL_var *var,
  ...
  if (var->freeval) {
    if (DatumIsReadWriteExpandedObject(var->value, var->isnull,
                                       var->datatype->typlen))
  ...
  prm->value =
      MakeExpandedObjectReadOnly(newvalue, isnull, var->datatype->typlen);
  prm->isnull = isnull;
...
static PreparedParamsData *exec_eval_using_params(PLpgSQL_execstate *estate,
  ...
  foreach (lc, params) {
    ...
    ppd->values[i] =
        exec_eval_expr(estate, param, &isnull, &ppd->types[i], &ppdtypmod);
...
static void free_params_data(PreparedParamsData *ppd) {
  ...
  for (i = 0; i < ppd->nargs; i++) {
    if (ppd->freevals[i]) pfree(DatumGetPointer(ppd->values[i]));
  }
...
static Portal exec_dynquery_with_params(PLpgSQL_execstate *estate,
  ...
  if (params) {
    ...
    portal = SPI_cursor_open_with_args(portalname, querystr, ppd->nargs,
                                       ppd->types, ppd->values, ppd->nulls,
                                       estate->readonly_func, cursorOptions);
    free_params_data(ppd);
  } else {
    portal =
        SPI_cursor_open_with_args(portalname, querystr, 0, NULL, NULL, NULL,
                                  estate->readonly_func, cursorOptions);
  }

  if (portal == NULL)
    elog(ERROR, "could not open implicit cursor for query \"%s\": %s", querystr,
         SPI_result_code_string(SPI_result));
  pfree(querystr);
...
static char *format_expr_params(PLpgSQL_execstate *estate,
  ...
  while ((dno = bms_next_member(expr->paramnos, dno)) >= 0) {
    ...
    exec_eval_datum(estate, (PLpgSQL_datum *)curvar, &paramtypeid, &paramtypmod,
                    &paramdatum, &paramisnull);

    appendStringInfo(&paramstr, "%s%s = ", paramno > 0 ? ", " : "",
                     curvar->refname);
...
static char *format_preparedparamsdata(PLpgSQL_execstate *estate,
  ...
  for (paramno = 0; paramno < ppd->nargs; paramno++) {
    appendStringInfo(&paramstr, "%s$%d = ", paramno > 0 ? ", " : "",
                     paramno + 1);

    if (ppd->nulls[paramno] == 'n')
      appendStringInfoString(&paramstr, "NULL");
    else {
      char *value = convert_value_to_string(estate, ppd->values[paramno],
                                            ppd->types[paramno]);
      char *p;
