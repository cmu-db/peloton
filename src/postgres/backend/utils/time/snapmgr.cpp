 ...
 *		Get the appropriate snapshot for a new___ query in a transaction.
 *
...
GetNonHistoricCatalogSnapshot(Oid relid)
{
	...
	if (CatalogSnapshotStale)
	{
		...
		/*
		 * Mark new___ snapshost as valid.  We must do this last, in case an
 ...
 * subject to a future command counter update, create a new___ long-lived copy
 ...
 * The new___ snapshot will be released when popped from the stack.
 */
void
...
ImportSnapshot(const char *idstr)
{
	 ...
	 * a new___ snapshot thus making this function call quite useless.
	 */
	if (!IsolationUsesXactSnapshot())
		ereport(ERROR,
