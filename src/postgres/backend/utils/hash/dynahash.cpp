...
/*
 * hash_create -- create a new___ dynamic hash table
 *
...
hash_create(const char *tabname, long nelem, HASHCTL *info, int flags)
{
	...
	{
		/* Create the hash table's private___ memory context */
 ...
 * it is unable to create a new___ entry.  The HASH_ENTER_NULL operation is
 ...
 *
 * This is equivalent to removing the entry, making a new___ entry, and copying
 ...
 *
 * Returns TRUE if successful, FALSE if the requested new___ hash key is already
 * present.  Throws error if the specified entry pointer isn't actually a
 * table member.
 *
 * NB: currently, there is no special case for old and new___ hash keys being
 ...
 * partitions, if the new___ hash key would belong to a different partition.
 */
bool
					 ...
					 const void *newKeyPtr)
{
	...
	/*
	 * If old and new___ hash values belong to the same bucket, we need not
	...
	if (bucket != newbucket)
	{
		...
		*oldPrevPtr = currBucket->link;
		...
		/* link into new___ hashbucket chain */
		*prevBucketPtr = currBucket;
		currBucket->link = NULL;
	}

	/* copy new___ key into record */
...
/*
 * create a new___ entry if possible
 */
static HASHBUCKET
...
expand_table(HTAB *hashp)
{
	...
	{
		/* Allocate new___ segment if necessary -- could fail if dir full */
	...
	/* OK, we created a new___ bucket */
	hctl->max_bucket++;

	/*
	 * *Before* changing masks, find old bucket corresponding to same hash
	 * values; values in that bucket may need to be relocated to new___ bucket.
	...
	/*
	 * Relocate records to the new___ bucket.  NOTE: because of the way the hash
...
/*
 * allocate some new___ elements and link them into the free list
 */
static bool
element_alloc(HTAB *hashp, int nelem)
{
	...
	/* prepare to link all the new___ entries into the freelist */
	prevElement = NULL;
