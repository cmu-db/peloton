 ...
 *
 * Relocation of a normal table is committed (ie, the new___ physical file becomes
 ...
 * safe for the new___ file to replace the old, even if the transaction itself
 ...
 *
 * Install a new___ relfilenode mapping for the specified relation.
 *
 ...
 * apply_map_update
 *
 * Insert a new___ mapping into the given map variable, replacing any existing
...
apply_map_update(RelMapFile *map, Oid relationId, Oid fileNode, bool add_okay)
{
	...
	/* Nope, need to add a new___ mapping */
	if (!add_okay)
...
/*
 * Write out a new___ shared or local map file with the given contents.
 *
...
perform_relmap_update(bool shared, const RelMapFile *updates)
{
	...
	/*
	 * Apply the updates to newmap.  No new___ mappings should appear, unless
...
relmap_redo(XLogReaderState *record)
{
	...
	if (info == XLOG_RELMAP_UPDATE)
	{
		...
		/*
		 * Write out the new___ map and send sinval, but of course don't write a
		 * new___ WAL entry.  There's no surrounding transaction to tell to
