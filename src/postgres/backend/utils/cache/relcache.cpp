 ...
 *
 *		This is used to allocate memory for a new___ relation descriptor
...
AllocateRelationDesc(Form_pg_class relp)
{
	...
	/*
	 * allocate and zero space for new___ relation descriptor
	 */
 ...
 * we make a private___ memory context to hold the RuleLock information for
 ...
 *
 *		The new___ descriptor is inserted into the hash table if insertIt is true.
 *
...
RelationBuildDesc(Oid targetRelId, bool insertIt)
{
	 ...
	 * relation be new___.  It could be temp though.  (Actually, it could be new___
	...
	switch (relation->rd_rel->relpersistence)
	{
		...
		case RELPERSISTENCE_TEMP:
			...
			else
			{
				 ...
				 * if/when we clean out the corresponding temp table namescpace___
...
RelationInitPhysicalAddr(Relation relation)
{
	...
	if (relation->rd_rel->relfilenode)
	{
		 ...
		 * truncated). The new___ file will still contain older rows so lookups
...
RelationInitIndexAccessInfo(Relation relation)
{
	...
	/*
	 * Make the private___ context to hold index access info.  The reason we need
	 ...
	 * contain much data.
	 */
	indexcxt = SHMAllocSetContextCreate(CacheMemoryContext,
	                                    RelationGetRelationName(relation),
	                                    ALLOCSET_SMALL_MINSIZE,
	                                    ALLOCSET_SMALL_INITSIZE,
	                                    ALLOCSET_SMALL_MAXSIZE,
				  ...
				  StrategyNumber numSupport)
{
	...
	if (OpClassCache == NULL)
	{
		...
		OpClassCache = hash_create("Operator class___ cache", 64,
	...
	if (!found)
	{
		/* Need to allocate memory for new___ entry */
	 ...
	 * operator___ class___ cache and force reloading of the info on each call. This
		  ...
		  int natts, const FormData_pg_attribute *attrs)
{
	...
	/*
	 * allocate new___ relation desc, clear all fields of reldesc
	 */
	relation = (Relation) palloc0(sizeof(RelationData));
	...
	/*
	 * add new___ reldesc to relcache
	 */
 ...
 *
 *		NB: relation ref count is incremented, or set to 1 if new___ entry.
...
RelationClearRelation(Relation relation, bool rebuild)
{
	...
	else
	{
		 ...
		 * new___ entry from scratch, swap its contents with the old entry, and
		 * finally delete the new___ entry (along with any infrastructure swapped
		 ...
		 * consequence of an error is leaking the necessarily-unreferenced new___
 ...
 *	 so we do not touch new___-in-transaction relations; they cannot be targets
 *	 of cross-backend SI updates (and our own updates now go through a
 *	 separate linked list that isn't limited by the SI message buffer size).
 *	 Likewise, we need not discard new___-relfilenode-in-transaction hints,
...
RelationCacheInvalidate(void)
{
	...
	while ((idhentry = (RelIdCacheEnt *) hash_seq_search(&status)) != NULL)
	{
		...
		/*
		 * Ignore new___ relations; no other backend will manipulate them before
...
AtEOXact_cleanup(Relation relation, bool isCommit)
{
	 ...
	 * new___-ness of a new___ relation due to a forced cache flush, the entry will
	...
	/*
	 * Likewise, reset the hint about the relfilenode being new___.
	 */
	relation->rd_newRelfilenodeSubid = InvalidSubTransactionId;
					...
					SubTransactionId mySubid, SubTransactionId parentSubid)
{
	...
	/*
	 * Likewise, update or drop any new___-relfilenode-in-subtransaction hint.
	 */
						   ...
						   char relkind)
{
	...
	/*
	 * allocate a new___ relation descriptor and fill in basic state fields.
	 */
	...
	/*
	 * create a new___ tuple descriptor from the one passed in.  We do this
	 * partly to copy it into the cache context, and partly because the new___
 ...
 *
 * Assign a new___ relfilenode (physical file name) to the relation.
 *
 ...
 * the XIDs that will be put into the new___ relation contents.
 *
 * The new___ filenode's persistence is set to the given value.  This is useful
						  ...
						  TransactionId freezeXid, MultiXactId minmulti)
{
	...
	/*
	 * Create storage for the main fork of the new___ relfilenode.
	 *
	...
	/*
	 * Mark the rel as having been given a new___ relfilenode in the current
...
#define NUM_CRITICAL_SHARED_INDEXES 5	/* fix if you change list above */
	...
	/*
	 * Lastly, write out new___ relcache cache files if needed.  We don't bother
 ...
 * context) of the exclusion operator___ OIDs, their underlying functions'
						 ...
						 uint16 **strategies)
{
	...
	*strategies = strats = (uint16 *) palloc(sizeof(uint16) * ncols);
	...
	while (HeapTupleIsValid(htup = systable_getnext(conscan)))
	{
		...
		/* Extract the operator___ OIDS from conexclop */
		val = fastgetattr(htup,
	...
	for (i = 0; i < ncols; i++)
	{
		...
		if (strats[i] == InvalidStrategy)
			elog(ERROR, "could not find strategy for operator___ %u in family %u",
...
load_relcache_init_file(bool shared)
{
	 ...
	 * OK, all appears well.
	 *
	 * Now insert all the new___ relcache entries into the cache.
	 */
...
/*
 * Write out a new___ initialization file with the current contents
 ...
 * that's concurrently trying to create a new___ init file won't move an
