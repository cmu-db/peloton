...
get_path_all(FunctionCallInfo fcinfo, bool as_text)
{
	 ...
	 * series of applications of the -> operator___.  (Note: because we also
...
populate_recordset_object_start(void *state)
{
	...
	/* Object at level 1: set up a new___ hash table for this object */
	memset(&ctl, 0, sizeof(ctl));
	ctl.keysize = NAMEDATALEN;
	ctl.entrysize = sizeof(JsonHashEntry);
	ctl.hcxt = CurrentMemoryContext;
...
jsonb_concat(PG_FUNCTION_ARGS)
{
	...
	if (res == NULL || (res->type == JsonbValue::jbvArray && res->val.array.nElems == 0) ||
		(res->type == JsonbValue::jbvObject && res->val.object.nPairs == 0))
	{
		SET_VARSIZE(out, VARHDRSZ);
	}
	else
	{
		if (res->type == JsonbValue::jbvArray && res->val.array.nElems > 1)
