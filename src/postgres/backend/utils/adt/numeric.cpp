...
numeric		(PG_FUNCTION_ARGS)
{
	...
	if (typmod < (int32) (VARHDRSZ))
	{
		new___ = (Numeric) palloc(VARSIZE(num));
		memcpy(new___, num, VARSIZE(num));
		PG_RETURN_NUMERIC(new___);
	}
			...
			|| !NUMERIC_IS_SHORT(num)))
	{
		...
		if (NUMERIC_IS_SHORT(num))
			new___->choice.n_short.n_header =
				(num->choice.n_short.n_header & ~NUMERIC_SHORT_DSCALE_MASK)
				| (scale << NUMERIC_SHORT_DSCALE_SHIFT);
		else
			new___->choice.n_long.n_sign_dscale = NUMERIC_SIGN(new___) |
...
numeric_mul(PG_FUNCTION_ARGS)
{
	 ...
	 * case of numeric_mul(), which is invoked for the * operator___ on numerics,
...
/*
 * Accumulate a new___ input value for numeric aggregate functions.
 */
static void
 ...
 * won't be able to tell what the new___ aggregated value's dscale should be.
...
do_numeric_discard(NumericAggState *state, Numeric newval)
{
	...
	if (X.dscale == state->maxScale)
	{
		...
		{
			/* Correct new___ maxScale is uncertain, must fail */
			return false;
		}
	}
...
/*
 * Accumulate a new___ input value for 128-bit aggregate functions.
 */
static void
...
int2_sum(PG_FUNCTION_ARGS)
{
	 ...
	 * the new___ value of the transition variable. (If int8 is pass-by-value,
	...
	if (AggCheckCallContext(fcinfo, NULL))
	{
		...
		/* Leave the running sum unchanged in the new___ input is null */
		if (!PG_ARGISNULL(1))
...
#endif
	{
		...
		/* Leave sum unchanged if new___ input is null. */
		if (PG_ARGISNULL(1))
		...
		int rscale)
{
	...
	for (i1 = var1ndigits - 1; i1 >= 0; ri--, i1--)
	{
		...
		if (maxdig > INT_MAX / (NBASE - 1))
		{
			...
			Assert(carry == 0);
			/* Reset maxdig to indicate new___ worst-case */
			maxdig = 1 + var1digit;
		}
	...
	for (qi = 0; qi < div_ndigits; qi++)
	{
		...
		if (qdigit != 0)
		{
			...
			if (maxdiv > INT_MAX / (NBASE - 1))
			{
				...
				/*
				 * Recompute the quotient digit since new___ info may have
