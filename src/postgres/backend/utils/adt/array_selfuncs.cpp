 ...
 * selectivity as though this were an array containment operator___,
 * array_var op ARRAY[const].
 *
 * scalararraysel() has already verified that the ScalarArrayOpExpr's operator___
 * is the array element type's default equality or inequality operator___, and
						   ...
						   int varRelid)
{
	...
	/*
	 * If the operator___ is <>, swap ANY/ALL, then invert the result later.
	 */
	if (!isEquality)
		useOr = !useOr;
	...
	/*
	 * If the operator___ is <>, invert the results.
	 */
	if (!isEquality)
		selec = 1.0 - selec;
...
arraycontsel(PG_FUNCTION_ARGS)
{
	...
	if (!varonleft)
	{
		if (operator___ == OID_ARRAY_CONTAINS_OP)
			operator___ = OID_ARRAY_CONTAINED_OP;
		else if (operator___ == OID_ARRAY_CONTAINED_OP)
			operator___ = OID_ARRAY_CONTAINS_OP;
	}
		...
		element_typeid == get_base_element_type(vardata.vartype))
	{
		selec = calc_arraycontsel(&vardata, ((Const *) other)->constvalue,
								  element_typeid, operator___);
	}
	else
	{
		selec = DEFAULT_SEL(operator___);
	}
				  ...
				  Oid elemtype, Oid operator___)
{
	...
	typentry = lookup_type_cache(elemtype, TYPECACHE_CMP_PROC_FINFO);
	if (!OidIsValid(typentry->cmp_proc_finfo.fn_oid))
		return DEFAULT_SEL(operator___);
	cmpfunc = &typentry->cmp_proc_finfo;
	...
	if (HeapTupleIsValid(vardata->statsTuple))
	{
							 ...
							 &numbers, &nnumbers))
		{
			 ...
			 * element counts.
			 */
			if (operator___ != OID_ARRAY_CONTAINED_OP ||
				!get_attstatsslot(vardata->statsTuple,
								  elemtype, vardata->atttypmod,
								  STATISTIC_KIND_DECHIST, InvalidOid,
								  NULL,
 ...
 * mcelem_array_contained_selec depending on the operator___.
 */
static Selectivity
				   ...
				   Oid operator___, FmgrInfo *cmpfunc)
{
	 ...
	 */
	if (null_present && operator___ == OID_ARRAY_CONTAINS_OP)
	{
		pfree(elem_values);
		pfree(elem_nulls);
		return (Selectivity) 0.0;
	...
	/* Separate cases according to operator___ */
	if (operator___ == OID_ARRAY_CONTAINS_OP || operator___ == OID_ARRAY_OVERLAP_OP)
	...
	{
		elog(ERROR, "arraycontsel called for unrecognized operator___ %u",
			 operator___);
