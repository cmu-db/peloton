...
pg_signal_backend(int pid, int sig)
{
	 ...
	 * recycled for a new___ process, before reaching here?  Then we'd be trying
...
pg_collation_for(PG_FUNCTION_ARGS)
{
	...
	typeid___ = get_fn_expr_argtype(fcinfo->flinfo, 0);
	if (!typeid___)
		PG_RETURN_NULL();
	if (!type_is_collatable(typeid___) && typeid___ != UNKNOWNOID)
		ereport(ERROR,
