			...
			ereport(ERROR,
					(errcode(ERRCODE_INVALID_DATETIME_FORMAT),
					 ...
					 errhint("Do not mix Gregorian and ISO week date "
							 "conventions in a formatting ctemplate.")));
	}
}
	...
	*len = 0;
	...
	for (last = 0, a = array; *a != NULL; a++)
	{
		...
		for (i = 1, p = *a + 1, n = name + 1;; n++, p++, i++)
		{
			...
			/*
			 * Convert (but convert new___ chars only)
			 */
			if (i > last)
			{
...
datetime_to_char_body(TmToChar *tmtc, text *fmt, bool is_interval, Oid collid)
{
	...
	*result = '\0';
	...
	/*
	 * Allocate new___ memory if format picture is bigger than static cache and
	...
	else
	{
		...
		if ((ent = DCH_cache_search(fmt_str)) == NULL)
		{
			...
			/*
			 * Not in the cache, must run parser and save a new___ format-picture
			 * to the cache.
			 */
				...
				struct pg_tm * tm, fsec_t *fsec)
{
	...
	*fsec = 0;
	...
	if (fmt_len)
	{
		...
		/*
		 * Allocate new___ memory if format picture is bigger than static cache
		...
		else
		{
			...
			if ((ent = DCH_cache_search(fmt_str)) == NULL)
			{
				...
				/*
				 * Not in the cache, must run parser and save a new___
