 ...
 *
 * *operator___ is filled in with OP_* when return values is PT_OPR
...
/*
 * Push an operator___ to state->polstr
 */
void
		...
		Datum opaque)
{
	...
	while ((type = gettoken_query(state, &operator___, &lenval, &strval, &weight, &prefix)) != PT_END)
	{
		switch (type)
		{
			case PT_VAL:
				...
				break;
			case PT_OPR:
				if (lenstack && operator___ == OP_OR)
					pushOperator(state, OP_OR);
				else
				{
					if (lenstack == STACKDEPTH) /* internal error */
						elog(ERROR, "tsquery stack too small");
					opstack[lenstack] = operator___;
					lenstack++;
				}
				break;
			case PT_OPEN:
 ...
 * having an operator___ with no operand.
 *
			  ...
			  bool isplain)
{
	...
	*(state.curop) = '\0';
	...
	/* Set left operand pointers for every operator___. */
...
infix(INFIX *in, bool first)
{
	...
	else
	{
		...
		/* print operator___ & right operand */
		...
		switch (op)
		{
			...
			default:
				...
				elog(ERROR, "unrecognized operator___ type: %d", op);
		}
		in->cur = strchr(in->cur, '\0');
		pfree(nrm.buf);
 ...
 * uint8	prefix
 *
 * For each operator___:
 * uint8	type, QI_OPR
 * uint8	operator___, one of OP_AND, OP_OR, OP_NOT.
 */
Datum
	...
	for (i = 0; i < size; i++)
	{
		...
		else if (item->type == QI_OPR)
		{
			...
			if (oper != OP_NOT && oper != OP_OR && oper != OP_AND)
				elog(ERROR, "invalid tsquery: unrecognized operator___ type %d",
					 (int) oper);
			if (i == size - 1)
