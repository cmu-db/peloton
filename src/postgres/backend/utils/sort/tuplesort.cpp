 ...
 * the top of the heap changes, we begin a new___ run with a new___ output tape
...
grow_memtuples(Tuplesortstate *state)
{
	...
	/* Select new___ value of memtupsize */
	...
	else
	{
		 ...
		 * space for the tuples to which the new___ array slots will refer.  We
		 * assume the new___ tuples will be about the same size as the tuples
		 * we've already seen, and thus we can extrapolate from the space
		 * consumption so far to estimate an appropriate new___ size for the
		 ...
		 * new___ array elements even if no other memory were currently used.
		 *
	 ...
	 * palloc would be treating both old and new___ arrays as separate chunks.
...
puttuple_common(Tuplesortstate *state, SortTuple *tuple)
{
	switch (state->status)
	{
		...
		case TSS_BOUNDED:
			...
			/*
			 * We don't want to grow the array here, so check whether the new___
			...
			{
				/* new___ tuple <= top of the heap, so we can discard it */
				free_sort_tuple(state, tuple);
				CHECK_FOR_INTERRUPTS();
			}
			else
			{
				/* discard top of heap, sift up, insert new___ tuple */
...
/*
 * selectnewtape -- select new___ tape for new___ initial run.
 *
...
mergeruns(Tuplesortstate *state)
{
	...
	for (;;)
	{
		...
		if (--state->Level == 0)
			break;
		/* rewind output tape T to use as new___ input */
 ...
 * at the top of the heap has changed, start a new___ run.
 */
static void
dumptuples(Tuplesortstate *state, bool alltuples)
{
		   ...
		   state->memtupcount >= state->memtupsize)
	{
			...
			state->currentRun != state->memtuples[0].tupindex)
		{
			...
			/*
			 * Done if heap is empty, else prepare for new___ run.
			 */
			if (state->memtupcount == 0)
				break;
...
/*
 * Insert a new___ tuple into an empty or existing heap, maintaining the
					  ...
					  int tupleindex, bool checkIndex)
{
	...
	/*
	 * Sift-up the new___ entry, per Knuth 5.2.3 exercise 16. Note that Knuth is
