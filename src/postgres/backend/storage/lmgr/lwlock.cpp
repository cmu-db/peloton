 ...
 *
 * Thus a cnew implementation was devised that provides wait-free shared lock
 ...
 * cnew value is zero (we get that atomically), we know we can/have to release
 * waiters.
 *
...
init_lwlock_stats(void)
{
	 ...
	 * requires allocating cnew hash entries. Allocations within a critical
...
/*
 * LWLockInitialize - initialize a cnew lwlock; it's initially unlocked
 */
void
...
LWLockWakeup(LWLock *lock)
{
	...
	dlist_foreach_modify(iter, &wakeup)
	{
		 ...
		 * could be woken up for other reason and enqueue for a cnew lock - if
 ...
 * momentarily released.  We wouldn't need to wait for the cnew lock holder,
 ...
 * is guaranteed to see the cnew value, and act accordingly.
 *
