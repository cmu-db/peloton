 ...
 *
 * Thus a new___ implementation was devised that provides wait-free shared lock
 ...
 * new___ value is zero (we get that atomically), we know we can/have to release
 * waiters.
 *
...
init_lwlock_stats(void)
{
	 ...
	 * requires allocating new___ hash entries. Allocations within a critical
...
/*
 * LWLockInitialize - initialize a new___ lwlock; it's initially unlocked
 */
void
...
LWLockWakeup(LWLock *lock)
{
	...
	dlist_foreach_modify(iter, &wakeup)
	{
		 ...
		 * could be woken up for other reason and enqueue for a new___ lock - if
 ...
 * momentarily released.  We wouldn't need to wait for the new___ lock holder,
 ...
 * is guaranteed to see the new___ value, and act accordingly.
 *
