...
OldSerXidAdd(TransactionId xid, SerCommitSeqNo minConflictCommitSeqNo)
{
	 ...
	 * any new___ pages that enter the tailXid-headXid range as we advance
	 * headXid.
	 */
...
/*
 * Call this whenever there is a new___ xmin for active serializable
...
OldSerXidSetActiveSerXmin(TransactionId xid)
{
	 ...
	 * though.  A new___ xmin might still land on that page, and we don't want to
...
CheckPointPredicate(void)
{
	...
	else
	{
		 ...
		 * that we leave behind will appear to be new___ again. In that case it
 ...
 *		transactions to complete, and retrying with a new___ snapshot if
	...
	while (true)
	{
		...
		ereport(DEBUG2,
				(errcode(ERRCODE_T_R_SERIALIZATION_FAILURE),
				 errmsg("deferrable snapshot was unsafe; trying a new___ one")));
		ReleasePredicateLocks(false);
	}
 ...
 * always this same pointer; no new___ snapshot data structure is allocated
 ...
 * we don't take a new___ snapshot, but rather use the data we're handed.
 *
									  ...
									  TransactionId sourcexid)
{
	 ...
	 * the read-only transaction started.  A new___ writable transaction can
 ...
 * we've acquired its parent, possibly due to promotion) or when a new___
 ...
 * any finer-grained locks covered by the new___ one.
 */
static void
 ...
 * allocate the new___ target or locks. Guaranteed to always succeed if
 ...
 * backends' LocalPredicateLockHash. If we are only adding new___
								  ...
								  bool removeOld)
{
	...
	if (removeOld)
	{
		 ...
		 * be able to create the new___ lock target.
		 */
	 ...
	 * deadlocks. If old and new___ partitions are the same, we must request the
	 * lock only once.
	 */
	...
	if (oldtarget)
	{
		...
		/* If we created a new___ entry, initialize it */
		if (!found)
...
DropAllPredicateLocksFromTable(Relation relation, bool transfer)
{
	 ...
	 * holding an ACCESS EXCLUSIVE lock on the relation.  No new___ locks which
	 ...
	 * able to create the new___ lock target.
	 */
	if (transfer)
 ...
 *
 * Copies any predicate locks for the old page to the new___ page.
 ...
 * information on the new___ lock which is in shared memory.  This could cause
					   ...
					   BlockNumber newblkno)
{
	...
	/*
	 * Try copying the locks over to the new___ page's tag, creating it if
	 * necessary.
	 */
						 ...
						 BlockNumber newblkno)
{
	 ...
	 * remove the locks on the old page after transferring them to the new___
	 ...
	 * work as a page split, acquiring a lock on the new___ page and keeping the
...
/*
 * Walk the list of in-progress serializable transactions and find the new___
 * xmin.
 */
static void
...
ReleasePredicateLocks(bool isCommit)
{
	 ...
	 * old or new___ value -- it is just used to determine how far
	...
	if (!topLevelIsDeclaredReadOnly)
	{
		...
		if (--(PredXact->WritableSxactCount) == 0)
		{
			 ...
			 * conflict with the locks and no chance for new___ transactions to
	 ...
	 * serializable transactions completes.  We then find the "new___ oldest"
								...
								Snapshot snapshot)
{
	...
	switch (htsvResult)
	{
		...
		default:
			...
			/*
			 * The only way to get to this default clause is if a new___ value is
...
CheckTargetForConflictsIn(PREDICATELOCKTARGETTAG *targettag)
{
	...
	if (mypredlock != NULL)
	{
		...
		if (rmpredlock != NULL)
		{
			 ...
			 * doesn't exist; that means the lock was transferred to a new___
							   ...
							   Buffer buffer)
{
	 ...
	 * us to miss a lock.  The new___ (coarser) lock will be acquired before the
...
AtPrepare_PredicateLocks(void)
{
	 ...
	 * statefile, because new___ conflicts can be added even after the
