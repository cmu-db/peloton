...
ProcArrayApplyRecoveryInfo(RunningTransactions running)
{
	 ...
	 *
	 * NB: this can be reached at least twice, so make sure new___ code can deal
	 * with that.
	 */
	 ...
	 * sort them first.
	 *
	 * Some of the new___ xids are top-level xids and some are subtransactions.
...
GetSnapshotData(Snapshot snapshot)
{
	...
	/*
	 * This is a new___ snapshot, so set both refcounts are zero, and mark it as
 ...
 * that atomically with installing the new___ xmin.
 *
...
ProcArrayInstallImportedXmin(TransactionId xmin, TransactionId sourcexid)
{
	...
	for (index = 0; index < arrayP->numProcs; index++)
	{
		...
		/*
		 * We're good.  Install the new___ xmin.  As in GetSnapshotData, set
 ...
 * releasing them. Acquiring XidGenLock ensures that no new___ XIDs enter the proc
 ...
 *
 * This is useful to initialize the cutoff xid after which a new___ changeset
...
GetOldestSafeDecodingTransactionId(void)
{
	 ...
	 * running. If no transaction with xid were running concurrently a new___ xid
 ...
 * list expands as new___ XIDs are observed or inferred, and contracts when
 ...
 *	* startup process adding new___ known-assigned XIDs
 ...
 *
 * It's cheap to maintain the sortedness during insertions, since new___ known
 ...
 *
 *	* Adding a new___ XID is O(1) and needs little locking (unless compression
 *		must happen)
