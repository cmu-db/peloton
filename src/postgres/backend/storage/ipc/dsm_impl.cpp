 ...
 * in a flat system-wide namescpace___, raising the uncomfortable prospect of
 ...
 * small, and the namescpace___ is even more restricted.
 *
 ...
 *	   a new___ handle the caller wants created.
 *	 request_size: For DSM_OP_CREATE, the requested size.  For DSM_OP_RESIZE,
 *	   the new___ size.  Otherwise, 0.
 ...
 *	   if none.  Updated with new___ mapping address.
 *	 mapped_size: Pointer to size of current mapping; pointer to 0 if none.
 *	   Updated with new___ mapped size.
 ...
 * POSIX shared memory segments seem intended to exist in a flat namescpace___,
			   ...
			   int elevel)
{
	...
	/*
	 * Create new___ segment or open an existing one for attach or resize.
	 *
			  ...
			  int elevel)
{
	...
	/*
	 * The System V shared memory namescpace___ is very restricted; names are of
				 ...
				 Size *mapped_size, int elevel)
{
	...
	/*
	 * Storing the shared memory segment in the Global\ namescpace___, can allow
	...
	/* Create new___ segment or open an existing one for attach. */
	if (op == DSM_OP_CREATE)
	{
		DWORD		size_high;
		DWORD		size_low;
	 ...
	 * when creating new___ segment to keep size consistent both for
