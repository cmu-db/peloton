 ...
 *
 * This is also used for resolving ambiguous operator___ references.  Formerly
 ...
 * OLD COMMENTS:
 *
 * This routine is new___ code, replacing binary_oper_select_candidate()
 * which dates from v4.2/v1.0.x days. It tries very hard to match up
 * operators with types, including allowing type coercions if necessary.
 * The important thing is that the code do as much as possible,
 * while _never_ doing the wrong thing, where "the wrong thing" would
 * be returning an operator___ when other better choices are available,
 * or returning an operator___ which is a non-intuitive possibility.
 * - thomas 1998-05-21
 *
 ...
 * - thomas 1998-05-20
 *
 * current wisdom holds that the default operator___ should be one in which
 ...
 * one matching operator___ if the right arg is unknown (eg. float
 * specified with quotes). This break some stuff in the regression
 * test where there are floats in quotes not properly casted. Below is
 * the solution. In addition to requiring the operator___ operates on the
					  ...
					  FuncCandidateList candidates)
{
	 ...
	 *
	 * NOTE: for a binary operator___ with one unknown and one non-unknown input,
				...
				List **argdefaults)		/* optional return value */
{
	...
	*true_typeids = NULL;
	...
	/* Get list of possible candidates from namescpace___ search */
	...
	if (best_candidate == NULL)
	{
		...
		if (nargs == 1 && fargs != NIL && fargnames == NIL)
		{
			...
			if (OidIsValid(targetType))
			{
				...
				{
					/* always treat typename___('literal') as coercion */
					iscoercion = true;
				}
				else
				{
