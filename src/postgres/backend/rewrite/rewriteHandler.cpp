				  ...
				  bool *returning_flag)
{
	...
	/*
	 * Rewrite new___.attribute with right hand side of target-list entry for
...
/*
 * Convert a matched TLE from the original tlist into a correct new___ TLE.
 *
				  ...
				  bool forUpdatePushedDown)
{
	...
	if (rt_index == parsetree->resultRelation)
	{
				 ...
				 parsetree->commandType == CMD_DELETE)
		{
			 ...
			 * represent NEW values, so change them to reference the new___ RTE.
			 *
...
fireRIRrules(Query *parsetree, List *activeRIRs, bool forUpdatePushedDown)
{
	...
	while (rt_index < list_length(parsetree->rtable))
	{
		 ...
		 * This prevents infinite expansion loop due to new___ rtable entries
		...
		/*
		 * Also, if this is a new___ result relation introduced by
	 ...
	 * requires special recursion detection if the new___ quals have sublink
	...
	foreach(lc, parsetree->rtable)
	{
		...
		/*
		 * Fetch any new___ security quals that must be applied to this RTE.
		 */
		...
		if (securityQuals != NIL || withCheckOptions != NIL)
		{
			...
			{
				/*
				 * Recursively process the new___ quals, checking for infinite
				 * recursion.
				 */
				if (list_member_oid(activeRIRs, RelationGetRelid(rel)))
					ereport(ERROR,
			...
			/*
			 * Add the new___ security quals to the start of the RTE's list so
		 ...
		 * applies, or if the new___ quals had sublinks.
		 */
		if (hasRowSecurity)
...
rewriteTargetView(Query *parsetree, Relation view)
{
	...
	/*
	 * Create a new___ target RTE describing the base relation, and add it to the
	 ...
	 * the new___ target RTE, ie make their varnos be new_rt_index instead of
	...
	/*
	 * Mark the new___ target RTE for the permissions checks that we want to
	...
	/*
	 * Move any security barrier quals from the view RTE onto the new___ target
	 * RTE.  Any such quals should now apply to the new___ target RTE and will
	 ...
	 * Note that when we recurse through rewriteQuery a new___ junk TLE will be
	 * added to allow the executor to find the proper row in the new___ target
	 ...
	 * (for example, parsetree->resultRelation itself) to point to the new___
	 ...
	 * only adjust their varnos to reference the new___ target (just the same as
	 ...
	 * user-supplied quals, so these quals are kept on the new___ target RTE.
	 *
	...
	if (parsetree->commandType != CMD_DELETE)
	{
		...
		/*
		 * Add the new___ WithCheckOption to the start of the list, so that
		 * checks on inner views are run before checks on outer views, as
		 * required by the SQL standard.
		 *
		 * If the new___ check is CASCADED, we need to add it even if this view
