...
/*
 * Check whether a proposed operator___ name is legal
 *
 ...
 * are operator___ names rather than some other lexical entity.
 */
static bool
validOperatorName(const char *name)
{
	 ...
	 * a multi-char operator___ unless the operator___ contains chars that are not
	 * in SQL operators. The idea is to lex '=-' as two operators, but not to
	 * forbid operator___ names like '?-' that could not be sequences of standard
	 * SQL operators.
	 */
	if (len > 1 &&
		(name[len - 1] == '+' ||
 ...
 * OperatorGet
 *
 *		finds an operator___ given an exact specification (name, namescpace___,
 ...
 * OperatorLookup
 *
 *		looks up an operator___ given a possibly-qualified name and
 *		left and right type IDs.
 *
 *		*defined is set TRUE if defined (not a shell)
 */
...
/*
 * OperatorShellMake
 *		Make a "shell" entry for a not-yet-existing operator___.
 */
static Oid
				  ...
				  Oid rightTypeId)
{
	...
	/*
	 * validate operator___ name
	 */
	if (!validOperatorName(operatorName))
		ereport(ERROR,
				(errcode(ERRCODE_INVALID_NAME),
				 errmsg("\"%s\" is not a valid operator___ name",
						operatorName)));
	...
	/*
	 * initialize values[] with the operator___ name and input data types. Note
	...
	/*
	 * create a new___ operator___ tuple
	 */
	tup = heap_form_tuple(tupDesc, values, nulls);

	/*
	 * insert our "shell" operator___ tuple
	 */
	...
	/* Post creation hook for new___ shell operator___ */
	...
	/*
	 * close the operator___ relation and return the oid.
	 */
 ...
 *		operatorName			name for new___ operator___
 *		operatorNamespace		namescpace___ for new___ operator___
 *		leftTypeId				X left type ID
 *		rightTypeId				X right type ID
 *		procedureId				procedure ID for operator___
 *		commutatorName			X commutator operator___
 *		negatorName				X negator operator___
 *		restrictionId			X restriction selectivity procedure ID
 *		joinId					X join selectivity procedure ID
 *		canMerge				merge join can be used with this operator___
 *		canHash					hash join can be used with this operator___
 *
 ...
 * negator operator___ references specially, however, since those need not
 * exist beforehand.
 *
 * This routine gets complicated because it allows the user to
 * specify operators that do not exist.  For example, if operator___
 * "op" is being defined, the negator operator___ "negop" and the
 ...
 * information about the operator___ (just its name and types).
			   ...
			   bool canHash)
{
	...
	if (operatorAlreadyDefined)
		ereport(ERROR,
				(errcode(ERRCODE_DUPLICATE_FUNCTION),
				 errmsg("operator___ %s already exists",
						operatorName)));
	...
	if (commutatorName)
	{
		...
		/* Permission check: must own other operator___ */
		...
		/*
		 * self-linkage to this operator___; will fix below. Note that only
	...
	/*
	 * set up values in the operator___ tuple
	 */
	...
	/*
	 * If we are replacing an operator___ shell, update; else insert
	 */
	if (operatorObjectId)
	{
	...
	/* Post creation hook for new___ operator___ */
	InvokeObjectPostCreateHook(OperatorRelationId, operatorObjectId, 0);
	 ...
	 * operator___(s) to point at this one, if they don't already have a link.
	 * This supports an alternative style of operator___ definition wherein the
	 * user first defines one operator___ without giving negator or commutator,
	 * then defines the other operator___ of the pair with the proper commutator
	 * or negator attribute.  That style doesn't require creation of a shell,
	 * and it's the only style that worked right before Postgres version 6.5.
	 * This code also takes care of the situation where the new___ operator___ is
	 * its own commutator.
	 */
	if (selfCommutator)
...
/*
 * Try to lookup another operator___ (commutator, etc)
 *
 * If not found, check to see if it is exactly the operator___ we are trying
 ...
 * the same operator___, create a shell operator___.
 */
static Oid
				   ...
				   Oid leftTypeId, Oid rightTypeId, bool isCommutator)
{
	...
	{
		/*
		 * self-linkage to this operator___; caller will fix later. Note that
		 * only self-linkage for commutation makes sense.
		 */
		if (!isCommutator)
			ereport(ERROR,
					(errcode(ERRCODE_INVALID_FUNCTION_DEFINITION),
			 errmsg("operator___ cannot be its own negator or sort operator___")));
		return InvalidOid;
	}

	/* not in catalogs, different from operator___, so make shell */
 ...
 * OperatorUpd
 *
 *	For a given operator___, look up its negator and commutator operators.
 *	If they are defined, but their negator and commutator fields
 *	(respectively) are empty, then use the new___ operator___ for neg or comm.
 *	This solves a problem for users who need to insert two new___ operators
...
OperatorUpd(Oid baseId, Oid commId, Oid negId)
{
	 ...
	 * operator___: we'll need to update the tuple that we just inserted.
	 */
	CommandCounterIncrement();
	...
	/*
	 * if the commutator and negator are the same operator___, do one update. XXX
...
/*
 * Create dependencies for a new___ operator___ (either a freshly inserted
 * complete operator___, a new___ shell operator___, or a just-updated shell).
 *
 * NB: the OidIsValid tests in this routine are necessary, in case
 * the given operator___ is a shell.
 */
...
makeOperatorDependencies(HeapTuple tuple)
{
	...
	/*
	 * NOTE: we do not consider the operator___ to depend on the associated
	 * operators oprcom and oprnegate. We would not want to delete this
	 * operator___ if those go away, but only reset the link fields; which is not
	 * a function that the dependency code can presently handle.  (Something
	 * could perhaps be done with objectSubId though.)	For now, it's okay to
	 * let those links dangle if a referenced operator___ is removed.
	 */
