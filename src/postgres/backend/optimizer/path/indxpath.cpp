							   ...
							   List **considered_relids)
{
	...
	foreach(lc, indexjoinclauses)
	{
		 ...
		 * it prepends them to the list, so that we won't visit new___ entries
		...
		foreach(lc2, *considered_relids)
		{
			...
			/*
			 * If either is a subset of the other, no new___ set is possible.
			...
			/*
			 * If this clause was derived from an equivalence class___, the
 ...
 *		the specified equivalence class___.
 */
static bool
...
choose_bitmap_and(PlannerInfo *root, RelOptInfo *rel, List *paths)
{
	...
	for (i = 0; i < npaths; i++)
	{
		...
		for (j = i + 1; j < npaths; j++)
		{
			...
			/* tentatively add new___ path to paths, so we can estimate cost */
			paths = lappend(paths, pathinfo->path);
			newcost = bitmap_and_cost_est(root, rel, paths);
			if (newcost < costsofar)
			{
				/* keep new___ path in paths, update subsidiary variables */
				costsofar = newcost;
			...
			{
				/* reject new___ path, remove it from paths list */
 ...
 *		   and
 *	  (2)  must contain an operator___ which is in the same family as the index
 *		   operator___ for this column, or is a "special" operator___ as recognized
 ...
 *	  suitable commutator operator___ is available.
 *
						 ...
						 RestrictInfo *rinfo)
{
	...
	/*
	 * Check for clauses of the form: (indexkey operator___ constant) or
	 * (constant operator___ indexkey).  See above notes about const-ness.
	 */
		...
		!contain_volatile_functions(rightop))
	{
		 ...
		 * is a "special" indexable operator___.
		 */
		if (plain_op &&
 ...
 *	  Does the operator___ match the specified index opfamily?
 *
 * If the indexkey is on the right, what we actually want to know
 * is whether the operator___ has a commutator operator___ that matches
 * the opfamily.
 */
static bool
is_indexable_operator(Oid expr_op, Oid opfamily, bool indexkey_on_left)
{
	/* Get the commuted operator___ if necessary */
	if (!indexkey_on_left)
	{
	...
	/* OK if the (commuted) operator___ is a member of the index's opfamily */
							 ...
							 RowCompareExpr *clause)
{
	 ...
	 * reverse-sort family.  So look only at the operator___, and match if it is
		...
		!contain_volatile_functions(rightop))
	{
		/* OK, indexkey is on left */
			 ...
			 !contain_volatile_functions(leftop))
	{
		/* indexkey is on right, so commute the operator___ */
		expr_op = get_commutator(expr_op);
		if (expr_op == InvalidOid)
			return false;
	}
	...
	/* We're good if the operator___ is the right type of opfamily member */
 ...
 * "indexedcol operator___ pseudoconstant", along with an integer list of the
 ...
 *	  Determines whether an ordering operator___ expression matches an
 *	  index column.
 *
 ...
 *	  operator___ for the column's opfamily.
 *
 * 'index' is the index of interest.
 * 'indexcol' is a column number of 'index' (counting from 0).
 ...
 * operator___'s result.  In practical cases the result type will be numeric
 * and thus have no collation, and it's not very clear what to match to
 * if it did have a collation.  The index's collation should match the
 * ordering operator___'s input collation, not its result.
 *
							...
							Oid pk_opfamily)
{
		...
		!contain_volatile_functions(rightop))
	{
		commuted = false;
			 ...
			 !contain_volatile_functions(leftop))
	{
		/* Might match, but we need a commuted operator___ */
		expr_op = get_commutator(expr_op);
		if (expr_op == InvalidOid)
			return NULL;
		commuted = true;
	...
	/*
	 * Is the (commuted) operator___ an ordering operator___ for the opfamily? And
							  ...
							  List *exprlist, List *oprlist)
{
	...
	foreach(ic, rel->baserestrictinfo)
	{
		 ...
		 * mergeopfamilies will be if it has a mergejoinable operator___ and
	...
	foreach(ic, rel->indexlist)
	{
		...
		for (c = 0; c < ind->ncolumns; c++)
		{
			...
			foreach(lc, restrictlist)
			{
				...
				/*
				 * The condition's equality operator___ must be a member of the
			...
			forboth(lc, exprlist, lc2, oprlist)
			{
				...
				/*
				 * The equality operator___ must be a member of the index
				 * opfamily, else it is not asserting the right kind of
				 * equality behavior for this index.  We assume the caller
				 * determined it is an equality operator___, so we don't need to
 ...
 * for a collation match, if it's dealing with an operator___ where that matters.
 *
					   ...
					   IndexOptInfo *index)
{
	 ...
	 * able to apply indexscanning in binary-compatible-operator___ cases. Note:
 ...
 * that pass the operator___ clause itself must also satisfy the simpler
 * indexscan condition(s).  Then we can use the indexscan machinery
 * to avoid scanning as much of the table as we'd otherwise have to,
 * while applying the original operator___ as a qpqual condition to ensure
 ...
 * restriction opclause's operator___ as a member of an index's opfamily,
 ...
 * index's operator___ family.  We check to see if the clause matches the
 * index's key.
 */
static bool
							 ...
							 bool indexkey_on_left)
{
	 ...
	 * this test is *not* dependent on the LIKE/regex operator___'s collation.
	 */
	switch (expr_op)
	{
		case OID_TEXT_LIKE_OP:
 ...
 *	  a boolean equality operator___ clause.
 *
...
expand_indexqual_opclause(RestrictInfo *rinfo, Oid opfamily, Oid idxcollation)
{
	 ...
	 * as GIN.  Therefore, we should only do expansion if the operator___ is
	 * actually not in the opfamily.  But checking that requires a syscache
	 * lookup, so it's best to first see if the operator___ is one we are
	 * interested in.
	 */
	switch (expr_op)
	{
							...
							int indexcol)
{
	...
	/* Else we need a new___ RestrictInfo */
							...
							bool *var_on_left_p)
{
	...
	*indexcolnos = list_make1_int(indexcol);
	...
	/* Build lists of the opfamilies and operator___ datatypes in case needed */
	...
	while (largs_cell != NULL)
	{
		...
		else
		{
			...
			expr_op = get_commutator(expr_op);
			if (expr_op == InvalidOid)
				break;			/* operator___ is not usable */
		}
 ...
 * operator___ family; we use it to deduce the appropriate comparison
...
/*
 * Given a leftop and a rightop, and an inet-family sup/sub operator___,
 * generate suitable indexqual condition(s).  expr_op is the original
 * operator___, and opfamily is the index opfamily.
 */
static List *
network_prefix_quals(Node *leftop, Oid expr_op, Oid opfamily, Datum rightop)
{
	...
	switch (expr_op)
	{
		...
		case OID_INET_SUBEQ_OP:
			...
			break;
		default:
			elog(ERROR, "unexpected operator___: %u", expr_op);
			return NIL;
	}

	/*
	 * create clause "key >= network_scan_first( rightop )", or ">" if the
	 * operator___ disallows equality.
	 */
	if (is_eq)
	{
