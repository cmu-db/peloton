...
ExecHashIncreaseNumBatches(HashJoinTable hashtable)
{
	...
	while (oldchunks != NULL)
	{
		...
		while (idx < oldchunks->used)
		{
			...
			if (batchno == curbatch)
			{
				/* keep tuple in memory - copy it into the new___ chunk */
					 ...
					 uint32 *hashvalue)
{
	...
	foreach(hk, hashkeys)
	{
		...
		/*
		 * If the attribute is NULL, and the join operator___ is strict, then
		 * this tuple cannot pass the join qual so we can reject it
		 * immediately (unless we're scanning the outside of an outer join, in
		 * which case we must not reject it).  Otherwise we act like the
		 * hashcode of NULL is zero (this will support operators that act like
		 * IS NOT DISTINCT, though not any more-random behavior).  We treat
		 * the hash support function as strict even if the operator___ is not.
		 *
				   ...
				   ExprContext *econtext)
{
	 ...
	 * bucket, or NULL if it's time to start scanning a new___ bucket.
	 *
...
ExecScanHashTableForUnmatched(HashJoinState *hjstate, ExprContext *econtext)
{
	...
	for (;;)
	{
		 ...
		 * current bucket, or NULL if it's time to start scanning a new___
		 * bucket.
		 */
		if (hashTuple != NULL)
			hashTuple = hashTuple->next;
 ...
 * ExecHashTableReset
 *
 *		reset hash table header for new___ batch
 */
void
ExecHashTableReset(HashJoinTable hashtable)
{
	 ...
	 * reinitialize the context for a new___ pass.
	 */
...
ExecHashBuildSkewHash(HashJoinTable hashtable, Hash *node, int mcvsToUse)
{
						 ...
						 &numbers, &nnumbers))
	{
		...
		for (i = 0; i < mcvsToUse; i++)
		{
			...
			/* Okay, create a new___ skew bucket for this hashvalue. */
...
dense_alloc(HashJoinTable hashtable, Size size)
{
	...
	{
		/* allocate new___ chunk and put it at the beginning of the list */
