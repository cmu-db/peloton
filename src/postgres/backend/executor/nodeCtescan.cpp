...
CteScanNext(CteScanState *node)
{
	...
	if (eof_tuplestore && !node->leader->eof_cte)
	{
		 ...
		 * any other readers will *not* move past the new___ tuple, which is what
		 * they want.
		 */
...
ExecInitCteScan(CteScan *node, EState *estate, int eflags)
{
	...
	/*
	 * create new___ CteScanState for node
	 */
...
ExecReScanCteScan(CteScanState *node)
{
	...
	/*
	 * Clear the tuplestore if a new___ scan of the underlying CTE is required.
