...
static void _SPI_cursor_operation(Portal portal, FetchDirection direction,
...
int SPI_connect(void) {
   ...
   * _SPI_connected
   */
  if (_SPI_curid != _SPI_connected) return SPI_ERROR_CONNECT;

  if (_SPI_stack == NULL) {
    if (_SPI_connected != -1 || _SPI_stack_depth != 0)
      elog(ERROR, "SPI stack corrupted");
    newdepth = 16;
    _SPI_stack = (_SPI_connection *)MemoryContextAlloc(
        TopTransactionContext, newdepth * sizeof(_SPI_connection));
    ...
    if (_SPI_stack_depth == _SPI_connected + 1) {
      ...
      _SPI_stack = (_SPI_connection *)repalloc(
          _SPI_stack, newdepth * sizeof(_SPI_connection));
      _SPI_stack_depth = newdepth;
    }
  }
  ...
  _SPI_current->procCxt = AllocSetContextCreate(
      TopTransactionContext, "SPI Proc", ALLOCSET_DEFAULT_MINSIZE,
...
void SPI_push(void) { _SPI_curid++; }

/* Pops SPI stack to allow recursive SPI calls */
void SPI_pop(void) { _SPI_curid--; }
...
int SPI_execute(const char *src, bool read_only, long tcount) {
  ...
  res = _SPI_execute_plan(&plan, NULL, InvalidSnapshot, InvalidSnapshot,
...
int SPI_execute_plan(SPIPlanPtr plan, Datum *Values, const char *Nulls,
  ...
  if (plan->nargs > 0 && Values == NULL) return SPI_ERROR_PARAM;

  res = _SPI_begin_call(true);
  if (res < 0) return res;

  res = _SPI_execute_plan(
      plan, _SPI_convert_params(plan->nargs, plan->argtypes, Values, Nulls),
      InvalidSnapshot, InvalidSnapshot, read_only, true, tcount);
...
int SPI_execute_plan_with_paramlist(SPIPlanPtr plan, ParamListInfo params,
  ...
  res = _SPI_execute_plan(plan, params, InvalidSnapshot, InvalidSnapshot,
                          read_only, true, tcount);
 ...
 * fetching a new___ snapshot for each query.
 */
int SPI_execute_snapshot(SPIPlanPtr plan, Datum *Values, const char *Nulls,
  ...
  res = _SPI_execute_plan(
      plan, _SPI_convert_params(plan->nargs, plan->argtypes, Values, Nulls),
      snapshot, crosscheck_snapshot, read_only, fire_triggers, tcount);
...
int SPI_execute_with_args(const char *src, int nargs, Oid *argtypes,
  ...
  if (src == NULL || nargs < 0 || tcount < 0) return SPI_ERROR_ARGUMENT;

  if (nargs > 0 && (argtypes == NULL || Values == NULL)) return SPI_ERROR_PARAM;
  ...
  res = _SPI_execute_plan(&plan, paramLI, InvalidSnapshot, InvalidSnapshot,
                          read_only, true, tcount);
...
int SPI_freeplan(SPIPlanPtr plan) {
  ...
  if (plan == NULL || plan->magic != _SPI_PLAN_MAGIC) return SPI_ERROR_ARGUMENT;
...
HeapTuple SPI_copytuple(HeapTuple tuple) {
  ...
  if (oldcxt) MemoryContextSwitchTo(oldcxt);
...
HeapTuple SPI_modifytuple(Relation rel, HeapTuple tuple, int natts, int *attnum,
  ...
  if (rel == NULL || tuple == NULL || natts < 0 || attnum == NULL ||
      Values == NULL) {
    SPI_result = SPI_ERROR_ARGUMENT;
    return NULL;
  }
  ...
  if (i == natts) /* no errors in *attnum */
  {
    ...
    if (rel->rd_att->tdhasoid) HeapTupleSetOid(mtuple, HeapTupleGetOid(tuple));
  } else {
    mtuple = NULL;
...
int SPI_fnumber(TupleDesc tupdesc, const char *fname) {
  ...
  for (res = 0; res < tupdesc->natts; res++) {
    if (namestrcmp(&tupdesc->attrs[res]->attname, fname) == 0) return res + 1;
  }

  sysatt = SystemAttributeByName(fname, true /* "oid" will be accepted */);
  if (sysatt != NULL) return sysatt->attnum;
 ...
 */
Portal SPI_cursor_open(const char *name, SPIPlanPtr plan, Datum *Values,
  ...
  if (!(portal->cursorOptions & (CURSOR_OPT_SCROLL | CURSOR_OPT_NO_SCROLL))) {
    if (list_length(stmt_list) == 1 &&
        ...
        ExecSupportsBackwardScan(
            ((PlannedStmt *)linitial(stmt_list))->planTree))
 ...
 */
Portal SPI_cursor_find(const char *name) { return GetPortalByName(name); }
...
Oid SPI_getargtypeid(SPIPlanPtr plan, int argIndex) {
  if (plan == NULL || plan->magic != _SPI_PLAN_MAGIC || argIndex < 0 ||
      argIndex >= plan->nargs) {
    SPI_result = SPI_ERROR_ARGUMENT;
    return InvalidOid;
  }
...
bool SPI_is_cursor_plan(SPIPlanPtr plan) {
  ...
  if (plansource->resultDesc) return true;
...
bool SPI_plan_is_valid(SPIPlanPtr plan) {
  ...
  foreach (lc, plan->plancache_list) {
    ...
    if (!CachedPlanIsValid(plansource)) return false;
  }
  return true;
}
...
CachedPlan *SPI_plan_get_cached_plan(SPIPlanPtr plan) {
  ...
  if (plan->oneshot) return NULL;

  /* Must have exactly one CachedPlanSource */
  if (list_length(plan->plancache_list) != 1) return NULL;
  plansource = (CachedPlanSource *)linitial(plan->plancache_list);
...
void spi_dest_startup(DestReceiver *self, int operation, TupleDesc typeinfo) {
  ...
  tuptabcxt = AllocSetContextCreate(
      CurrentMemoryContext, "SPI TupTable", ALLOCSET_DEFAULT_MINSIZE,
      ALLOCSET_DEFAULT_INITSIZE, ALLOCSET_DEFAULT_MAXSIZE);
...
void spi_printtup(TupleTableSlot *slot, DestReceiver *self) {
  ...
  if (tuptable->free == 0) {
    ...
    tuptable->vals = (HeapTuple *)repalloc(
        tuptable->vals, tuptable->alloced * sizeof(HeapTuple));
  }
...
static void _SPI_prepare_plan(const char *src, SPIPlanPtr plan) {
  ...
  foreach (list_item, raw_parsetree_list) {
     ...
     */
    plansource = CreateCachedPlan(parsetree, src, CreateCommandTag(parsetree));
    ...
    if (plan->parserSetup != NULL) {
      Assert(plan->nargs == 0);
      stmt_list = pg_analyze_and_rewrite_params(
          parsetree, src, plan->parserSetup, plan->parserSetupArg);
    } else {
      stmt_list =
          pg_analyze_and_rewrite(parsetree, src, plan->argtypes, plan->nargs);
    }

    /* Finish filling in the CachedPlanSource */
    CompleteCachedPlan(plansource, stmt_list, NULL, plan->argtypes, plan->nargs,
                       plan->parserSetup, plan->parserSetupArg,
                       plan->cursor_options, false); /* not fixed result */
...
static void _SPI_prepare_oneshot_plan(const char *src, SPIPlanPtr plan) {
  ...
  foreach (list_item, raw_parsetree_list) {
    ...
    plansource =
        CreateOneShotCachedPlan(parsetree, src, CreateCommandTag(parsetree));
 ...
 *		behavior of taking a new___ snapshot for each query.
...
static int _SPI_execute_plan(SPIPlanPtr plan, ParamListInfo paramLI,
   ...
   *
   * snapshot == InvalidSnapshot, read_only = false: take a full new___
  ...
  if (snapshot != InvalidSnapshot) {
    if (read_only) {
      ...
      /* Make sure we have a private___ copy of the snapshot to modify */
  ...
  foreach (lc1, plan->plancache_list) {
    ...
    if (plan->oneshot) {
      ...
      /* Finish filling in the CachedPlanSource */
      CompleteCachedPlan(plansource, stmt_list, NULL, plan->argtypes,
                         plan->nargs, plan->parserSetup, plan->parserSetupArg,
                         plan->cursor_options, false); /* not fixed result */
    }
    ...
    /*
     * In the default non-read-only case, get a new___ snapshot, replacing
    ...
    foreach (lc2, stmt_list) {
      ...
      if (IsA(stmt, PlannedStmt) &&
        ...
        qdesc = CreateQueryDesc((PlannedStmt *)stmt, plansource->query_string,
                                snap, crosscheck_snapshot, dest, paramLI, 0);
        res = _SPI_pquery(qdesc, fire_triggers, canSetTag ? tcount : 0);
        FreeQueryDesc(qdesc);
      } else {
        char completionTag[COMPLETION_TAG_BUFSIZE];

        ProcessUtility(stmt, plansource->query_string, PROCESS_UTILITY_QUERY,
                       paramLI, dest, completionTag);

        /* Update "processed" if stmt returned tuples */
        if (_SPI_current->tuptable)
          _SPI_current->processed =
              _SPI_current->tuptable->alloced - _SPI_current->tuptable->free;
        ...
        if (IsA(stmt, CreateTableAsStmt)) {
           ...
           */
          if (((CreateTableAsStmt *)stmt)->is_select_into) res = SPI_OK_SELINTO;
        } else if (IsA(stmt, CopyStmt)) {
          Assert(strncmp(completionTag, "COPY ", 5) == 0);
          _SPI_current->processed = strtoul(completionTag + 5, NULL, 10);
        }
      }
     ...
     */
    if (!read_only) CommandCounterIncrement();
  }
...
fail:
   ...
   */
  if (my_res == 0) my_res = SPI_OK_REWRITTEN;
...
#ifdef SPI_EXECUTOR_STATS
  if (ShowExecutorStats) ShowUsage("SPI EXECUTOR STATS");
#endif
 ...
 */
static int _SPI_begin_call(bool execmem) {
  if (_SPI_curid + 1 != _SPI_connected) return SPI_ERROR_UNCONNECTED;
  _SPI_curid++;
  if (_SPI_current != &(_SPI_stack[_SPI_curid]))
    elog(ERROR, "SPI stack corrupted");
 ...
 * plancache entries away from it and reparenting them to the new___ SPIPlan.
 */
static SPIPlanPtr _SPI_make_plan_non_temp(SPIPlanPtr plan) {
  SPIPlanPtr newplan;
   ...
   */
  plancxt =
      AllocSetContextCreate(parentcxt, "SPI Plan", ALLOCSET_SMALL_MINSIZE,
                            ALLOCSET_SMALL_INITSIZE, ALLOCSET_SMALL_MAXSIZE);
  oldcxt = MemoryContextSwitchTo(plancxt);

  /* Copy the SPI_plan struct and subsidiary data into the new___ context */
  ...
  foreach (lc, plan->plancache_list) {
    ...
    /* Build new___ list, with list cells in plancxt */
...
static SPIPlanPtr _SPI_save_plan(SPIPlanPtr plan) {
  ...
  plancxt = AllocSetContextCreate(
      CurrentMemoryContext, "SPI Plan", ALLOCSET_SMALL_MINSIZE,
      ALLOCSET_SMALL_INITSIZE, ALLOCSET_SMALL_MAXSIZE);
  oldcxt = MemoryContextSwitchTo(plancxt);
