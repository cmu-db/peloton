           ...
           bool canSetTag)
{
   ...
   * rowtype.
   *
   * XXX if we ever wanted to allow users to assign their own OIDs to new___
  ...
  else
  {
    ...
    else
    {
       ...
       *
       * Note: heap_insert returns the tid (location) of the new___ tuple
       * in the t_self field.
       */
      newId = heap_insert(resultRelationDesc, tuple,
                          estate->es_output_cid,
           ...
           bool canSetTag)
{
  ...
  else
  {
     ...
     *
     * If we generate a new___ candidate tuple after EvalPlanQual testing, we
     ...
     * with the heap tuple, all we do is form and insert new___ index tuples.
     * This is because UPDATEs are actually DELETEs and INSERTs, and index
     * tuple deletion is done later by VACUUM (see notes in ExecDelete).
     * All we do here is insert new___ index tuples.  -cim 9/27/89
     */

    /*
     * insert index entries for tuple
     *
     * Note: heap_update returns the tid (location) of the new___ tuple in
     * the t_self field.
     *
     * If it's a HOT update, we mustn't insert new___ index entries.
     */
                     ...
                     TupleTableSlot **returning)
{
  ...
  switch (test)
  {
    ...
    case HeapTupleUpdated:
       ...
       * loop here, as the new___ version of the row might not conflict
   ...
   *
   * This means we only ever continue when a new___ command in the current
  ...
  /* Project the new___ tuple version */
...
ExecInitModifyTable(ModifyTable *node, EState *estate, int eflags)
{
   ...
   * indexes for insertion of new___ index entries.  Note we *must* set
