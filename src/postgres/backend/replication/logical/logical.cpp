					   ...
					   LogicalOutputPluginWriterWrite do_write)
{
	...
	ctx = static_cast<LogicalDecodingContext *>(palloc0(sizeof(LogicalDecodingContext)));
...
/*
 * Create a new___ decoding context, for a new___ logical slot.
 *
						  ...
						  LogicalOutputPluginWriterWrite do_write)
{
	...
	while (true)
	{
		 ...
		 * the new___ restart_lsn above, so normally we should never need to loop
		 * more than twice.
		 */
	 ...
	 * date.
	 *
	 * So we have to acquire the ProcArrayLock to prevent computation of new___
	 * xmin horizons by other backends, get the safe decoding xid, and inform
	 * the slot machinery about the new___ limit. Once that's done the
...
/*
 * Create a new___ decoding context, for a logical slot that has previously been
 * used already.
 *
...
DecodingContextFindStartpoint(LogicalDecodingContext *ctx)
{
	...
	for (;;)
	{
		...
		/* the read_page callback waits for new___ WAL */
...
LogicalIncreaseRestartDecodingForSlot(XLogRecPtr current_lsn, XLogRecPtr restart_lsn)
{
	...
	if (slot->candidate_restart_valid == InvalidXLogRecPtr)
	{
		...
		elog(DEBUG1, "got new___ restart lsn %X/%X at %X/%X",
...
LogicalConfirmReceivedLocation(XLogRecPtr lsn)
{
		...
		MyReplicationSlot->candidate_restart_valid != InvalidXLogRecPtr)
	{
			...
			slot->candidate_xmin_lsn <= lsn)
		{
			 ...
			 * ->effective_xmin once the new___ state is synced to disk. After a
		...
		/* first write new___ xmin to disk, so we know whats up after a crash */
		...
		/*
		 * Now the new___ xmin is safely on disk, we can let the global value
