 ...
 *	  individual chunks stored in WAL. When a new___ (or initial) version of a
...
/*
 * Allocate a new___ ReorderBuffer
 */
ReorderBuffer *
					  ...
					  bool *is_new, XLogRecPtr lsn, bool create_as_top)
{
	...
	{
		/* initialize the new___ entry, if creation was requested */
		Assert(ent != NULL);
		...
		while ((change = ReorderBufferIterTXNNext(rb, iterstate)) != NULL)
		{
			...
			switch (change->action)
			{
				...
				case REORDER_BUFFER_CHANGE_INTERNAL_SNAPSHOT:
					...
					/* and continue with the new___ one */
					SetupHistoricSnapshot(snapshot_now, txn->tuplecid_hash);
					break;
				...
				case REORDER_BUFFER_CHANGE_INTERNAL_COMMAND_ID:
					...
					if (command_id < change->data.command_id)
					{
						 ...
						 * see new___ catalog contents, so execute all
						 * invalidations.
						 */
...
/*
 * Add a new___ snapshot to this transaction that may only used after lsn 'lsn'
...
/*
 * Add new___ (relfilenode, tid) -> (cmin, cmax) mappings.
 */
void
...
restart:
	 ...
	 * apply mapping if so, but only do that once - there can be no new___
