 ...
 * walreceiver.c
 *
 * The WAL receiver process (walreceiver) is new___ as of Postgres 9.0. It
 ...
 * goes into "waiting" mode, and waits for the startup process to give new___
...
WalReceiverMain(void)
{
	...
	// Peloton porting issue: the 2nd arg is false
	...
	for (;;)
	{
		 ...
		 * 'latest', the startup process will scan pg_xlog and find the new___
								  ...
								  slotname[0] != '\0' ? slotname : NULL))
		{
			...
			while (!endofwal)
			{
				...
				{
					/*
					 * We didn't receive anything new___. If we haven't heard
					...
					if (wal_receiver_timeout > 0)
					{
						...
						/*
						 * We didn't receive anything new___, for half of
			...
			/*
			 * If the server had switched to a new___ timeline that we didn't
		 ...
		 * segment, and await for new___ orders from the startup process.
		 */
		if (recvFile >= 0)
		{
		...
		elog(DEBUG1, "walreceiver ended streaming and awaits new___ instructions");
...
XLogWalRcvFlush(bool dying)
{
	if (LogstreamResult.Flush < LogstreamResult.Write)
	{
		...
		/* Signal the startup process and walsender that new___ WAL has arrived */
		WakeupRecovery();
