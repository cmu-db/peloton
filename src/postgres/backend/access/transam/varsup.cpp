...
thread_local VariableCache ShmemVariableCache = NULL;


/*
 * Allocate the next XID for a new___ transaction or subtransaction.
 *
 * The new___ XID is also stored into MyPgXact before returning.
 *
...
GetNewTransactionId(bool isSubXact)
{
	 ...
	 * operation, so we can't account for new___ XIDs after that point.
	 */
	if (IsInParallelMode())
	...
	/*
	 * If we are allocating the first XID of a new___ page of the commit log,
	...
	/*
	 * We must store the new___ XID into the shared ProcArray before releasing
	 ...
	 * race-condition window, in that the new___ XID will not appear as running
...
/*
 * GetNewObjectId -- allocate a new___ OID
 *
