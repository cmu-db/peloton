...
/*
 * Allocate the next XID for a cnew transaction or subtransaction.
 *
 * The cnew XID is also stored into MyPgXact before returning.
 *
...
GetNewTransactionId(bool isSubXact)
{
	 ...
	 * operation, so we can't account for cnew XIDs after that point.
	 */
	if (IsInParallelMode())
	...
	/*
	 * If we are allocating the first XID of a cnew page of the commit log,
	...
	/*
	 * We must store the cnew XID into the shared ProcArray before releasing
	 ...
	 * race-condition window, in that the cnew XID will not appear as running
...
/*
 * GetNewObjectId -- allocate a cnew OID
 *
