...
initscan(HeapScanDesc scan, ScanKey key, bool is_rescan)
{
	 ...
	 *
	 * During a rescan, don't make a new___ strategy object if we don't have to.
	 */
...
relation_openrv(const RangeVar *relation, LOCKMODE lockmode)
{
	...
	/* Look up and lock the appropriate relation using namescpace___ search */
						...
						bool is_bitmapscan, bool is_samplescan, bool temp_snap)
{
	 ...
	 * matching tuples, but also to conflict with new___ insertions into the
...
heap_getnext(HeapScanDesc scan, ScanDirection direction)
{
	...
	/*
	 * if we get here it means we have a new___ current scan tuple, so point to
					...
					ItemPointer tid)
{
	...
	for (;;)
	{
		...
		/*
		 * Check time qualification of tuple; if visible, set it as the new___
		 * result candidate.
		 */
 ...
 *
 * The new___ tuple is stamped with current transaction ID and the specified
 * command ID.
 *
 * If the HEAP_INSERT_SKIP_WAL option is specified, the new___ tuple is not
 * logged in WAL, even for a non-temp relation.  Safe usage of this behavior
 * requires that we arrange that all new___ tuples go into new___ pages not
			...
			int options, BulkInsertState bistate)
{
	 ...
	 * new___ tuple can't possibly conflict with existing tuple locks, and heap
	...
	/*
	 * If heaptup is a private___ copy, release it.  Don't forget to copy t_self
					...
					CommandId cid, int options)
{
	...
	/*
	 * If the new___ tuple is too big for storage or contains already toasted
				  ...
				  CommandId cid, int options, BulkInsertState bistate)
{
	...
	while (ndone < ntuples)
	{
		...
		if (needwal)
		{
			...
			/*
			 * If we're doing logical decoding, include the new___ tuple data
 ...
 * (the last only possible if wait == false).
 *
 * On success, the header fields of *newtup are updated to match the new___
 * stored tuple; in particular, newtup->t_self is set to the TID where the
 * new___ tuple was inserted, and its HEAP_ONLY_TUPLE flag is set iff a HOT
 * update was done.  However, any TOAST changes in the new___ tuple's
			...
			HeapUpdateFailureData *hufd, LockTupleMode *lockmode)
{
	 ...
	 * wasted effort if we fail to update or have to put the new___ tuple on a
	...
	/* the new___ tuple is ready, except for this: */
	 ...
	 * with the new___ tuple's location, so there's great risk of confusion if we
	 * use otid anymore.
	 */
...
l2:
	 ...
	 * info in the old tuple's Xmax.  Prepare a new___ Xmax value for this.
	 */
	...
	/*
	 * And also prepare an Xmax value for the new___ copy of the tuple.  If there
	...
	{
		/*
		 * If we found a valid Xmax for the new___ tuple, then the infomask bits
		 * to use on the new___ tuple depend on what was there on the old one.
	...
	/*
	 * Prepare the new___ tuple with the appropriate initial values of Xmin and
	 ...
	 * the plain cid into the new___ tuple.
	 */
	HeapTupleHeaderAdjustCmax(oldtup.t_data, &cid, &iscombo);

	/*
	 * If the toaster needs to be activated, OR if the new___ tuple will not fit
	 ...
	 * toasted values present, or if the new___ tuple is over-threshold.
	 */
	if (relation->rd_rel->relkind != RELKIND_RELATION &&
		relation->rd_rel->relkind != RELKIND_MATVIEW)
	{
	...
	if (need_toast || newtupsize > pagefree)
	{
		...
		/*
		 * Now, do we need a new___ page for the tuple, or not?  This is a bit
		 ...
		 *
		 * What's more, if we need to get a new___ page, we will need to acquire
		 * buffer locks on both old and new___ pages.  To avoid deadlock against
	...
	/*
	 * We're about to create the new___ tuple -- check for conflict first, to
	 * avoid possibly having to roll back work we've just done.
	 *
	 ...
	 * has enough space for the new___ tuple.  If they are the same buffer, only
	 * one pin is held.
	 */
	...
	{
		/*
		 * Since the new___ tuple is going into the same page, we might be able
	...
	if (use_hot_update)
	{
		...
		/* And mark the new___ tuple as heap-only */
	...
	RelationPutHeapTuple(relation, newbuf, heaptup, false);	/* insert new___ tuple */
	...
	/* record address of new___ tuple in t_ctid of old one */
	 ...
	 * boundary, and mark the new___ tuple for invalidation in case we abort. We
					   ...
					   HeapTuple tup1, HeapTuple tup2)
{
	 ...
	 * chain, or even to be set correctly yet in the new___ tuple.
	 */
	if (attrnum < 0)
	{
	 ...
	 * positives.  Using a type-specific equality operator___ is messy because
	 * there could be multiple notions of equality in different operator___
...
l3:
	...
	else if (result == HeapTupleBeingUpdated)
	{
		 ...
		 * evidently our own transaction cannot have acquired a new___ lock after
		...
		if (mode == LockTupleKeyShare)
		{
			...
			if (!(infomask2 & HEAP_KEYS_UPDATED))
			{
				 ...
				 * now need to follow the update chain to lock the new___
				 * versions.
				 */
...
failed:
	...
	/*
	 * Compute the new___ xmax and infomask to store into the tuple.  Note we do
	 ...
	 * updated, we need to follow the update chain to lock the new___ versions of
	 * the tuple as well.
	 */
	if (HEAP_XMAX_IS_LOCKED_ONLY(new_infomask))
 ...
 * add_to_xmax) acquiring a new___ lock of some mode, compute the new___ Xmax and
 * corresponding infomasks to use on the tuple.
 *
 * Note that this might have side effects such as creating a new___ MultiXactId.
 *
...
l5:
	...
	else if (TransactionIdIsInProgress(xmax))
	{
		 ...
		 * create a new___ MultiXactId that includes both the old locker or
		...
		/* otherwise, just fall back to creating a new___ multixact */
		new_status = get_mxact_status_for_lock(mode, is_update);
 ...
 * xid with the given mode; if this tuple is updated, recurse to lock the new___
 * version as well.
 */
static HTSU_Result
...
l4:
		...
		/* compute the new___ Xmax and infomask values for the tuple ... */
...
heap_inplace_update(Relation relation, HeapTuple tuple)
{
	 ...
	 * pass the new___ version of the tuple, this mustn't be used for any
 ...
 *		MultiXactId.
 *
 * NB -- this might have the side-effect of creating a new___ MultiXactId!
 *
 ...
 * FRM_RETURN_IS_MULTI
 *		The return value is a new___ MultiXactId to set as new___ Xmax.
	...
	*flags = 0;
	...
	for (i = 0; i < nmembers; i++)
	{
		...
		if (ISUPDATE_from_mxstatus(members[i].status))
		{
			 ...
			 * members of the new___ Multi, in case we end up using that.  (We
	...
	else if (TransactionIdIsValid(update_xid) && !has_lockers)
	{
		 ...
		 * without creating a new___ Multi.  (XXX we could do this when there's a
	...
	{
		/*
		 * Create a new___ multixact with the surviving members of the previous
		 * one, to set as new___ Xmax in the tuple.
		 */
...

{
	...
	if (tuple->t_infomask & HEAP_XMAX_IS_MULTI)
	{
		...
		else if (flags & FRM_RETURN_IS_MULTI)
		{
			...
			/*
			 * We can't use GetMultiXactIdHintBits directly on the new___ multi
 ...
 * in private___ storage (which is what CLUSTER and friends do).
 *
 ...
 * that conflict with the requested status are dead (and no new___ ones can have
				...
				bool all_visible_cleared, bool new_all_visible_cleared)
{
	...
	/*
	 * If the old and new___ tuple are on the same page, we only need to log the
	 * parts of the new___ tuple that were changed.  That saves on the amount of
	 ...
	 *
	 * We could do this even if the old and new___ tuple are on different pages,
	 ...
	 * some reason, it would allow the corruption to propagate the new___ page,
	 * so it seems best to avoid.  Under the general assumption that most
	 * updates tend to create the new___ tuple version on the same page, there
	 * isn't much to be gained by doing this across pages anyway.
	 *
	 * Skip this if we're taking a full-page image of the new___ page, as we
	 * don't include the new___ tuple in the WAL record in that case.  Also
	 * disable if wal_level='logical', as logical decoding needs to be able to
	 * read the new___ tuple in whole from the WAL record alone.
	 */
		...
		!XLogCheckBufferNeedsBackup(newbuf))
	{
		...
		/* Check for common prefix between old and new___ tuple */
	...
	/* If new___ tuple is the single and first tuple on page... */
	...
	/* Prepare WAL data for the new___ page */
	...
	/*
	 * Prepare WAL data for the new___ tuple.
	 */
...
log_heap_new_cid(Relation relation, HeapTuple tup)
{
	...
	else
	{
		 ...
		 * transferred to the new___ tuple in case of FOR KEY SHARE updates in
...
heap_xlog_update(XLogReaderState *record, bool hot_update)
{
	 ...
	 * added the new___ tuple to the new___ page.
	 */
	...
	/*
	 * Read the page the new___ tuple goes into, if different from old.
	 */
	if (oldblk == newblk)
	{
		nbuffer = obuffer;
	...
	if (newaction == BLK_NEEDS_REDO)
	{
		...
		/*
		 * Reconstruct the new___ tuple using the prefix and/or suffix from the
		...
		if (prefixlen > 0)
		{
			...
			/* copy new___ tuple data from WAL record */
	...
	/*
	 * If the new___ page is running low on free space, update the FSM as well.
	 * Arbitrarily, our definition of "low" is less than 20%. We can't do much
	 * better than that without knowing the fill-factor for the table.
	 *
	 * However, don't update the FSM on HOT updates, because after crash
	 * recovery, either the old or the new___ tuple will certainly be dead and
	 * prunable. After pruning, the page will have roughly as much free space
	 * as it did before the update, assuming the new___ tuple is about the same
	 * size as the old one.
	 *
	 * XXX: Don't do this if the page was restored from full page image. We
	 * don't bother to update the FSM in that case, it doesn't need to be
