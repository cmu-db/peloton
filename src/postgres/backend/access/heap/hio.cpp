 ...
 *	same buffer we select for insertion of the new___ tuple (this could only
 ...
 *	if HEAP_INSERT_SKIP_FSM is specified, we just append a new___ empty page to
 *	the end of the relation if the tuple won't fit on the current target page.
 *	This can save some cycles when we know the relation is new___ and doesn't
						  ...
						  Buffer *vmbuffer, Buffer *vmbuffer_other)
{
	 ...
	 * same time, else we will both try to initialize the same new___ page.  We
	 * can skip locking for new___ or temp relations, however, since no one else
	...
	/*
	 * Now acquire lock on the new___ page.
	 */
	LockBuffer(buffer, BUFFER_LOCK_EXCLUSIVE);
	 ...
	 * we have buffer lock on the new___ page, or we risk a race condition
	...
	/*
	 * We need to initialize the empty new___ page.  Double-check that it really
	...
	/*
	 * Remember the new___ page as our target for future insertions.
	 *
	 * XXX should we enter the new___ page into the free space map immediately,
