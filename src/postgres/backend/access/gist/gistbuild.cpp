				...
				BlockNumber startblkno, int startlevel)
{
	...
	if (LEVEL_HAS_BUFFERS(level, gfbb))
	{
		...
		/* Find the buffer or create a new___ one */
 ...
 *
 * Returns the block number of the page where the (first) new___ or updated tuple
						  ...
						  BlockNumber parentblk, OffsetNumber downlinkoffnum)
{
	...
	if (is_split && BufferGetBlockNumber(buffer) == GIST_ROOT_BLKNO)
	{
		 ...
		 * pages. Visit all the new___ child pages to memorize the parents of the
		 * grandchildren.
		 */
		if (gfbb->rootlevel > 1)
		{
			...
			for (off = FirstOffsetNumber; off <= maxoff; off++)
			{
				...
				/*
				 * Also remember that the parent of the new___ child page is the
				 * root block.
				 */
...
gistProcessEmptyingQueue(GISTBuildState *buildstate)
{
	...
	while (gfbb->bufferEmptyingQueue != NIL)
	{
		...
		while (true)
		{
			 ...
			 * likely that the new___ left buffer is no longer over the half-full
...
gistEmptyAllBuffers(GISTBuildState *buildstate)
{
	...
	{
		/*
		 * Empty all buffers on this level. Note that new___ buffers can pop up
 ...
 * the parent of the new___ child page needs to be recorded, and also the
 * entries for all page whose downlinks are moved to a new___ page at the split
 * needs to be updated.
 *
