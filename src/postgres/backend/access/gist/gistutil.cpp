...
gistgetadjusted(Relation r, IndexTuple oldtup, IndexTuple addtup, GISTSTATE *giststate)
{
	...
	for (i = 0; i < r->rd_att->natts; i++)
	{
		...
		if (neednew)
			/* we already need new___ key, so we can skip check */
			continue;
 ...
 * of the new___ index key contained in "it".
 *
		   ...
		   GISTSTATE *giststate)
{
	 ...
	 * to the old best, or use the new___ one.  Once we decide to stick to the
	...
	for (i = FirstOffsetNumber; i <= maxoff; i = OffsetNumberNext(i))
	{
		...
		for (j = 0; j < r->rd_att->natts; j++)
		{
			...
			if (best_penalty[j] < 0 || usize < best_penalty[j])
			{
				...
				/* we have new___ best, so reset keep-it decision */
				keep_current_best = -1;
			}
		...
		if (j == r->rd_att->natts && result != i)
		{
			...
			{
				/* we choose to use the new___ tuple */
				result = i;
 ...
 * returns new___ IndexTuple that contains GISTENTRY with fetched data
 */
IndexTuple
...
/*
 * Initialize a new___ index page
 */
void
