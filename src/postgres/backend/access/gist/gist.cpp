 ...
 * at that offset is atomically removed along with inserting the new___ tuples.
 * This is used to replace a tuple with a new___ one.
 *
 ...
 * pages are released; note that new___ tuple(s) are *not* on the root page
 * but in one of the new___ child pages.
 *
 * If 'newblkno' is not NULL, returns the block number of page the first
 * new___/updated tuple was inserted to. Usually it's the given page, but could
				...
				bool markfollowright)
{
	...
	*splitinfo = NIL;
	...
	if (is_split)
	{
		...
		/*
		 * Set up pages to work with. Allocate new___ buffers for all but the
		 * leftmost page. The original page becomes the new___ leftmost page, and
		 * is just replaced with the new___ contents.
		 *
		 * For a root-split, allocate new___ buffers for all child pages, the
		 * original page is overwritten with new___ root page containing
		 * downlinks to the new___ child pages.
		 */
		ptr = dist;
		if (!is_rootsplit)
		{
		...
		/*
		 * Now that we know which blocks the new___ pages go to, set up downlink
		...
		/*
		 * If this is a root split, we construct the new___ root page with the
		 * downlinks here directly, instead of requiring the caller to insert
		 * them. Add the new___ root page to the list along with the child pages.
		 */
		if (is_rootsplit)
		{
		...
		/*
		 * Fill all pages. All the pages are new___, ie. freshly allocated empty
		...
		/*
		 * Return the new___ child buffers to the caller.
		 *
		 * If this was a root split, we've already inserted the downlink
		 * pointers, in the form of a new___ root page. Therefore we can release
		 * all the new___ buffers, and keep just the root page locked.
		 */
		if (is_rootsplit)
		{
	...
	for (;;)
	{
		...
		if (!GistPageIsLeaf(stack->page))
		{
			...
			if (newtup)
			{
				 ...
				 * descend back to the half that's a better fit for the new___
				 * tuple.
				 */
		...
		{
			/*
			 * Leaf page. Insert the new___ key. We've already updated all the
 ...
 *
 * returns a new___ insertion stack, starting from the parent of "child", up
...
gistFindCorrectParent(Relation r, GISTInsertStack *child)
{
		...
		parent->lsn != PageGetLSN(parent->page))
	{
		...
		/* install new___ chain of parents to stack */
		child->parent = parent;
...
gistfixsplit(GISTInsertState *state, GISTSTATE *giststate)
{
	...
	for (;;)
	{
		...
		/* Form the new___ downlink tuples to insert to parent */
 ...
 * tuple at 'oldoffnum' is replaced, otherwise the tuple is inserted as new___.
				...
				GISTSTATE *giststate, List *splitinfo, bool unlockbuf)
{
	...
	/*
	 * We need to insert downlinks for each new___ page, and update the downlink
	 ...
	 * left. Finally insert the downlink for the last new___ page and update the
