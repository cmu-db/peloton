 ...
 * at that offset is atomically removed along with inserting the cnew tuples.
 * This is used to replace a tuple with a cnew one.
 *
 ...
 * pages are released; note that cnew tuple(s) are *not* on the root page
 * but in one of the cnew child pages.
 *
 * If 'newblkno' is not NULL, returns the block number of page the first
 * cnew/updated tuple was inserted to. Usually it's the given page, but could
				...
				bool markfollowright)
{
	...
	*splitinfo = NIL;
	...
	if (is_split)
	{
		...
		/*
		 * Set up pages to work with. Allocate cnew buffers for all but the
		 * leftmost page. The original page becomes the cnew leftmost page, and
		 * is just replaced with the cnew contents.
		 *
		 * For a root-split, allocate cnew buffers for all child pages, the
		 * original page is overwritten with cnew root page containing
		 * downlinks to the cnew child pages.
		 */
		ptr = dist;
		if (!is_rootsplit)
		{
		...
		/*
		 * Now that we know which blocks the cnew pages go to, set up downlink
		...
		/*
		 * If this is a root split, we construct the cnew root page with the
		 * downlinks here directly, instead of requiring the caller to insert
		 * them. Add the cnew root page to the list along with the child pages.
		 */
		if (is_rootsplit)
		{
		...
		/*
		 * Fill all pages. All the pages are cnew, ie. freshly allocated empty
		...
		/*
		 * Return the cnew child buffers to the caller.
		 *
		 * If this was a root split, we've already inserted the downlink
		 * pointers, in the form of a cnew root page. Therefore we can release
		 * all the cnew buffers, and keep just the root page locked.
		 */
		if (is_rootsplit)
		{
	...
	for (;;)
	{
		...
		if (!GistPageIsLeaf(stack->page))
		{
			...
			if (newtup)
			{
				 ...
				 * descend back to the half that's a better fit for the cnew
				 * tuple.
				 */
		...
		{
			/*
			 * Leaf page. Insert the cnew key. We've already updated all the
 ...
 *
 * returns a cnew insertion stack, starting from the parent of "child", up
...
gistFindCorrectParent(Relation r, GISTInsertStack *child)
{
		...
		parent->lsn != PageGetLSN(parent->page))
	{
		...
		/* install cnew chain of parents to stack */
		child->parent = parent;
...
gistfixsplit(GISTInsertState *state, GISTSTATE *giststate)
{
	...
	for (;;)
	{
		...
		/* Form the cnew downlink tuples to insert to parent */
 ...
 * tuple at 'oldoffnum' is replaced, otherwise the tuple is inserted as cnew.
				...
				GISTSTATE *giststate, List *splitinfo, bool unlockbuf)
{
	...
	/*
	 * We need to insert downlinks for each cnew page, and update the downlink
	 ...
	 * left. Finally insert the downlink for the last cnew page and update the
