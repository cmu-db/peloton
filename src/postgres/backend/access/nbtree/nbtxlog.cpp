...
btree_xlog_split(bool onleft, bool isroot, XLogReaderState *record)
{
	...
	/* Reconstruct right (new___) sibling page from scratch */
	...
	if (XLogReadBufferForRedo(record, 0, &lbuf) == BLK_NEEDS_REDO)
	{
		...
		for (off = P_FIRSTDATAKEY(lopaque); off < xlrec->firstright; off++)
		{
			...
			/* add the new___ item if it was inserted on left page */
			if (onleft && off == xlrec->newitemoff)
			{
				if (PageAddItem(newlpage, newitem, newitemsz, leftoff,
								false, false) == InvalidOffsetNumber)
					elog(ERROR, "failed to add new___ item to left page after split");
				leftoff = OffsetNumberNext(leftoff);
			}
	...
	/*
	 * Fix left-link of the page to the right of the new___ right sibling.
	 *
