 ...
 *
 * Note that new___ items are always appended at the end of the list; this
...
spgAddPendingTID(spgBulkDeleteState *bds, ItemPointer tid)
{
	...
	/* not there, so append new___ entry */
			   ...
			   bool forPending)
{
	 ...
	 * moveDest[]: new___ locations for moveSrc tuples
	 * chainSrc[]: tuple numbers whose chain links (nextOffset) need updates
	 * chainDest[]: new___ values of nextOffset for chainSrc members
	 *
...
spgvacuumpage(spgBulkDeleteState *bds, BlockNumber blkno)
{
	 ...
	 * a new___ tuple.  Otherwise, check for empty/deletable page, and make sure
	 * FSM knows about it.
	 */
	if (!SpGistBlockIsRoot(blkno))
	{
...
spgvacuumscan(spgBulkDeleteState *bds)
{
	...
	/* We can skip locking for new___ or temp relations */
