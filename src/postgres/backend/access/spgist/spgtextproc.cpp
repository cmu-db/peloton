 ...
 * but no new___ entries will ever be pushed down into a zero-labeled child.
 * No new___ entries ever get pushed into a -2-labeled child, either.
 *
 *
...
spg_text_inner_consistent(PG_FUNCTION_ARGS)
{
	...
	for (i = 0; i < in->nNodes; i++)
	{
		...
		for (j = 0; j < in->nkeys; j++)
		{
			...
			/*
			 * If it's a collation-aware operator___, but the collation is C, we
