...
typedef struct
{
	 ...
	 *				ignored
	 * INSERT		this is a completely new___ segment
	 * REPLACE		this replaces an existing segment with new___ content
					...
					void *insertdata, Page *newlpage, Page *newrpage)
{
	...
	/*
	 * Count how many of the new___ items belong to this page.
	 */
	if (!GinPageRightMost(page))
	{
	...
	/*
	 * Are we appending to the end of the page? IOW, are all the new___ items
	 ...
	 * Otherwise we have to limit the number of new___ items to insert, because
	...
	if (append)
	{
		 ...
		 * not completely free, because we will merge the new___ items and old
		 * items into an array below. In the best case, every new___ item fits in
		 * a single byte, and we can use all the free space on the old page as
		 * well as the new___ page. For simplicity, ignore segment overhead etc.
		 */
	...
	{
		/*
		 * Calculate a conservative estimate of how many new___ items we can fit
		 * on the two pages after splitting.
		 *
		 * We can use any remaining free space on the old page to store full
		 * segments, as well as the new___ page. Each full-sized segment can hold
	...
	/* Add the new___ items to the segments */
	...
	if (ItemPointerIsValid(&remaining))
	{
		...
		/* Count how many of the new___ items did fit. */
		for (i = 0; i < maxitems; i++)
		{
		...
		if (i == 0)
			elog(ERROR, "could not split GIN page; no new___ items fit");
		maxitems = i;
	}
	...
	if (!needsplit)
	{
		...
		if (append)
			elog(DEBUG2, "appended %d new___ items to block %u; %d bytes (%d to go)",
				 maxitems, BufferGetBlockNumber(buf), (int) leaf->lsize,
				 items->nitem - items->curitem - maxitems);
		else
			elog(DEBUG2, "inserted %d new___ items to block %u; %d bytes (%d to go)",
				 maxitems, BufferGetBlockNumber(buf), (int) leaf->lsize,
				 items->nitem - items->curitem - maxitems);
	}
	else
	{
		 ...
		 * put the rest to the right page. When building a new___ index, that's
 ...
 * the original page directly, but to temporary in-memory copies of the new___
						 ...
						 Page lpage, Page rpage)
{
	...
	/* Initialize temporary pages to hold the new___ left and right pages */
...
/*
 * Split page and fill WAL record. Returns a new___ temp buffer filled with data
					  ...
					  Page *newlpage, Page *newrpage)
{
	...
	/*
	 * First construct a new___ list of PostingItems, which includes all the old
	 * items, and the new___ item.
	 */
	...
	/*
	 * When creating a new___ index, fit as many tuples as possible on the left
...
/*
 * Fills new___ root by right bound values from child.
 ...
 *
 * Any segments that acquire new___ items are decoded, and the new___ items are
 * merged with the old items.
 *
 * Returns true if any new___ items were added. False means they were all
...
addItemsToLeaf(disassembledLeaf *leaf, ItemPointer newItems, int nNewItems)
{
	...
	/*
	 * If the page is completely empty, just construct one new___ segment to hold
	 * all the new___ items.
	 */
	if (dlist_is_empty(&leaf->segments))
	{
	...
	dlist_foreach(iter, &leaf->segments)
	{
		...
		/*
		 * How many of the new___ items fall into this segment?
		 */
		...
		/* Merge the new___ items with the existing items. */
		if (!cur->items)
		 ...
		 * larger than the target, create a new___ segment before that happens.
		 */
		if (!dlist_has_next(&leaf->segments, iter.cur) &&
		...
		if (seginfo->action != GIN_SEGMENT_DELETE)
		{
			if (seginfo->seg == NULL)
			{
				...
				if (npacked != seginfo->nitems)
				{
					 ...
					 * create a new___ segment to represent the remaining items.
					 * The new___ segment is inserted after this one, so it will
...
/*
 * Creates new___ posting tree containing the given TIDs. Returns the page
 * number of the root of the new___ posting tree.
 *
				  ...
				  GinStatsData *buildStats)
{
	...
	/* Construct the new___ root page in memory first. */
	...
	/*
	 * All set. Get a new___ physical page, and copy the in-memory page to it.
	 */
...
/*
 * Starts a new___ scan on a posting tree.
 */
GinBtreeStack *
