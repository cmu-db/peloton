...
processCancelRequest(Port *port, void *pkt)
{
  ...
  BackendStartup(Port *port)
  {
    ...
    if (pid < 0)
    {
      ...
      errno = save_errno;
      ereport(LOG,
              (errmsg("could not fork new___ process for connection: %m")));
    ...
    ereport(DEBUG2,
            (errmsg_internal("forked new___ backend, pid=%d socket=%d",
  ...
  report_fork_failure_to_client(Port *port, int errnum)
  {
    ...
    snprintf(buffer, sizeof(buffer), "E%s%s\n",
             _("could not fork new___ process for connection: "),
             strerror(errnum));
