 ...
 *
 * If no entry, return NULL, don't create a new___ one
 */
...
pgstat_end_function_usage(PgStat_FunctionCallUsage *fcu, bool finalize)
{
	...
	/*
	 * Compute the new___ f_total_time as the total elapsed time added to the
...
get_tabstat_entry(Oid rel_id, bool isshared)
{
	...
	/*
	 * Use the first entry of the new___ TabStatusArray.
	 */
	entry = &tsa->tsa_entries[tsa->tsa_used++];
	entry->t_id = rel_id;
	entry->t_shared = isshared;
...
/*
 * get_tabstat_stack_level - add a new___ (sub)transaction stack entry if needed
 */
...
/*
 * add_tabstat_xact_level - add a new___ (sub)transaction state record
 */
static void
		...
		for (trans = xact_state->first; trans != NULL; trans = next_trans)
		{
			...
			if (isCommit)
			{
				...
				else
				{
					 ...
					 * pushing a new___ entry into the pgStatXactStack.
					 */
...
PgstatCollectorMain(int argc, char *argv[])
{
	...
	for (;;)
	{
		...
		while (!need_exit)
		{
			...
			/*
			 * Write the stats file if a new___ request has arrived that is not
...
pgstat_get_db_entry(Oid databaseid, bool create)
{
	...
	/*
	 * If not found, initialize the new___ one.  This creates empty hash tables
...
pgstat_get_tab_entry(PgStat_StatDBEntry *dbentry, Oid tableoid, bool create)
{
	...
	/* If not found, initialize the new___ one. */
	if (!found)
	{
		result->numscans = 0;
 ...
 *	starting up under a new___ postmaster can't read the old data before
 *	the new___ collector is ready.
 *
 ...
 *
 *	- if there's no db stat entry (e.g. for a new___ or inactive database),
 ...
 *	request will cause new___ snapshots to be read.
 *
...
pgstat_recv_inquiry(PgStat_MsgInquiry *msg, int len)
{
	...
	if ((dbentry != NULL) && (msg->clock_time < dbentry->stats_timestamp))
	{
		...
		{
			/*
			 * Sure enough, time went backwards.  Force a new___ stats file write
...
pgstat_recv_tabstat(PgStat_MsgTabstat *msg, int len)
{
	...
	for (i = 0; i < msg->m_nentries; i++)
	{
		...
		{
			/*
			 * If it's a new___ table entry, initialize counters to the values we
			 * just got.
			 */
...
pgstat_recv_funcstat(PgStat_MsgFuncstat *msg, int len)
{
	...
	for (i = 0; i < msg->m_nentries; i++, funcmsg++)
	{
		...
		{
			/*
			 * If it's a new___ function entry, initialize counters to the values
			 * we just got.
			 */
