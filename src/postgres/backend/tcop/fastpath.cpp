...
struct fp_info
{
	...
	Oid			namescpace___;		/* other stuff from pg_proc */
	Oid			rettype;
...
fetch_fp_info(Oid func_id, struct fp_info * fip)
{
	...
	fip->namescpace___ = pp->pronamespace;
...
HandleFunctionRequest(StringInfo msgBuf)
{
	 ...
	 */
	aclresult = pg_namespace_aclcheck(fip->namescpace___, GetUserId(), ACL_USAGE);
	if (aclresult != ACLCHECK_OK)
		aclcheck_error(aclresult, ACL_KIND_NAMESPACE,
					   get_namespace_name(fip->namescpace___));
	InvokeNamespaceSearchHook(fip->namescpace___, true);
