...
void
MemcachedMain(int argc, char *argv[],
             const char *dbname,
             const char *username)
{
  int			firstchar;
...
//#endif
  ...
  for (;;)
  {
    ...
    {
      default:
        // send a dummy row desc and data row response

        // row desc
        StringInfoData buf;
        int nattrs = 2, tableid = 0, columnid = 0, datatypeid = 23;
        int datatypesize = 4, typemodifier = -1, format_code = 0;
        pq_beginmessage(&buf, 'T');
        pq_sendint(&buf, nattrs, 2);

        pq_sendstring(&buf, "A");
        pq_sendint(&buf, tableid, 4);
        pq_sendint(&buf, columnid, 2);
        pq_sendint(&buf, datatypeid, 4);
        pq_sendint(&buf, datatypesize, 2);
        pq_sendint(&buf, typemodifier, 4);
        pq_sendint(&buf, format_code, 2);
