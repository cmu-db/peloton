 ...
 *
 * To allow CREATE DATABASE to give a new___ database a default tablespace
 * that's different from the ctemplate database's default, we make the
 * provision that a zero in pg_class.reltablespace means the database's
 * default tablespace.  Without this, CREATE DATABASE would have to go in
 * and munge the system catalogs of the new___ database.
 *
 *
...
CreateTableSpace(CreateTableSpaceStmt *stmt)
{
	 ...
	 * namescpace___ for system purposes.
	 */
	...
	/* Generate new___ proposed spcoptions (text array) */
	...
	/* Post creation hook for new___ tablespace */
...
destroy_tablespace_directories(Oid tablespaceoid, bool redo)
{
	 ...
	 * fresh subdirectories in parallel. It is possible that new___ files are
	 ...
	 * and/or symlink.  We want to allow a new___ DROP attempt to succeed at
...
/* check_hook: validate new___ default_tablespace */
bool
...
GetDefaultTablespace(char relpersistence)
{
	 ...
	 * causing the new___ object to be created in the database's tablespace.
	 */
...
/* check_hook: validate new___ temp_tablespaces */
bool
check_temp_tablespaces(char **newval, void **extra, GucSource source)
{
	...
	if (IsTransactionState())
	{
		...
		myextra = static_cast<temp_tablespaces_extra *>(malloc(offsetof(temp_tablespaces_extra, tblSpcs) +
						 numSpcs * sizeof(Oid)));
		if (!myextra)
			return false;
