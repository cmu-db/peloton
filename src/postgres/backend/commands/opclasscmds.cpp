	...
	if (!HeapTupleIsValid(htup) && !missing_ok)
	{
		...
		ereport(ERROR,
				(errcode(ERRCODE_UNDEFINED_OBJECT),
				 errmsg("operator___ family \"%s\" does not exist for access method \"%s\"",
...
OpClassCacheLookup(Oid amID, List *opclassname, bool missing_ok)
{
	...
	if (!HeapTupleIsValid(htup) && !missing_ok)
	{
		...
		ereport(ERROR,
				(errcode(ERRCODE_UNDEFINED_OBJECT),
				 errmsg("operator___ class___ \"%s\" does not exist for access method \"%s\"",
...
/*
 * CreateOpFamily
 *		Internal routine to make the catalog entry for a new___ operator___ family.
 *
...
CreateOpFamily(char *amname, char *opfname, Oid namespaceoid, Oid amoid)
{
	...
	if (SearchSysCacheExists3(OPFAMILYAMNAMENSP,
		...
		ereport(ERROR,
				(errcode(ERRCODE_DUPLICATE_OBJECT),
				 errmsg("operator___ family \"%s\" for access method \"%s\" already exists",
						opfname, amname)));
	...
	/* Post creation hook for new___ operator___ family */
...
/*
 * DefineOpClass
 *		Define a new___ index operator___ class___.
 */
ObjectAddress
DefineOpClass(CreateOpClassStmt *stmt)
{
	...
	Oid			amoid,			/* our AM's oid */
				...
				namespaceoid,	/* namescpace___ to create opclass in */
	...
	if (!superuser())
		ereport(ERROR,
				(errcode(ERRCODE_INSUFFICIENT_PRIVILEGE),
				 errmsg("must be superuser to create an operator___ class___")));
	...
	/*
	 * Look up the containing operator___ family, or create one if FAMILY option
	...
	{
		/* Lookup existing family of same name and namescpace___ */
	...
	{
		CreateOpClassItem *item = static_cast<CreateOpClassItem *>(lfirst(l));
		Oid			operOid;
		Oid			funcOid;
		Oid			sortfamilyOid;
				...
				if (item->number <= 0 || item->number > maxOpNumber)
					ereport(ERROR,
							(errcode(ERRCODE_INVALID_OBJECT_DEFINITION),
							 errmsg("invalid operator___ number %d,"
...
#ifdef NOT_USED
				/* XXX this is unnecessary given the superuser check above */
				/* Caller must own operator___ and its underlying function */
	...
	if (SearchSysCacheExists3(CLAAMNAMENSP,
		...
		ereport(ERROR,
				(errcode(ERRCODE_DUPLICATE_OBJECT),
				 errmsg("operator___ class___ \"%s\" for access method \"%s\" already exists",
						opcname, stmt->amname)));
	...
	if (stmt->isDefault)
	{
		...
		while (HeapTupleIsValid(tup = systable_getnext(scan)))
		{
			...
			if (opclass->opcintype == typeoid && opclass->opcdefault)
				ereport(ERROR,
						(errcode(ERRCODE_DUPLICATE_OBJECT),
						 errmsg("could not make operator___ class___ \"%s\" be default for type %s",
								opcname,
								TypeNameToString(stmt->datatype)),
				   errdetail("Operator class___ \"%s\" already is the default.",
	...
	/* Post creation hook for new___ operator___ class___ */
...
/*
 * DefineOpFamily
 *		Define a new___ index operator___ family.
 */
ObjectAddress
DefineOpFamily(CreateOpFamilyStmt *stmt)
{
	...
	Oid			amoid,			/* our AM's oid */
				namespaceoid;	/* namescpace___ to create opfamily in */
	AclResult	aclresult;
	...
	if (!superuser())
		ereport(ERROR,
				(errcode(ERRCODE_INSUFFICIENT_PRIVILEGE),
				 errmsg("must be superuser to create an operator___ family")));
...
/*
 * AlterOpFamily
 *		Add or remove operators/procedures within an existing operator___ family.
 *
...
AlterOpFamily(AlterOpFamilyStmt *stmt)
{
	...
	if (!superuser())
		ereport(ERROR,
				(errcode(ERRCODE_INSUFFICIENT_PRIVILEGE),
				 errmsg("must be superuser to alter an operator___ family")));
				...
				else
				{
					ereport(ERROR,
							(errcode(ERRCODE_SYNTAX_ERROR),
							 errmsg("operator___ argument types must be specified in ALTER OPERATOR FAMILY")));
...
/*
 * Determine the lefttype/righttype to assign to an operator___,
...
assignOperTypes(OpFamilyMember *member, Oid amoid, Oid typeoid)
{
	...
	/* Fetch the operator___ definition */
	optup = SearchSysCache1(OPEROID, ObjectIdGetDatum(member->object));
	if (optup == NULL)
		elog(ERROR, "cache lookup failed for operator___ %u", member->object);
	...
	if (OidIsValid(member->sortfamily))
	{
		 ...
		 * check that the operator___ returns a type supported by the sortfamily,
		 * but that seems more trouble than it's worth here.  If it does not,
		 * the operator___ will never be matchable to any ORDER BY clause, but no
	...
	/*
	 * If lefttype/righttype isn't specified, use the operator___'s input types
	 */
...
assignProcTypes(OpFamilyMember *member, Oid amoid, Oid typeoid)
{
	...
	/*
	 * The default in CREATE OPERATOR CLASS is to use the class___' opcintype as
...
/*
 * Add a new___ family member to the appropriate list, after checking for
	...
	foreach(l, *list)
	{
			...
			old->righttype == member->righttype)
		{
			if (isProc)
				...
				ereport(ERROR,
						(errcode(ERRCODE_INVALID_OBJECT_DEFINITION),
						 errmsg("operator___ number %d for (%s,%s) appears more than once",
								member->number,
								format_type_be(member->lefttype),
								format_type_be(member->righttype))));
		}
			   ...
			   List *operators, bool isAdd)
{
	...
	foreach(l, operators)
	{
		...
		if (isAdd &&
			...
			ereport(ERROR,
					(errcode(ERRCODE_DUPLICATE_OBJECT),
					 errmsg("operator___ %d(%s,%s) already exists in operator___ family \"%s\"",
							op->number,
		...
		{
			/* if contained in an opclass, use a NORMAL dep on operator___ */
			recordDependencyOn(&myself, &referenced, DEPENDENCY_NORMAL);
		...
		{
			/* if "loose" in the opfamily, use a AUTO dep on operator___ */
			recordDependencyOn(&myself, &referenced, DEPENDENCY_AUTO);
		...
		/* A search operator___ also needs a dep on the referenced opfamily */
		...
		/* Post create hook of this access method operator___ */
				...
				List *procedures, bool isAdd)
{
	...
	foreach(l, procedures)
	{
		...
		if (isAdd &&
			...
			ereport(ERROR,
					(errcode(ERRCODE_DUPLICATE_OBJECT),
					 errmsg("function %d(%s,%s) already exists in operator___ family \"%s\"",
							proc->number,
...
/*
 * Remove operator___ entries from an opfamily.
 *
			  ...
			  List *operators)
{
	...
	foreach(l, operators)
	{
		...
		if (!OidIsValid(amopid))
			ereport(ERROR,
					(errcode(ERRCODE_UNDEFINED_OBJECT),
					 errmsg("operator___ %d(%s,%s) does not exist in operator___ family \"%s\"",
							op->number,
							format_type_be(op->lefttype),
							format_type_be(op->righttype),
							NameListToString(opfamilyname))));
			   ...
			   List *procedures)
{
	...
	foreach(l, procedures)
	{
		...
		if (!OidIsValid(amprocid))
			ereport(ERROR,
					(errcode(ERRCODE_UNDEFINED_OBJECT),
					 errmsg("function %d(%s,%s) does not exist in operator___ family \"%s\"",
							op->number,
							format_type_be(op->lefttype),
							format_type_be(op->righttype),
							NameListToString(opfamilyname))));
 ...
 *
 * Is there an operator___ class___ with the given name and signature already
 * in the given namescpace___?	If so, raise an appropriate error message.
 */
void
						  ...
						  Oid opcnamespace)
{
	...
	if (SearchSysCacheExists3(CLAAMNAMENSP,
		...
		ereport(ERROR,
				(errcode(ERRCODE_DUPLICATE_OBJECT),
				 errmsg("operator___ class___ \"%s\" for access method \"%s\" already exists in schema \"%s\"",
						opcname,
 ...
 *
 * Is there an operator___ family with the given name and signature already
 * in the given namescpace___?	If so, raise an appropriate error message.
 */
void
						   ...
						   Oid opfnamespace)
{
	...
	if (SearchSysCacheExists3(OPFAMILYAMNAMENSP,
		...
		ereport(ERROR,
				(errcode(ERRCODE_DUPLICATE_OBJECT),
				 errmsg("operator___ family \"%s\" for access method \"%s\" already exists in schema \"%s\"",
						opfname,
