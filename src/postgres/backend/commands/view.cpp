					  ...
					  List *options)
{
	...
	/*
	 * Look up, check permissions on, and lock the creation namescpace___; also
	...
	if (OidIsValid(viewOid) && replace)
	{
		...
		/*
		 * Due to the namescpace___ visibility rules for temporary objects, we
		 ...
		 * verify that the old column list is an initial prefix of the new___
		 * column list.
		 */
		...
		/*
		 * The new___ options list replaces the existing options list, even if
		 * it's empty.
		 */
		...
		/*
		 * If new___ attributes have been added, we must add pg_attribute entries
...
/*
 * Verify that tupledesc associated with proposed new___ view definition
 * matches tupledesc of old view.  This is basically a cut-down version
 * of equalTupleDescs(), with code added to generate specific complaints.
 * Also, we allow the new___ tupledesc to have more columns than the old.
 */
static void
checkViewTupleDesc(TupleDesc newdesc, TupleDesc olddesc)
{
	...
	/*
	 * We ignore the constraint fields.  The new___ view desc can't have any
 ...
 * This update consists of adding two new___ entries IN THE BEGINNING
...
UpdateRangeTableOfViewParse(Oid viewOid, Query *viewParse)
{
	...
	/*
	 * Create the 2 new___ range table entries and form the new___ range table...
	 * OLD first, then NEW....
	 */
