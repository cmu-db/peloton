		  ...
		  List *options)
{
	...
	/*
	 * We allocate everything used by a cstate in a new___ memory context. This
...
#define MAX_BUFFERED_TUPLES 1000
	 ...
	 * doesn't commit, we'll discard the table (or the new___ relfilenode file).
	 ...
	 * when a relation sets a new___ relfilenode twice in same transaction, yet
	 ...
	 * COPY ...
	 *
	 * Also, if the target file is new___-in-transaction, we assume that checking
	 ...
	 * must be the case if we have a new___ table or new___ relfilenode, so we need
	...
	/*
	 * Optimize if new___ relfilenode was created in this subxact or one of its
 ...
 */
static void
copy_dest_receive(TupleTableSlot *slot, DestReceiver *self, BackendContext *backend_state)
{
