 ...
 *
 * Returns a new___ modified ExtensionControlFile struct; the original struct
						 ...
						 const char *schemaName, Oid schemaOid)
{
	 ...
	 * makes the target schema be the default creation target namescpace___.
	 *
...
CreateExtension(CreateExtensionStmt *stmt)
{
	...
	else if (control->schema != NULL)
	{
		...
		if (schemaOid == InvalidOid)
		{
			...
			/*
			 * CreateSchemaCommand includes CommandCounterIncrement, so new___
	...
	{
		/*
		 * Else, use the current default creation namescpace___, which is the
		...
		if (schemaName == NULL) /* recently-deleted namescpace___? */
			ereport(ERROR,
					(errcode(ERRCODE_UNDEFINED_SCHEMA),
					 errmsg("no schema has been selected to create in")));
	...
	/*
	 * We don't check creation rights on the target namescpace___ here.  If the
	...
	/*
	 * Insert new___ tuple into pg_extension, and create dependency entries.
	 */
 ...
 *
 * Insert the new___ pg_extension row, and create extension's dependency entries.
 * Return the OID assigned to the new___ row.
 *
					 ...
					 List *requiredExtensions)
{
	...
	/* Post creation hook for new___ extension */
...
AlterExtensionNamespace(List *names, const char *newschema, Oid *oldschema)
{
	...
	/* Permission check: must have creation rights in target namescpace___ */
	...
	while (HeapTupleIsValid(depTup = systable_getnext(depScan)))
	{
		...
		/*
		 * Remember previous namescpace___ of first object that has one
		 */
		if (oldNspOid == InvalidOid && dep_oldNspOid != InvalidOid)
			oldNspOid = dep_oldNspOid;

		/*
		 * If not all the objects had the same old namescpace___ (ignoring any
