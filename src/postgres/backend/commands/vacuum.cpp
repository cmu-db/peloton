...
get_rel_oids(Oid relid, const RangeVar *vacrel)
{
	...
	else if (vacrel)
	{
		 ...
		 * process the OID we looked up here, rather than the new___ one. Neither
		 * is ideal, but there's little practical alternative, since we're
		 * going to commit this transaction and begin a new___ one between now
		 * and then.
		 */
...
/*
 * vac_estimate_reltuples() -- estimate the new___ value for pg_class.reltuples
 *
					   ...
					   double scanned_tuples)
{
	 ...
	 * statistics in this situation, since no new___ information has been
	 * provided.)
	 */
	if (scanned_pages == 0)
	 ...
	 * probably did, if the new___ density measurement is different. But over
	 * repeated cycles, the value of reltuples will converge towards the
	 * correct value, if repeated measurements show the same new___ density.
	 *
 ...
 *		existing pg_class tuple with the new___ values.  This is reasonably
 *		safe as long as we're sure that the new___ values are correct whether or
					...
					bool in_outer_xact)
{
	 ...
	 * indicating it has no new___ data.
	 *
	 * Ordinarily, we don't let relfrozenxid go backwards: if things are
	 * working correctly, the only way the new___ frozenxid could be older would
 ...
 *		existing pg_database tuple with the new___ values.  This is reasonably
 *		safe since the new___ values are correct whether or not this transaction
...
vac_update_datfrozenxid(void)
{
	 ...
	 * committed pg_class entries for new___ tables; see AddNewRelationTuple().
	 ...
	 * used on pg_class for new___ tables.  See AddNewRelationTuple().
	 */
				  ...
				  MultiXactId lastSaneMinMulti)
{
	 ...
	 *
	 * Note: we need not worry about a race condition with new___ entries being
	 * inserted by CREATE DATABASE.  Any such entry will have a copy of some
	 * existing DB's datfrozenxid, and that source DB cannot be ours because
	 * of the interlock against copying a DB containing an active backend.
	 * Hence the new___ entry will not reduce the minimum.  Also, if two VACUUMs
	...
	/*
	 * Update the wrap limit for GetNewTransactionId and creation of new___
...
vacuum_rel(Oid relid, RangeVar *relation, int options, VacuumParams *params)
{
	...
	/* all done with this class___, but hold lock until commit */
	if (onerel)
		relation_close(onerel, NoLock);
