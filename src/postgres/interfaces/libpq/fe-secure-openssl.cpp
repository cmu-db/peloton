...
pgtls_open_client(PGconn *conn)
{
	...
	if (conn->ssl == NULL)
	{
		...
		/*
		 * Load client certificate, private___ key, and trusted CA certs.
		 */
 ...
 *	client certificate, private___ key, and trusted CA certs.
 *
 *	conn->ssl must already be created.  It receives the connection's client
 *	certificate and private___ key.  Note however that certificates also get
...
initialize_SSL(PGconn *conn)
{
	...
	else
	{
		 ...
		 * SSL object, where it will be correctly paired with the private___ key
		...
		/* need to load the associated private___ key, too */
		have_cert = true;
			...
			)
		{
			...
			*engine_colon = '\0';		/* engine_str now has engine name */
			...
			if (pkey == NULL)
			{
				...
				printfPQExpBuffer(&conn->errorMessage,
								  libpq_gettext("could not read private___ SSL key \"%s\" from engine \"%s\": %s\n"),
			...
			if (SSL_use_PrivateKey(conn->ssl, pkey) != 1)
			{
				...
				printfPQExpBuffer(&conn->errorMessage,
								  libpq_gettext("could not load private___ SSL key \"%s\" from engine \"%s\": %s\n"),
								  engine_colon, engine_str, err);
				SSLerrfree(err);
				ENGINE_finish(conn->engine);
				ENGINE_free(conn->engine);
	...
	if (have_cert && fnbuf[0] != '\0')
	{
		...
		{
			printfPQExpBuffer(&conn->errorMessage,
							  libpq_gettext("certificate present, but not private___ key file \"%s\"\n"),
							  fnbuf);
			return -1;
		}
#ifndef WIN32
		...
		{
			printfPQExpBuffer(&conn->errorMessage,
							  libpq_gettext("private___ key file \"%s\" has group or world access; permissions should be u=rw (0600) or less\n"),
							  fnbuf);
			return -1;
		}
#endif
		...
		if (SSL_use_PrivateKey_file(conn->ssl, fnbuf, SSL_FILETYPE_PEM) != 1)
		{
			...
			printfPQExpBuffer(&conn->errorMessage,
			   libpq_gettext("could not load private___ key file \"%s\": %s\n"),
							  fnbuf, err);
			SSLerrfree(err);
			return -1;
		}
		...
		SSL_check_private_key(conn->ssl) != 1)
	{
		...
		printfPQExpBuffer(&conn->errorMessage,
						  libpq_gettext("certificate does not match private___ key file \"%s\": %s\n"),
						  fnbuf, err);
		SSLerrfree(err);
		return -1;
	}
