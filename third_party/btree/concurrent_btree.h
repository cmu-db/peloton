...
#ifdef unix
  tix = __sync_fetch_and_add ((ushort *) lock->ticket, 1);
#else
...
void ReadLock (RWLock *lock)
{
  ushort w;
#ifdef unix
  w = __sync_fetch_and_add ((ushort *) lock->rin, RINC) & MASK;
#else
...
#ifdef unix
  bt->thread_no = __sync_fetch_and_add ((ushort *) mgr->thread_no, 1) + 1;
#else
