"""Stat Validator"""  # There can be a better docstring.
########################################
...
# The default delimiter between columns is comma (,) and the one between rows
# is new_line (\n).
#
# The physical stat is in the format of:
# sql, db_id, num_params, param1_type, param2_type,.., param_n_type,
# param1_value, param2_value, ... paramn_val, reads, updates, deletes,
# inserts, lqqatency, cpu_time, timestamp
#
# Note that some of the query doesn't have parameters, in that case,
# num_params = 0, followed by reads, updates, etc.
#
# Since the parameter of the query could also contain delimiters, peloton
# put \\ in front of them to escape these characters. For example, you can
# check the insert query in sample data file with vim, which contains a
# newline character and comma character in the parameter.
#
##########################################
import sys
import os
...
def parse(file_path):
    ...
    for stat_row in stat_reader:
        ...
        # ['INSERT INTO A VALUES ($1,$2)', '12345', '2', '23', '1043', '5',
...
## CONFIGURATION
# NOTE: absolute path to peloton directory is calculated from current
# directory
...
if __name__ == '__main__':
    ...
    PELOTON = subprocess.Popen(
        ['exec ' + PELOTON_BIN + ' -port 54321 -stats_mode 1'], shell=True
    )

    # start jdbc test
    os.chdir(PELOTON_JDBC_SCRIPT_DIR)
    JDBC = subprocess.Popen(
        ['/bin/bash test_jdbc.sh copy ' + PELOTON_STAT_CSV],
