...
public class IndexTest extends PLTestBase {
  ...
  private static final String SQL_DROP_TABLE = "DROP TABLE IF EXISTS tbl;";

  private static final String SQL_CREATE_TABLE = "CREATE TABLE tbl(c1 int, c2 int);";
  ...
  public void test_create_insert() throws SQLException, InterruptedException {
    ...
    if (rs.next()) {
      checkRow(rs, new String[] {"c1", "c2"}, new int[] {-1, -1});
      assertNoMoreRows(rs);
    } else {
      assert(false);
    }
  ...
  public void test_create_update() throws SQLException, InterruptedException {
    ...
    if (rs.next()) {
      checkRow(rs, new String[] {"c1", "c2"}, new int[] {-1, 0});
      assertNoMoreRows(rs);
    } else {
      assert(false);
    }
