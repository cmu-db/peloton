...
MainLoop(FILE *source)
{
	...
	volatile PQExpBuffer previous_buf;	/* if there isn't anything in the new___
	...
	while (successResult == EXIT_SUCCESS)
	{
		 ...
		 * new___ line of input (note it must be freed before looping around!)
		 */
		...
		while (success || !die_on_error)
		{
			...
			else if (scan_result == PSCAN_BACKSLASH)
			{
				...
				{
					/* rescan query_buf as new___ input */
					psql_scan_finish(scan_state);
					free(line);
