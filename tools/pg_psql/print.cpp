...
/*
 * Returns the appropriately formatted string in a new___ allocated block,
 * caller must free
 */
static char *
		...
		while (!dcomplete || !hcomplete)
		{
			...
			if (!hcomplete)
			{
				int			swidth = hwidth,
							target_width = hwidth;
				/*
				 * Left spacer or new___ line indicator
				 */
				if ((opt_border == 2) ||
...
print_asciidoc_text(const printTableContent *cont, FILE *fout)
{
	...
	if (cont->opt->start_table)
	{
		/* print table in new___ paragraph - enforce preliminary new___ line */
		fputs("\n", fout);
 ...
 * content given in *footer.  If there was no previous footer, add a new___ one.
 *
...
strlen_max_width(unsigned char *str, int *target_width, int encoding)
{
	...
	while (str < end)
	{
		...
		/*
		 * If the display width of the new___ character causes the string to
