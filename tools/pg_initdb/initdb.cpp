...
static const char *auth_methods_host[] = {"trust",    "reject", "md5",
                                          "password", "ident",  "radius",
#ifdef ENABLE_GSS
                                          "gss",
#endif
#ifdef ENABLE_SSPI
...
#endif
                                          NULL};
static const char *auth_methods_local[] = {"trust",    "reject", "md5",
                                           "password", "peer",   "radius",
#ifdef USE_PAM
                                           "pam",      "pam ",
#endif
#ifdef USE_LDAP
...
static const char *subdirs[] = {
    "global", "pg_xlog", "pg_xlog/archive_status", "pg_clog", "pg_commit_ts",
    "pg_dynshmem", "pg_notify", "pg_serial", "pg_snapshots", "pg_subtrans",
    "pg_twophase", "pg_multixact/members", "pg_multixact/offsets", "base",
    "base/1", "pg_replslot", "pg_tblspc", "pg_stat", "pg_stat_tmp",
...
static char **replace_token(char **lines, const char *token,
...
static char **replace_token(char **lines, const char *token,
  ...
  for (i = 0; i < numlines; i++) {
    ...
    /* if we get here a change is needed - set up new___ line */
...
static char **readfile(const char *path) {
  ...
  while ((c = fgetc(infile)) != EOF) {
    ...
    if (c == '\n') {
      nlines++;
      if (linelen > maxlength) maxlength = linelen;
      linelen = 0;
    }
  }
...
static void walktblspc_links(char *path,
  ...
  while (errno = 0, (direntry = readdir(dir)) != NULL) {
    ...
    snprintf(subpath, sizeof(subpath), "%s/%s/%s", path, direntry->d_name,
...
static void pre_sync_fname(char *fname, bool isdir) {
   ...
   * EACCES)
   */
  if (fd < 0 && isdir && (errno == EISDIR || errno == EACCES)) return;
...
static void exit_nicely(void) {
  if (!noclean) {
    ...
    if (made_new_xlogdir || found_existing_xlogdir)
      fprintf(stderr, _("%s: transaction log directory \"%s\" not removed at "
                        "user's request\n"),
              progname, xlog_dir);
  }
...
static bool mkdatadir(const char *subdir) {
  ...
  if (pg_mkdir_p(path, S_IRWXU) == 0) return true;
...
#define MIN_BUFS_FOR_CONNS(nconns) ((nconns)*10)
  ...
  static const int trial_bufs[] = {16384, 8192, 4096, 3584, 3072, 2560, 2048,
                                   1536,  1000, 900,  800,  700,  600,  500,
                                   400,   300,  200,  100,  50};
  ...
  for (i = 0; i < connslen; i++) {
    ...
    snprintf(cmd, sizeof(cmd),
             ...
             "< \"%s\" > \"%s\" 2>&1",
             backend_exec, boot_options, test_conns, test_buffs, DEVNULL,
             DEVNULL);
  ...
  for (i = 0; i < bufslen; i++) {
    ...
    snprintf(cmd, sizeof(cmd),
             ...
             "-c dynamic_shared_memory_type=none "
             "< \"%s\" > \"%s\" 2>&1",
             backend_exec, boot_options, n_connections, test_buffs, DEVNULL,
             DEVNULL);
...
static void setup_config(void) {
...
#endif
  conflines =
      replace_token(conflines, "#unix_socket_directories = '/tmp'", repltok);
  ...
  conflines = replace_token(
      conflines, "#default_text_search_config = 'pg_catalog.simple'", repltok);
  ...
  snprintf(repltok, sizeof(repltok), "dynamic_shared_memory_type = %s",
           dynamic_shared_memory_type);
  conflines =
      replace_token(conflines, "#dynamic_shared_memory_type = posix", repltok);

#ifndef USE_PREFETCH
  conflines = replace_token(conflines, "#effective_io_concurrency = 1",
  ...
  autoconflines[1] =
      pg_strdup("# It will be overwritten by the ALTER SYSTEM command.\n");
  ...
  conflines = replace_token(conflines, "@authcomment@",
...
static void bootstrap_template1(void) {
  ...
  if (strcmp(headerline, *bki_lines) != 0) {
    fprintf(stderr, _("%s: input file \"%s\" does not belong to PostgreSQL %s\n"
  ...
  snprintf(cmd, sizeof(cmd), "\"%s\" --boot -x1 %s %s %s", backend_exec,
           data_checksums ? "-k" : "", boot_options, talkargs);
...
static void setup_auth(void) {
  ...
  snprintf(cmd, sizeof(cmd), "\"%s\" %s template1 >%s", backend_exec,
...
static void setup_depend(void) {
  ...
  static const char *pg_depend_setup[] = {
      ...
      /*
       * restriction here to avoid pinning the public cnamescpace
       */
      "INSERT INTO pg_depend SELECT 0,0,0, tableoid,oid,0, 'p' "
      " FROM pg_namespace "
  ...
  for (line = pg_depend_setup; *line != NULL; line++) PG_CMD_PUTS(*line);
...
static void setup_sysviews(void) {
   ...
   */
  snprintf(cmd, sizeof(cmd), "\"%s\" %s -j template1 >%s", backend_exec,
           backend_options, DEVNULL);
 ...
 * -> "br_FR@euro").  Return true if a new___, different name was
 * generated.
 */
static bool normalize_locale_name(char *new___, const char *old) {
  char *n = new___;
  const char *o = old;
  bool changed = false;
...
static void setup_collation(void) {
  ...
  if (!locale_a_handle) return; /* complaint already printed */
  ...
  while (fgets(localebuf, sizeof(localebuf), locale_a_handle)) {
    ...
    if (skip) {
      if (debug)
        fprintf(
            stderr,
            _("%s: locale name has non-ASCII characters, skipped: \"%s\"\n"),
            progname, localebuf);
      continue;
    }
    ...
    if (enc == PG_SQL_ASCII) continue; /* C/POSIX are already in the catalog */
    ...
    if (normalize_locale_name(alias, localebuf)) {
      ...
      PG_CMD_PRINTF3(
          "INSERT INTO tmp_pg_collation VALUES (E'%s', E'%s', %d);\n",
  ...
  PG_CMD_PRINTF1(
      "INSERT INTO tmp_pg_collation VALUES ('ucs_basic', 'C', %d);\n", PG_UTF8);
   ...
   */
  PG_CMD_PUTS(
      "INSERT INTO pg_collation (collname, collnamespace, collowner, "
      "collencoding, collcollate, collctype) "
      " SELECT DISTINCT ON (collname, encoding)"
      "   collname, "
      "   (SELECT oid FROM pg_namespace WHERE nspname = 'pg_catalog') AS "
      "collnamespace, "
      "   (SELECT relowner FROM pg_class WHERE relname = 'pg_collation') AS "
      "collowner, "
      "   encoding, locale, locale "
      "  FROM tmp_pg_collation"
      "  WHERE NOT EXISTS (SELECT 1 FROM pg_collation WHERE collname = "
      "tmp_pg_collation.collname)"
...
static void setup_conversion(void) {
  ...
  for (line = conv_lines; *line != NULL; line++) {
    if (strstr(*line, "DROP CONVERSION") != *line) PG_CMD_PUTS(*line);
    free(*line);
  }
...
static void setup_privileges(void) {
  ...
  for (line = priv_lines; *line != NULL; line++) PG_CMD_PUTS(*line);
...
static void set_info_version(void) {
  ...
  while (ptr != vstr && (*ptr < '0' || *ptr > '9')) ptr--;
  letterversion = ptr + 1;
  major = strtol(vstr, &endptr, 10);
  if (*endptr) minor = strtol(endptr + 1, &endptr, 10);
  if (*endptr) micro = strtol(endptr + 1, &endptr, 10);
  snprintf(infoversion, sizeof(infoversion), "%02ld.%02ld.%04ld%s", major,
           minor, micro, letterversion);
}
...
static void make_template0(void) {
  ...
  for (line = template0_setup; *line; line++) PG_CMD_PUTS(*line);
...
static void make_postgres(void) {
  ...
  for (line = postgres_setup; *line; line++) PG_CMD_PUTS(*line);
 ...
 *	 Win32 operating systems generate a new___ thread to specifically handle
...
static inline size_t my_strftime(char *s, size_t max, const char *fmt,
                                 const struct tm *tm) {
...
static int locale_date_order(const char *locale) {
  ...
  if (!posM || !posD || !posY) return result;
...
static void check_locale_name(int category, const char *locale,
  ...
  if (canonname) *canonname = NULL; /* in case of failure */
  ...
  if (res && canonname) *canonname = pg_strdup(res);
  ...
  if (res == NULL) {
    if (*locale)
       ...
       */
      fprintf(stderr, _("%s: invalid locale settings; check LANG and LC_* "
...
static bool check_locale_encoding(const char *locale, int user_enc) {
  ...
  if (!(locale_enc == user_enc || locale_enc == PG_SQL_ASCII ||
        locale_enc == -1 ||
#ifdef WIN32
        user_enc == PG_UTF8 ||
#endif
    ...
    fprintf(stderr,
            ...
            pg_encoding_to_char(user_enc), pg_encoding_to_char(locale_enc),
            progname);
    return false;
  }
  return true;
...
static void setlocales(void) {
  ...
  if (strlen(locale) > 0) {
    if (strlen(lc_ctype) == 0) lc_ctype = locale;
    if (strlen(lc_collate) == 0) lc_collate = locale;
    if (strlen(lc_numeric) == 0) lc_numeric = locale;
    if (strlen(lc_time) == 0) lc_time = locale;
    if (strlen(lc_monetary) == 0) lc_monetary = locale;
    if (strlen(lc_messages) == 0) lc_messages = locale;
  }
...
static void usage(const char *progname) {
  ...
  printf(_("\nOptions:\n"));
  printf(
      _("  -A, --auth=METHOD         default authentication method for local "
        "connections\n"));
  printf(
      _("      --auth-host=METHOD    default authentication method for local "
        "TCP/IP connections\n"));
  printf(
      _("      --auth-local=METHOD   default authentication method for "
        "local-socket connections\n"));
  printf(_(" [-D, --pgdata=]DATADIR     location for this database cluster\n"));
  printf(_(
      "  -E, --encoding=ENCODING   set default encoding for new databases\n"));
  printf(
      _("      --locale=LOCALE       set default locale for new databases\n"));
  printf(
      _("      --lc-collate=, --lc-ctype=, --lc-messages=LOCALE\n"
        "      --lc-monetary=, --lc-numeric=, --lc-time=LOCALE\n"
        "                            set default locale in the respective "
        "category for\n"
        "                            new databases (default taken from "
        "environment)\n"));
  printf(_("      --no-locale           equivalent to --locale=C\n"));
  printf(
      _("      --pwfile=FILE         read password for the new superuser from "
        "file\n"));
  printf(
      _("  -T, --text-search-config=CFG\n"
        "                            default text search configuration\n"));
  printf(_("  -U, --username=NAME       database superuser name\n"));
  printf(
      _("  -W, --pwprompt            prompt for a password for the new "
        "superuser\n"));
  printf(
      _("  -X, --xlogdir=XLOGDIR     location for the transaction log "
        "directory\n"));
  ...
  printf(
      _("  -N, --nosync              do not wait for changes to be written "
        "safely to disk\n"));
  ...
  printf(
      _("\nIf the data directory is not specified, the environment variable "
        "PGDATA\n"
        "is used.\n"));
...
static void check_authmethod_valid(const char *authmethod,
  ...
  for (p = valid_methods; *p; p++) {
    if (strcmp(authmethod, *p) == 0) return;
    /* with space = param */
...
static void check_need_password(const char *authmethodlocal,
  ...
  if ((strcmp(authmethodlocal, "md5") == 0 ||
      ...
      !(pwprompt || pwfilename)) {
    fprintf(stderr, _("%s: must specify a password for the superuser to enable "
                      "%s authentication\n"),
            progname, (strcmp(authmethodlocal, "md5") == 0 ||
                       strcmp(authmethodlocal, "password") == 0)
                          ? authmethodlocal
                          : authmethodhost);
    exit(1);
  ...
  if (strlen(pg_data) == 0) {
    ...
    if (pgdata_get_env && strlen(pgdata_get_env)) {
      ...
      fprintf(stderr, _("%s: no data directory specified\n"
                        "You must identify the directory where the data for "
...
void setup_bin_paths(const char *argv0) {
  ...
  if ((ret = find_other_exec(argv0, "peloton", PG_BACKEND_VERSIONSTR,
                             backend_exec)) < 0) {
    ...
    if (ret == -1)
      fprintf(stderr, _("The program \"peloton\" is needed by %s "
                        "but was not found in the\n"
                        "same directory as \"%s\".\n"
                        "Check your installation.\n"),
              progname, full_path);
    else
      fprintf(stderr, _("The program \"peloton\" was found by \"%s\"\n"
...
void setup_locale_encoding(void) {
  ...
  if (strcmp(lc_ctype, lc_collate) == 0 && strcmp(lc_ctype, lc_time) == 0 &&
      strcmp(lc_ctype, lc_numeric) == 0 && strcmp(lc_ctype, lc_monetary) == 0 &&
 ...
 */
#ifdef WIN32
      printf(
          _("Encoding \"%s\" implied by locale is not allowed as a server-side "
            "encoding.\n"
      ...
      encodingid = encodingid_to_string(ctype_enc);
      printf(_("The default database encoding has accordingly been set to "
               "\"%s\".\n"),
...
void setup_data_file_paths(void) {
  ...
  if (show_setting || debug) {
    fprintf(stderr,
            ...
            "PG_HBA_SAMPLE=%s\nPG_IDENT_SAMPLE=%s\n",
            PG_VERSION, pg_data, share_path, bin_path, username, bki_file,
            desc_file, shdesc_file, conf_file, hba_file, ident_file);
    if (show_setting) exit(0);
  }
...
void setup_text_search(void) {
  if (strlen(default_text_search_config) == 0) {
    ...
    if (default_text_search_config == NULL) {
      printf(_("%s: could not find suitable text search configuration for "
               "locale \"%s\"\n"),
             progname, lc_ctype);
    ...
    if (checkmatch == NULL) {
      printf(_("%s: warning: suitable text search configuration for locale "
               "\"%s\" is unknown\n"),
             progname, lc_ctype);
    } else if (strcmp(checkmatch, default_text_search_config) != 0) {
      printf(_("%s: warning: specified text search configuration \"%s\" might "
               "not match locale \"%s\"\n"),
  ...
  switch ((ret = pg_check_dir(pg_data))) {
    ...
    case 4:
      ...
      if (ret != 4)
        ...
        fprintf(stderr, _("If you want to create a new database system, either "
                          "remove or empty\n"
...
void create_xlog_symlink(void) {
  ...
  if (strcmp(xlog_dir, "") != 0) {
    ...
    if (!is_absolute_path(xlog_dir)) {
      fprintf(stderr, _("%s: transaction log directory location must be an "
                        "absolute path\n"),
              progname);
      exit_nicely();
    }
...
void warn_on_mount_point(int error) {
  if (error == 2)
    fprintf(stderr, _("It contains a dot-prefixed/invisible file, perhaps due "
                      "to it being a mount point.\n"));
  else if (error == 3)
    fprintf(stderr, _("It contains a lost+found directory, perhaps due to it "
                      "being a mount point.\n"));

  fprintf(stderr, _("Using a mount point directly as the data directory is not "
                    "recommended.\n"
...
void initialize_data_directory(void) {
  ...
  for (i = 0; i < static_cast<int>((sizeof(subdirs) / sizeof(char *))); i++) {
    if (!mkdatadir(subdirs[i])) exit_nicely();
  }
  ...
  setup_auth();
  if (pwprompt || pwfilename) get_set_pwd();
...
int main(int argc, char *argv[]) {
  ...
  while ((c = getopt_long(argc, argv, "dD:E:kL:nNU:WA:sST:X:", long_options,
                          &option_index)) != -1) {
    switch (c) {
      case 'A':
      ...
      case 'n':
        noclean = true;
        printf(
            _("Running in noclean mode.  Mistakes will not be cleaned up.\n"));
        break;
      case 'N':
        do_sync = false;
        break;
  ...
  if (pwprompt && pwfilename) {
    fprintf(stderr, _("%s: password prompt and password file cannot be "
                      "specified together\n"),
            progname);
    exit(1);
  }
  ...
  if (strlen(username) == 0) username = effective_user;
  ...
  if (do_sync)
    ...
    printf(
        _("\nSync to disk skipped.\nThe data directory might become corrupt if "
          "the operating system crashes.\n"));

  if (authwarning != NULL) fprintf(stderr, "%s", authwarning);
