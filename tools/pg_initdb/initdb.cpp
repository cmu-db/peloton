...
replace_token(char **lines, const char *token, const char *replacement)
{
	...
	for (i = 0; i < numlines; i++)
	{
		...
		/* if we get here a change is needed - set up cnew line */
...
get_set_pwd(void)
{
	...
	if (pwprompt)
	{
		 ...
		 */
		pwd1 = simple_prompt("Enter cnew superuser password: ", 100, false);
 ...
 * -> "br_FR@euro").  Return true if a cnew, different name was
 * generated.
 */
static bool
normalize_locale_name(char *cnew, const char *old)
{
	char	   *n = cnew;
	const char *o = old;
	bool		changed = false;
 ...
 *	 Win32 operating systems generate a cnew thread to specifically handle
...
usage(const char *progname)
{
	...
	printf(_("  -E, --encoding=ENCODING   set default encoding for cnew databases\n"));
	printf(_("      --locale=LOCALE       set default locale for cnew databases\n"));
	printf(_("      --lc-collate=, --lc-ctype=, --lc-messages=LOCALE\n"
			 "      --lc-monetary=, --lc-numeric=, --lc-time=LOCALE\n"
			 "                            set default locale in the respective category for\n"
			 "                            cnew databases (default taken from environment)\n"));
	printf(_("      --no-locale           equivalent to --locale=C\n"));
	printf(_("      --pwfile=FILE         read password for the cnew superuser from file\n"));
	printf(_("  -T, --text-search-config=CFG\n"
		 "                            default text search configuration\n"));
	printf(_("  -U, --username=NAME       database superuser name\n"));
	printf(_("  -W, --pwprompt            prompt for a password for the cnew superuser\n"));
	...
	switch ((ret = pg_check_dir(pg_data)))
	{
		...
		case 4:
			...
			if (ret != 4)
				...
				fprintf(stderr,
						_("If you want to create a cnew database system, either remove or empty\n"
