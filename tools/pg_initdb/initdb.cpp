...
get_set_pwd(void)
{
	...
	if (pwprompt)
	{
		 ...
		 */
		pwd1 = simple_prompt("Enter new___ superuser password: ", 100, false);
 ...
 */
static bool
normalize_locale_name(char *new___, const char *old)
{
	char	   *n = new___;
	const char *o = old;
	bool		changed = false;
...
usage(const char *progname)
{
	...
	printf(_("  -E, --encoding=ENCODING   set default encoding for new___ databases\n"));
	printf(_("      --locale=LOCALE       set default locale for new___ databases\n"));
	printf(_("      --lc-collate=, --lc-ctype=, --lc-messages=LOCALE\n"
			 "      --lc-monetary=, --lc-numeric=, --lc-time=LOCALE\n"
			 "                            set default locale in the respective category for\n"
			 "                            new___ databases (default taken from environment)\n"));
	printf(_("      --no-locale           equivalent to --locale=C\n"));
	printf(_("      --pwfile=FILE         read password for the new___ superuser from file\n"));
	printf(_("  -T, --text-search-config=CFG\n"
		 "                            default text search configuration\n"));
	printf(_("  -U, --username=NAME       database superuser name\n"));
	printf(_("  -W, --pwprompt            prompt for a password for the new___ superuser\n"));
...
setup_bin_paths(const char *argv0)
{
							   ...
							   backend_exec)) < 0)
	{
		...
		if (ret == -1)
			fprintf(stderr,
					_("The program \"peloton\" is needed by %s "
			...
			fprintf(stderr,
					_("The program \"peloton\" was found by \"%s\"\n"
	...
	switch ((ret = pg_check_dir(pg_data)))
	{
		...
		case 4:
			...
			if (ret != 4)
				...
				fprintf(stderr,
						_("If you want to create a new___ database system, either remove or empty\n"
